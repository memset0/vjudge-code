...
inline vector<int> split(vector<int> a){
  vector<int> b;
  for(int i=0;i<a.size();i++){
    while(a[i]>3)b.push_back(3),a[i]-=3,++ans;
    b.push_back(a[i]);
  }
  return b;
}
int count(const vector<int> &a,int y){
  int res=0;
  for(int x:a)res+=x==y;
  return res;
}
int main(){
  read(T);
  while(T--){
    read(n),a.clear(),ans=0;
    for(int i=1;i<=n;i++)read(p[i]),vis[i]=false;
    for(int i=1;i<=n;i++)if(!vis[i]){
      a.push_back(0);
      for(int u=i;!vis[u];u=p[u])vis[u]=true,++a.back();
    }
    if(n%3==0){
      println(fpow(3,n/3));
      make_three(a);
      ...
      println(4ll*fpow(3,(n-4)/3)%mod);

    }else if(n%3==2){
      println(2ll*fpow(3,n/3)%mod);
      a=split(a);
      if(a.find(2)!=a.end()){
        a.erase(a.find(2));
      }else{
