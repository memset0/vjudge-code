...
const int N=510;
int T,n,m,a[N][N],b[N][N],tag[N*N],rnk[N],id[N];
...
int main(){
	...
	while(T--){
		...
		for(int i=1;i<=m;i++)
			for(int j=1;j<=n;j++)
				read(b[i][j]);
		for(int i=1;i<=n;i++)tag[b[1][i]]=i;
		for(int i=1;i<=n;i++){
			rnk[i]=0;
			for(int j=1;j<=m;j++)rnk[i]|=tag[a[i][j]];
		}
		for(int i=1;i<=n;i++)id[rnk[i]]=i;
		for(int i=1;i<=n;i++)
			for(int j=1;j<=m;j++)
				print(a[id[i]][j]," \n"[j==m]);
	}
