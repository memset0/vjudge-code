...
inline int count(const vector<int> &vec,int x){
	int cnt=0;
	for(int v:vec)cnt+=v==x;
	return cnt;
}
...
//		log(">> "); for(int i=1;i<=m;i++)log("%d%c",a[i]," \n"[i==m]);
		ans=0;
		vector<int> t;
		for(int i=1;i<=m;i++){
			int x=a[i];
		...
		if(n%3==0){
			println(fpow(3,n/3),' ',ans+calc(count(t,1),count(t,2)));
		}else if(n%3==1){
			print(fpow(3,n/3-1)*4ll%mod,' ');
			bool flag=false;
			for(int i=1;i<=m;i++)
				if(a[i]>=4&&a[i]%3==1){
					flag=true;
...
//			log(">> flag = %d\n",flag);
			int cur=INT_MAX;
			if(flag){
				cur=min(cur,-1+calc(count(t,1)-1,count(t,2)));
			}
			if(count(t,1)&&count(t,3)){
				cur=min(cur,1+calc(count(t,1)-1,count(t,2)));
			}
			if(count(t,2)>=2){
				cur=min(cur,calc(count(t,1),count(t,2)-2));
			}
			if(count(t,1)>=4){
				cur=min(cur,2+calc(count(t,1)-4,count(t,2)));
			}
			if(count(t,1)>=2&&count(t,2)){
				cur=min(cur,1+calc(count(t,1)-2,count(t,2)-1));
			}
			println(ans+cur);
		}else{
			print(fpow(3,n/3)*2ll%mod,' ');
			if(count(t,2)){
				println(ans+calc(count(t,1),count(t,2)-1));
			}else{
				println(ans+calc(count(t,1)-2,count(t,2))+1);
			}
		}
	}
}/////