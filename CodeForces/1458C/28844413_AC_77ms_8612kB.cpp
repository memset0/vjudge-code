...
int T,n,m,res[3],pos[3],tag[3],a[N][N],b[N][N];
char s[M];
...
inline int getV(int i,int j,int c){
  if(pos[c]==0)return i;
  if(pos[c]==1)return j;
  return b[i][j];
}
int main(){
  ...
  while(T--){
    ...
    for(int i=1;i<=n;i++)
      for(int j=1;j<=n;j++){
        res[0]=getV(i,j,0)+tag[0];
        res[1]=getV(i,j,1)+tag[1];
        res[2]=getV(i,j,2)+tag[2];
        // log("%d >> %d %d %d\n",i,res[0],res[1],res[2]);
