...
inline void pushup(int u,int x){p[u].tag=x,p[u].sum=x?p[u].r-p[u].l+1:0;}
inline void pushdown(int u){if(~p[u].tag&&p[u].l!=p[u].r)pushup(u<<1,p[u].tag),pushup(u<<1|1,p[u].tag),p[u].tag=-1;}
...
bool check(int u){
  pushdown(u);
  if(p[u].l==p[u].r){
    return f[p[u].l]=='1'?(p[u].sum==1):(p[u].sum==0);
  }
...
int main(){
  ...
  read(T);
  while(T--){
    read(n,q),read((char*)(f+1)),read((char*)(g+1));
