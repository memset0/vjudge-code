...
int main(){
  ...
  for(int i=1;i<=n;i++){
    for(int j=1;j<=n;j++){
      p[i][j]=j;
      dis[j]=(long long)(x[i]-x[j])*(x[i]-x[j])+(long long)(y[i]-y[j])*(y[i]-y[j]);
    }
    sort(p[i]+1,p[i]+n+1,[&](int x,int y){
      return dis[x]>dis[y];
    });
    // for(int j=1;j<=n;j++)printf("%d(%lld)%c",p[i][j],dis[p[i][j]]," \n"[j==n]);
  }
  for(int u=1,i=1;i<=n;i++){
    print(u," \n"[i==n]);
    vis[u]=true;
    for(int i=1;i<=n;i++)if(!vis[p[u][i]]){
      u=p[u][i];
      break;  
    }
  }
