...
using namespace mem;
const int N=1e5+10,M=N*40,inf=1e9+10;
int n,m,t,rt,nod,anc[N],l[N],r[N],ch[M][2];
vector<int> vec[M];
inline int find(int x){return x==anc[x]?x:anc[x]=find(anc[x]);}
inline void merge(int u,int v){
  // log("merge %d %d\n",u,v);
  if((u=find(u))==(v=find(v)))return;
  l[v]=min(l[u],l[v]),r[v]=max(r[u],r[v]);
  anc[u]=v;
}
void insert(int &u,int ql,int qr,int x,int l,int r){
  if(!u)u=++nod;
  // log("insert %d %d %d %d %d %d\n",u,ql,qr,x,l,r);
  if(ql==l&&qr==r){
    vec[u].push_back(x);
    return;
  }
  int mid=(l+r)>>1;
  if(qr<=mid)insert(ch[u][0],ql,qr,x,l,mid);
  else if(ql>mid)insert(ch[u][1],ql,qr,x,mid+1,r);
  else{
    insert(ch[u][0],ql,mid,x,l,mid);
    insert(ch[u][1],mid+1,qr,x,mid+1,r);
  }
}
void update(int u,int k,int x,int l,int r){
  if(!u)return;
  // log("update %d %d %d %d %d [%d]\n",u,k,x,l,r,(int)vec[u].size());
  if(vec[u].size()){
    for(int y:vec[u])merge(x,y);
    vec[u].clear(),vec[u].push_back(x); 
  }
  int mid=(l+r)>>1;
  if(k<=mid)update(ch[u][0],k,x,l,mid);
  else update(ch[u][1],k,x,mid+1,r);
}
int main(){
#ifdef memset0
  ...
  read(n),rt=nod=1;
  for(int a,b,i=1;i<=n;i++)if(read<int>()==1){
    ++t,read(l[t],r[t]);
    log("> insert[%d] (%d,%d)\n",t,l[t],r[t]);
    if(l[t]+1<=r[t]-1)insert(rt,l[t]+1,r[t]-1,t,-inf,inf);
    update(rt,l[t],t,-inf,inf);
    update(rt,r[t],t,-inf,inf);
  }else{
    read(a,b),a=find(a),b=find(b);
    log("> query(%d,%d)\n",a,b);
    println(a==b||l[b]<l[a]&&l[a]<r[b]||l[b]<r[a]&&r[a]<r[b]?"YES":"NO");
  }
}