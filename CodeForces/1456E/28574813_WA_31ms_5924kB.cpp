...
int n,m;
long long c[N],l[N],r[N],val[N][N][2][2],dp[N][N][N][2][2],tr[N][N][2][2];
template<class T> inline void updmin(T &x,const T &y){if(y<x)x=y;}
  ...
  for(int k=0;k<n;k++){
    for(int i=1;i<m;i++)
      for(int x=0;x<2;x++)
        for(int y=0;y<2;y++){
          ...
          int b=y?r[k+1]:l[k+1];
          val[i][k][x][y]=inf;
          if(x==0&&(a>>i&1)==1)continue;
          if(x==1&&(a>>i&1)==0)continue;
          if(y==0&&(b>>i&1)==1)continue;
          if(y==1&&(b>>i&1)==0)continue;
          if((l[k]>>(i+1))==(r[k]>>(i+1)))continue;
          if((l[k+1]>>(i+1))==(r[k+1]>>(i+1)))continue;
          log("[%d] %d<=>%d :: %d %d %d %d !!\n",i,k,k+1,x,y,a,b);
          val[i][k][x][y]=0;
          for(int k=i+1;k<m;k++)
            if((a>>k&1)^(b>>k&1)){
    ...
    for(int x=0;x<2;x++)
      for(int y=0;y<2;y++){
        ...
        for(int a=0;a<2;a++)
          for(int b=0;b<2;b++){
            int u=(x?r[k]:l[k])>>1<<1|a;
            int v=(y?r[k+1]:l[k+1])>>1<<1|b;
            if(l[k]<=u&&u<=r[k]&&l[k+1]<=v&&v<=r[k+1]){
              long long cur=a==b?0:c[0];
              for(int k=1;k<m;k++)
                if((u>>k&1)^(v>>k&1)){
                  cur+=c[k];
                }
              log("val %d,%d,%d,%d (%d,%d) (%d,%d) %lld\n",0,k,x,y,a,b,u,v,cur);
              updmin(val[0][k][x][y],cur);
            }
          }
      }
  }
  ...
  for(int i=m-1;i>=0;i--){
    for(int l=0;l<N;l++)
      for(int r=0;r<N;r++)
        for(int x=0;x<2;x++)
          for(int y=0;y<2;y++){
            tr[l][r][x][y]=l<=r?inf:0;
          }
    for(int l=1;l<=n;l++)
      for(int r=l;r<=n;r++){
        for(int x=0;x<2;x++)
          for(int a=0;a<2;a++){
            if(l>1&&a!=((x?::r[l]:(::l[l]))>>i&1))continue;
            for(int y=0;y<2;y++)
              for(int b=0;b<2;b++){
                if(r<n&&b!=((y?::r[r]:(::l[r]))>>i&1))continue;
                updmin(dp[i][l][r][x][y],dp[i+1][l][r][x][y]+(a==b?0:c[i]));
              }
          }
        for(int x=0;x<2;x++)
          for(int y=0;y<2;y++){
            updmin(tr[l][r][x][y],dp[i+1][l][r][x][0]+val[i][r][0][y]);
            updmin(tr[l][r][x][y],dp[i+1][l][r][x][1]+val[i][r][1][y]);
          }
      }
            ...
            for(int a=0;a<2;a++){
              ...
              for(int y=0;y<2;y++)
                for(int b=0;b<2;b++){
                  if(r<n&&b!=((y?::r[r]:(::l[r]))>>i&1))continue;
                  for(int z=0;z<2;z++){
                    updmin(dp[i][l][r][x][y],tr[l][k][x][z]+dp[i+1][k+1][r][z][y]+(a==b?0:c[i]));
                    updmin(tr[l][r][x][y],tr[l][k][x][z]+dp[i+1][k+1][r][z][0]+val[i][r][0][y]);
                    updmin(tr[l][r][x][y],tr[l][k][x][z]+dp[i+1][k+1][r][z][1]+val[i][r][1][y]);
                  }
                }
            }
        }
    ...
    for(int l=0;l<N;l++)
      for(int r=l;r<N;r++)
        for(int x=0;x<2;x++)
          for(int y=0;y<2;y++)
            if(dp[i][l][r][x][y]!=inf){
              log("%d,%d,%d,%d,%d = %lld\n",i,l,r,x,y,dp[i][l][r][x][y]);
            }
  long long ans=inf;
  for(int x=0;x<2;x++)
    for(int y=0;y<2;y++){
      updmin(ans,dp[0][1][n][x][y]);
    }
  println(ans);
}