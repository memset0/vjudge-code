...
int main() {
	...
	for (int i = 0; i < N; i++) { A[i + 3 * N] = A[i + 2 * N] = A[i + N] = A[i]; }

	pref[0] = 0;
	for (int i = 0; i < 4 * N; i++) { pref[i + 1] = pref[i] + A[i]; }
	...
	for (int st = bestStart; st < bestStart + N;) {
		...
		auto canMove = [&](int a) {
			...
			assert(st <= a && b <= en);
			assert((a - st) % (K - 1) == 0);
			return (pref[a] - pref[st] <= 0 || pref[a + 1] - pref[st] <= 0) &&
				(pref[b] - pref[st] >= 0 || pref[b - 1] - pref[st] >= 0);
		};
		unordered_set<int> moves;
		for (int a = st; a + K <= en; a += K - 1) {
			if (canMove(a)) { moves.insert(a); }
		}
		while (!moves.empty()) {
			auto it = moves.begin();
			int a = *it;
			...
			moved.insert(a);
			int b = a + K;
			for (int i = 1; i <= K - 1; i++) { pref[a + i] = pref[st]; }

			cout << (a % N);
			for (int i = 0; i < K; i++) {
				if (a + i < bestStart + N) { A[(a + i) % N] = pref[a + i + 1] - pref[a + i]; }
				cout << " " << A[(a + i) % N] + avg;
			}
			cout << '\n';
