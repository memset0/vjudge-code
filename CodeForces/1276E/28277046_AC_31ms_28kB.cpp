...
void move(vector<int> &p, int x, int y, int id) {
  if (p[x] == p[y])
    return;
  if (id == 1) {
    s1.push_back(make_pair(p[x], p[y]));
    p[x] = 2 * p[y] - p[x];
  } else {
    s2.push_back(make_pair(p[x] = 2 * p[y] - p[x], p[y]));
  }
}
...
void work2(vector<int> &A, vector<int> &B, int id) {
  ...
  int length = (B[0] - A[0]) / 2;
  while (A.back() - A[0] < length)
    extend();
  while (1) {
    while (length >= A.back() - A[0]) {
      length -= A.back() - A[0];
      shift();
    }
    if (A.back() - A[0] == 1)
