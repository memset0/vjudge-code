...
int n,q,a[N];
int top[N],siz[N],dfn[N],idfn[N],dep[N],fa[N];
...
pair<int,int> solve(int u,int l,int r){
  vector<Section> cur;
  auto it=--seq.upper_bound({l,0,0});
  ...
  for(const auto &it:cur){
    int ql=std::max(l,it.l);
    int qr=std::min(r,it.r);

  }

  for(const auto &it:cur)seq.erase(it);
  if(cur.front().l<l)seq.insert(Section{cur.front().l,l-1,cur.front().w});
  seq.insert(Section{l,r,u});
  if(cur.back().r>r)seq.insert(Section{r+1,cur.back().r,cur.back().w});
}
...
int main(){
  ...
  dfs(1);
  dfs(1,1);
  seq.push(Section{1,n,-N});
  for(int i=1;i<=q;i++){
    read(x);
    auto ans=solve(i,dfn[x],dfn[x]+siz[x]-1);
