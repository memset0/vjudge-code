...
namespace mem{ //v1.8.4 => size: 13.13KiB
  #define MEM_IO
  #define MEM_STR
  #define MEM_MATH
  #define MEM_UTILS
...
int n,m,w,h,ans,a[N],b[N];
bitset A,B,C,D;

void out(const bitset &a,int n){
  for(int i=0;i<=n;i++)putchar(a[i]?'1':'0');
  putchar('\n');
}

bitset init(bitset a){
  bitset res;
  for(int i=0;i<N;i++)if(a[i])a.flip(i),res|=a>>i,a.flip(i);
  return res;
}
...
#endif
  read(w,h,n,m);
  for(int i=1;i<=n;i++)read(a[i]),A[a[i]]=1;
  A=init(A);
  for(int i=1;i<=m;i++)read(b[i]),B[b[i]]=C[b[i]]=D[h-b[i]]=1;
  B=init(B);
  for(int i=0;i<=h;i++)ans=std::max(ans,(int)((B|(C>>i)|(D>>h-i))&A).count());
  printf("%d\n",ans);
}
...
int n,m,w,h,k,lim=1,ans,a[N],b[N];

using std::bitset;
bitset<N> ca,cb,fa,fb,f,g,p,q;

using namespace mem::modint;
using z=Z<mod>;
int rev[N<<2];
z ta[N<<2],tb[N<<2],wi[N<<2];

void out(const bitset<N> &a,int n){
  for(int i=0;i<=n;i++)putchar(a[i]?'1':'0');
  putchar('\n');
}

void dft(z *a,int lim){
  for(int i=0;i<lim;i++)if(i<rev[i])std::swap(a[i],a[rev[i]]);
  for(int len=1;len<lim;len<<=1)
    for(int i=0;i<lim;i+=(len<<1))
      for(int j=0;j<len;j++){
        z x=a[i+j],y=a[i+j+len]*wi[j+len];
        a[i+j]=x+y,a[i+j+len]=x-y;
      }
}

void init(int n,const bitset<N> &f,bitset<N> &g){
  memset(ta,0,lim<<2),memset(tb,0,lim<<2);
  for(int i=0;i<=n;i++)if(f[i])ta[i]=tb[N-i]=1;
  dft(ta,lim),dft(tb,lim);
  for(int i=0;i<lim;i++)ta[i]*=tb[i];
  std::reverse(ta+1,ta+lim),dft(ta,lim);
  for(int i=1;i<=n;i++)if(ta[i+N].x)g[i]=1;
}
...
int main(){
  ...
  for(int len=1;len<lim;len<<=1){
    z wn=fpow(z(3),(mod-1)/(len<<1));wi[len]=1;
    for(int i=1;i<len;i++)wi[i+len]=wi[i+len-1]*wn;
  }
  read(w,h,n,m);
  for(int i=1;i<=n;i++)read(a[i]),ca[a[i]]=1;
  for(int i=1;i<=n;i++)fa|=ca>>a[i];
  fa[0]=0;
  // init(w,ca,fa);
  for(int i=1;i<=m;i++)read(b[i]),cb[b[i]]=1;
  for(int i=1;i<=m;i++)fb|=cb>>b[i];
  fb[0]=0;
  // init(h,cb,fb);
  q=fa&fb,p=fa^q,f=fb;
  for(int i=0;i<=h;i++)if(f[i])g[h-i]=1;
  // out(q,std::max(w,h));
  // out(p,w);
  // out(f,h);
  // out(g,h);
  for(int i=0;i<=h;i++){
    ans=std::max(ans,(int)((fb|(f>>i)|(g>>(h-i)))&fa).count());
  }
  printf("%d\n",ans);
}
...
const int N=1e5+10;
int n,m,l,w,h,ans,sum,cur,a[N],b[N],c[N],f[N],g[N];
