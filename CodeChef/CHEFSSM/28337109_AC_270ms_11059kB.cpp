...
poly mul(poly a,poly b,int l=-1){
...
poly inv(const poly &f,int len=-1){
  ...
  poly a(&f[0],&f[len]),b=inv(f,(len+1)>>1); int lim=init((len<<1)-1);
  dft(a,lim),dft(b,lim);
...
int main(){
  ...
  while(T--){
    ...
    for(int i=0;i<=n;i++){
      G[i]=ifac[i+1];
      F[i]=G[i]*fpow(z(m+1),i+1);
    }
    G=inv(G),F=mul(F,G);
    for(int i=0;i<=n;i++)F[i]*=fac[i];
    S=solve(1,n),ans=0;
    for(int i=0;i<=n;i++)ans+=S[i]*F[i];
    for(int i=1;i<=n;i++)ans*=fpow(a[i],mod-2);
    println(ans-1);
    // for(int i=0;i<=n;i++)log<RED>("%d%c",F[i].x," \n"[i==n]);
    // for(int i=0;i<=n;i++)log<RED>("%d%c",G[i].x," \n"[i==n]);
    // for(int i=0;i<=n;i++)log<RED>("%d%c",S[i].x," \n"[i==n]);
  next:
    continue;
  }
}