...
struct node{
  ...
  // inline node simple_jump()const{
  //   if(abs(x)>(y<<1)){
  //     return {x>0?x-(y<<1):x+(y<<1),y};
  //   }else if(y>(abs(x)<<1)){
  //     return {x,y-(abs(x)<<1)};
  //   }else if(abs(x)<y){
  //     return {-x,y-(abs(abs(x)-y)<<1)};
  //   }else{
  //     i64 t=abs(x)-(abs(abs(x)-y)<<1);
  //     return {x>0?-t:t,y};
  //   }
  // }
...
path_t node::path(){
  path_t p;
  node u=*this;
  while(abs(u.x)!=1||u.y!=1){
    auto rsp=u.jump();
    p.push_back({u,rsp.second});
    u=rsp.first;
    assert(p.size()<100);
  }
  p.push_back({u,0});
...
node lca(const path_t &a,const path_t &b){
  int i=1;
  while(i<=std::min(a.size(),b.size())&&a[a.size()-i].first==b[b.size()-i].first)i++;
...
int main(){
  ...
  for(int cas=1;cas<=T;cas++){
    ...
    if(pa.back().first==pb.back().first){
      node c=lca(pa,pb);
      print(length(pa)+length(pb)-(length(c.path())<<1),'\n');
    }else{
      print(length(pa)+length(pb)+1,'\n');
    }
  }
}