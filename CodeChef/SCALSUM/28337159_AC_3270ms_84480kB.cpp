...
const int N=6e5+10,sqn=500;
int n,m,pos,vis[N],fa[N],dep[N],siz[N],son[N],top[N],cnt[N],dfn_in[N],dfn_out[N],idfn[N];
long long a[N],val[N];
unsigned int cur,ans[N],sum[N];
vector<int> G[N];

void dfs(int u){
  dfn_in[u]=++pos,idfn[pos]=u;
  siz[u]=1;
  sum[u]=sum[fa[u]]+a[u]*a[u];
...
struct query{
  ...
  inline bool operator<(const query &rhs)const{
    if(l/sqn!=rhs.l/sqn)return l/sqn<rhs.l/sqn;
...
int main(){ 
  ...
  for(int i=1,u,v,t;i<=m;i++){
    ...
    q[i]=(query){dfn_out[u],dfn_in[v],i,t};
  }
  ...
  for(int l=1,r=1,i=1;i<=m;i++){
    while(r<q[i].r)upd(idfn[++r]);
    while(l>q[i].l)upd(idfn[--l]);
    while(r>q[i].r)upd(idfn[r--]);
    while(l<q[i].l)upd(idfn[l++]);
    ans[q[i].id]=cur+sum[q[i].lca];
    // log("> %d %lu\n",q[i].id,ans[q[i].id]);
  }
