...
const int N=245,L=1<<26,LA=15,LB=26,mod=998244353;
int n,q,nosol,a[N],dp[1<<LB];
u64 map[L];

inline int dec(int a,int b){((a-=b)<0)&&(a+=mod); return a;}
inline int inc(int a,int b){((a+=b)>=mod)&&(a-=mod); return a;}
...
void out(u64 a,u64 b,u64 c,u64 d){
  if(d)++nosol;
#ifndef memset0
  print(a,' ',b,' ',c,' ',d,'\n');
#endif
...
#endif
  for(u32 x=0;x<(1<<LA);x++){
    int sum=0;
    for(int i=0;i<LA;i++)if((x>>i)&1){
      sum=inc(sum,a[i]);
    }
    vec[x]=Data{sum,x};
  }
  fprintf(stderr,"clocks: %.4lf\n",clock()/(double)CLOCKS_PER_SEC);
	for(int i=0;i<LB;i++)dp[1<<i]=a[60+i];
	for(int x=1;x<(1<<LB);x++){
		dp[x]=inc(dp[x^(x&-x)],dp[x&-x]);
		if(dp[x]<L){
      map[dp[x]]=x;
		}
	}
