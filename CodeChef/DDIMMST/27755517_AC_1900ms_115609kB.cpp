...
struct Atom{
  ...
  inline bool operator<(const Atom &rhs)const{
    return s==rhs.s?x<rhs.x:s<rhs.s;
  }
	inline bool operator==(const Atom &rhs)const{
...
struct Set{
  priority_queue<Atom> ins[1<<D],del[1<<D];
  inline void insert(int x,const Atom &val){
		ins[x].push(val);
  }
  inline void erase(int x,const Atom &val){
		del[x].push(val);
  }
  inline Atom max(int x){
		while(ins[x].size()&&del[x].size()&&ins[x].top()==del[x].top()){
			ins[x].pop();
			del[x].pop();
		}
		return ins[x].top();
...
struct Simple{
  ...
  inline void insert(int x,const Atom &val){
		// log<BLUE>("insert %d (%d %d) (%d %d)\n",x,val.s,val.x,sum[x].s,sum[x].x);
    if(sum[x]<val){
      sum[x]=val;
    }
		// log<BLUE>("insert %d (%d %d) (%d %d)\n",x,val.s,val.x,sum[x].s,sum[x].x);
...
int main(){
  ...
  for(int i=1;i<n;i++){
    ...
    for(int x=0;x<(1<<d);x++){
      Atom maxf=f.max(x);
      Atom maxg=g.max(x^((1<<d)-1));
      if(maxf.s+maxg.s>max){
        max=maxf.s+maxg.s;
        solf=maxf.x;
        solg=maxg.x;
      }
