...
using mem::container::vector;

const int N=3e5+10,L=31,mod=998244353;
using z=mem::Z<mod>;

int n,a[N],x[N],lst[L],nxt[N][L],sum[2][L][N],cnt[L][N];
z ans;
...
void calc(int l,int r){
  if(l==r){
    ...
    ans+=highvalue(x[l]^x[r]);
  }else if(r-l==2){
    ans+=std::min({
      highvalue(highvalue(x[l]^x[l+1])^x[l+2]),
      highvalue(highvalue(x[l]^x[l+2])^x[l+1]),
      highvalue(highvalue(x[l+1]^x[l+2])^x[l]),
    });
  }else if(r-l==3){
    int res=INT_MAX,p[]={0,1,2,3};
    do{
      res=std::min(res,highvalue(highvalue(highvalue(x[l+p[0]]^x[l+p[1]])^x[l+p[2]])^x[l+p[3]]));
    }while(std::next_permutation(p,p+4));
    ans+=std::min({
      res,
      highvalue(highvalue(x[l]^x[l+1])^highvalue(x[r-1]^x[r])),
      highvalue(highvalue(x[l]^x[r-1])^highvalue(x[l+1]^x[r])),
      highvalue(highvalue(x[l]^x[r])^highvalue(x[l+1]^x[r-1])),
    });
  }
...
int main(){
  ...
  // for(int i=1;i<=n;i++)log<DEEPBLUE>("%d%c",a[i]," \n"[i==n]);
  for(int i=0;i<L;i++)lst[i]=n+1;
  for(int i=n;i>=1;i--){
    for(int j=0;j<L;j++)nxt[i][j]=lst[j];
    if(a[i]>=0)lst[a[i]]=i;
  }
  for(int i=1;i<=n;i++){
    ...
    int u=i,x=a[i];
    // log<BLUE>("i=%d >> u=%d x=%d\n",i,u,x);
    // log<GREEN>("ans=%d\n",ans.x);
    while(u<=n){
      int v=x+1==L?n+1:*min_element(nxt[u]+1+x,nxt[u]+L);
      if(x>=0){
        // log<RED>("u=%d v=%d x=%d >> %d*(%d-%d)\n",u,v,x,1<<x,sum[cnt[x][u]&1][v-1],sum[cnt[x][u]&1][u-1]);
        ans+=z(1<<x)*(sum[cnt[x][u]&1][x][v-1]-sum[cnt[x][u]&1][x][u-1]);
      }else{
        // log<RED>("u=%d v=%d x=%d\n",u,v,x);
      }
      if(x+1==L)break;
      // for(int i=0;i<L;i++)log<YELLOW>("%d%c",nxt[u][i]," \n"[i+1==L]);
      x=min_element(nxt[u]+1+x,nxt[u]+L)-nxt[u];
      u=v;
    }
    // log<GREEN>("ans=%d\n",ans.x);
  }
