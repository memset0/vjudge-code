...
namespace mem{ //v2.9.6 => size: 16.10KiB
  #ifdef memset0
  #else
    #define MEM_FASTIO
  #endif
...
void update(int &u,int v,int ql,int qr,int l,int r){
  p[u=++cnt]=p[v];
  // log("update %d %d %d %d %d %d\n",u,v,ql,qr,l,r);
  if(ql==l&&qr==r)return p[u].val++,p[u].tag++,void();
  int mid=(l+r)>>1;
  if(qr<=mid)update(p[u].lc,p[v].lc,ql,qr,l,mid);
  else if(ql>mid)update(p[u].rc,p[v].rc,ql,qr,mid+1,r);
  else update(p[u].lc,p[v].lc,ql,mid,l,mid),update(p[u].rc,p[u].rc,mid+1,qr,mid+1,r);
  p[u].val=max(p[p[u].lc].val,p[p[u].rc].val)+p[u].tag;
}

int query(int u,int ql,int qr,int l,int r){
  // log("query %d %d %d %d %d\n",u,ql,qr,l,r);
  if(ql==l&&qr==r)return p[u].val;
  int mid=(l+r)>>1;
  if(qr<=mid)return query(p[u].lc,ql,qr,l,mid)+p[u].tag;
  if(ql>mid)return query(p[u].rc,ql,qr,mid+1,r)+p[u].tag;
  return max(query(p[u].lc,ql,mid,l,mid),query(p[u].rc,mid+1,qr,mid+1,r))+p[u].tag;
}
...
int main(){
  ...
  for(int i=1;i<=n;i++){
    ...
    update(rt[i],rt[i-1],stk.top()+1,i,1,n);
    stk.push(i);
  }

  int lastans=0;
  while(m--){
    int l=(read<int>()+lastans*S-1)%n+1;
    int r=(read<int>()+lastans*S-1)%n+1;
    if(l>r)swap(l,r);
    println(lastans=query(rt[r],l,r,1,n));
    // log<RED>("%d %d => \n",l,r,lastans);
  }
}