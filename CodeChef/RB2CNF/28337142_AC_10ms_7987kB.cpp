...
const int N=256+10;
int T,a[N],c[N],col[N<<1],mrk[N],n,m;
MaxFlow<int> flow;
...
int main(){
  ...
  while(T--){
    ...
    // for(int i=1;i<=(n<<1);i++)log<GREEN>("%d%c",col[i]," \n"[i==(n<<1)]);

    for(int i=1;i<=n;i++){
      if(col[i]==1)a[i]^=1,mrk[i]=1;
    }
    for(int i=1;i<=m;i++){
      reverse(e[i].u);
      reverse(e[i].v);
      if(e[i].u>n)swap(e[i].u,e[i].v);
    }
    flow.init(n);
    for(int i=1;i<=n;i++)if(a[i]){
      flow.add(flow.s,i,0),flow.add(i,flow.e,c[i]);
    }else{
      flow.add(flow.s,i,c[i]),flow.add(i,flow.e,0);
    }
    for(int i=1;i<=m;i++){
      flow.add(e[i].u,e[i].v-n,INT_MAX>>1);
    }
    println(flow.dinic());
  }
}