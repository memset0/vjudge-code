...
void solve(){
  if(n==1){
    ...
    sum[n]=sum[n-1]+a[n]-(a[n]&1?0:1);
    read(m);
    for(long long r,x,y;m--;){
      read(r);
      x=(r-1)/n,y=r-x*n;
      log<RED>("x=%lld y=%lld ans=%d\n",x,y,ans.x);
      if(y!=n-1&&a[y+1]==1&&(a[y]&1)){
        ans=sum[n]*z(x)+sum[y];
      }else if(y!=n-1&&a[y+1]==1&&!(a[y]&1)){
        ans=sum[n]*z(x)+sum[y]+1;
      }else if(y!=n&&y!=1&&a[y]==1&&!(a[y-1]&1)){
        ans=sum[n]*z(x)+sum[y]+2;
      }else if(y!=n&&y!=1&&a[y]==1&&(a[y-1]&1)){
        ans=sum[n]*z(x)+sum[y];
      }else if(y!=n&&(a[y]&1)){
        ans=sum[n]*z(x)+sum[y]+1;
