...
#define pb push_back
poly A,B;
int n,R[6000001],lim,ksm[30],L,w[6000001],t1[6000001],t2[6000001],m,a[6000001],fac[6000001];
poly val[1000001];
...
int Quadratic_Residue(int x){
    for(;;){
        ...
        return std::min(tem2.a,P-tem2.a);
    }
}
...
int c[2000001];
int sum,_mul[2000001],invmul[2000001],T;
int C(int n,int m){
    if(n<m||m<0)return 0;
    return mul(mul(_mul[n],invmul[m]),invmul[n-m]);
}
int main(){
    for(int i=1,wn;i<((1000000+1)<<2);i<<=1){
        wn=qsm(3,(P-1)/(i<<1));
        w[i]=1;
    ...
    for(int fd=1;fd<=T;fd++){
        ...
        poly f(c),g(n-c+1);
        for(int i=0;i<f.size();i++)f[i]=mul(C(c-1,i),qsm(sub(1,p),i));
        for(int i=0;i<g.size();i++)g[i]=C(n-c,i);
        f=mul(f,g);
        for(int i=0;i<f.size();i++)f[i]=mul(f[i],mul(p,qsm(sub(1,qsm(sub(1,p),i+1)),P-2)));
        g.clear();
        g.resize(n);
//        for(int i=0;i<f.size();i++)printf("%d ",f[i]);putchar('\n');
        for(int i=0;i<f.size();i++)f[i]=mul(f[i],_mul[i]);
        for(int i=0;i<g.size();i++)g[i]=(i&1)?P-invmul[i]:invmul[i];
        std::reverse(f.begin(),f.end());
        f=mul(f,g);
        f.resize(n);
        std::reverse(f.begin(),f.end());
        for(int i=0;i<f.size();i++)f[i]=mul(f[i],invmul[i]);
        std::reverse(f.begin(),f.end());
        for(int i=0;i<f.size();i++)write(f[i]),putchar('\n');
    }
    return 0;
}