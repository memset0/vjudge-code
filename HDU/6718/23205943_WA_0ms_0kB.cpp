...
z solve(){
	...
	for(int h=59;h>=0;h--){
		f.clear(),g.clear();
		for(int i=0;i<vet.size();i++)((vet[i]>>h)&1?f:g).push_back((vet[i]&((1ll<<h)-1))%mod);
		sum=f,mul=1,dp[0]=1,dp[1]=0;
		...
		for(int i=0;i<f.size();i++){
			...
			fr[0]=dp[0],fr[1]=dp[1];
			dp[0]=(1ll<<h)%mod*fr[0]+(f[i]+1)*dp[1];
			dp[1]=(1ll<<h)%mod*fr[1]+(f[i]+1)*dp[0];
		}
...
int main(){
	...
	for(auto it:E){
		...
		if(find(u)==find(v)){
			...
			fa[find(u)]=find(v); 
			G[u].push_back(std::make_pair(v,c[v]));
			G[v].push_back(std::make_pair(u,c[v]));
		}
	}
	// for(int u=1;u<=n;u++)for(auto it:G[u])if(~it.second){
	// 	ans=ans*(it.second%mod+1);
	// }
	// printf("%d\n",ans.x);
	printf("%d\n",((1ll<<60)%mod*ans).x);
}
