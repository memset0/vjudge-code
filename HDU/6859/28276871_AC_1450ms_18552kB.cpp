...
int dx[4], dy[4], p[1001][5], tot, lax[201][35][52][2], lay[201][35][52][2],
    laz[201][35][52][2], la1[201][35][52][2], la2[201][35][52][2], po[100010],
    X, Y;
// 0:up 1:down 2:left 3:right
void print(int x, int y) {
  if (ud)
...
struct Solver {
  ...
  void DFSS(int nw, int nwi) {
    while (nw < r - l + 1 && BOO[nw])
      nw++;
    if (nw < r - l + 1) {
      int ll = nw, rr = nw;
      while (rr < r - l && !BOO[rr + 1])
        rr++;
      if (ll == rr)
        return;
      if (rr == ll + 1)
  ...
  void DFS(int nw) {
    if (nw == ttn + 1) {
      ...
      for (int o = 1; o <= ttn; o++) {
        for (int oo = tt[o]; oo < pt[o]; oo++)
          BOO[oo] = 1, boc[oo] = 1;
        for (int oo = pt[o]; oo < tt[o]; oo++)
          BOO[oo] = 1, boc[oo] = 1;
        BOO[tt[o]] = BOO[pt[o]] = 1;
      }
      int nwi = 0;
  ...
  void trans(int ii, int jj, int kk, int II, int JJ, int KK, int ll, int rr,
             int idd) {
  ...
  if (r - l > 4 && sy != 1 && sy != m) {
    if (sy & 1) {
      ...
      for (int i = 1; i < sy; i += 2)
        a[sx - 1][i][2] = a[sx][i][0] = a[sx][i + 1][2] = a[sx - 1][i + 1][1] =
            1;
      ...
      for (int i = sy + 1; i < m; i += 2)
        a[sx - 1][i][1] = a[sx][i][3] = a[sx][i + 1][0] = a[sx - 1][i + 1][3] =
            1;
