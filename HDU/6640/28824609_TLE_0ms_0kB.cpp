...
const int N=1e5+9,K=309;
int T,n,m,lim,ans[K],tem[N];
double R;

inline long long get_hash(int x,int y,int z){
	return x*1000000000000+y*1000000+z;
}
struct position{
	int x,y,z;
	long long val;
	inline void init(int _x,int _y,int _z){
		x=_x,y=_y,z=_z;
		val=get_hash(x,y,z);
	}
}loc[N];
map<long long,vector<int>> hsh;
...
inline bool is_intersected(int i,int j){return i!=j&&sqr(a[i].x-a[j].x)+sqr(a[i].y-a[j].y)+sqr(a[i].z-a[j].z)<=(a[i].r+a[j].r)*(a[i].r+a[j].r);}

inline bool insert(int x){
	loc[x].init(a[x].x/R,a[x].z/R,a[x].z/R);
//	log("insert[%d] => %d %d %d %lld\n",x,loc[x].x,loc[x].y,loc[x].z,loc[x].val);
	hsh[loc[x].val].push_back(x);
	if(hsh[loc[x].val].size()>=lim){
		return false;
	}
	return true;
}
...
//	log("===== check[%d,%d] =====\n",mid,k);
	for(int i=1;i<=n;i++)a[i].r=tem[i]+mid/2.;
	sort(a+1,a+n+1);
	R=0;
	int cnt=0;
//	for(int i=1;i<=n;i++)
//		for(int j=i+1;j<=n;j++){
//			log("%d %d => %.6lf\n",i,j,sqrt(sqr(a[i].x-a[j].x)+sqr(a[i].y-a[j].y)+sqr(a[i].z-a[j].z))-a[i].r-a[j].r);
//		}
	for(int i=n;i>=1;i--){
		if(!R||a[i].r<sqrt(3)/2*R){
			R=a[i].r*2;
//			log("rebuild %.4lf\n",R);
			hsh.clear();
			for(int j=n;j>i;j--){
				if(!insert(j))return 114514;
			}
		}
		if(!insert(i))return 114514;
		for(int x=loc[i].x-1;x<=loc[i].x+1;x++)
			for(int y=loc[i].y-1;y<=loc[i].y+1;y++)
				for(int z=loc[i].z-1;z<=loc[i].z+1;z++){
					long long u=get_hash(x,y,z);
					if(hsh.find(u)==hsh.end()){
						continue;
					}
					for(int j:hsh[u]){
						cnt+=is_intersected(i,j);
//						if(is_intersected(i,j))log("    intersect %d %d\n",i,j);
						if(cnt>k)return 114514;
					}
				}
	}
	return cnt;
...
void solve(int k){
	if(!k)return;
	for(lim=0;lim*(lim-1)/2<=k;lim++);
	int l=0,r=k==m?3000000:ans[k+1],mid,res=-1,lst=0;
	while(l<=r){
		mid=(l+r)>>1;
		int cur=check(mid,k);
		if(cur<k){
...
//	log(">> res=%d lst=%d\n",res,lst);
	for(int i=lst+1;i<=k;i++)ans[i]=res+1;
	solve(lst);
}
