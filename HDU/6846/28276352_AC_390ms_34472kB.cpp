...
int n, R[2000001], lim, ksm[30], L, w[2000001], t1[2000001], t2[2000001], m,
    a[1000001], fac[1000001];
int _mul[1000010], invmul[1000010];
poly val[1000001];
...
inline void print(poly A, int n = -1) {
  if (!~n)
    n = A.size();
  for (int i = 0; i < n; i++)
    write(A[i]), putchar(' ');
...
inline int inv(const int &a) {
  return (a <= 1) ? 1 : mul(P - P / a, inv(P % a));
}
...
inline int mul_inv(const int &a, const int &b) {
  return sub(add(b, b), mul(a, mul(b, b)));
}
inline poly mul_inv(poly A, const poly &B, int len = -1) {
  ...
  NTT(t2);
  for (int i = 0; i < lim; i++)
    t1[i] = sub(add(t2[i], t2[i]), mul(t1[i], mul(t2[i], t2[i])));
  std::reverse(t1 + 1, t1 + lim);
  if (~len)
    size = len;
  NTT_with_limit(t1, min(size, lim >> 1));
...
inline poly ln(const poly &A, const int &len) {
  return inte(mul(diff(A), inv(A, len), len));
}
inline poly exp(const poly &A, const int &n) {
  poly B(1, 1);
  for (int i = 2; (i >> 1) < n; i <<= 1)
    B = mul(B, 1 - ln(B, i) + A, i);
  B.resize(n);
  return B;
}
struct point {
...
point mul(point lhs, point rhs, int c) {
  return (point){add(mul(lhs.a, rhs.a), mul(mul(lhs.b, rhs.b), c)),
                 add(mul(lhs.a, rhs.b), mul(lhs.b, rhs.a))};
}
int Quadratic_Residue(int x) {
  for (;;) {
    int t = mul(rand(), rand());
    if (qsm(sub(mul(t, t), x), (P - 1) >> 1) != P - 1)
      continue;
    int b = (P + 1) >> 1, c = sub(mul(t, t), x);
    point tem1 = (point){t, 1}, tem2 = (point){1, 0};
    while (b) {
      if (b & 1)
        tem2 = mul(tem1, tem2, c);
...
int main() {
  T = read();
  for (int i = 1, wn; i < (100001 << 2); i <<= 1) {
    wn = qsm(3, (P - 1) / (i << 1)), w[i] = 1;
    for (int j = 1; j < i; j++)
      w[i + j] = mul(w[i + j - 1], wn);
  }
  ...
  for (int i = 2; i <= 100000; i++)
    invmul[i] = mul(invmul[i - 1], invmul[i]);
  poly f(100001);
  f[0] = 1;
  for (int i = 1; i < f.size(); i++)
    f[i] = mul(f[i - 1], add(mul(i, i), 1));
  for (int i = 1; i < f.size(); i++)
    f[i] = mul(mul(f[i], 1), invmul[i]);
  f = sqrt(f, f.size());
  //    for(int i=0;i<10;i++)printf("%d ",mul(f[i],_mul[i]));putchar('\n');
  //    f[0]=1;
  //    f=inv(f,f.size());
  for (int i = 1; i <= T; i++) {
    int n;
