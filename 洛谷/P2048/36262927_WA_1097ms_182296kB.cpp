...
namespace seg {
	...
	void init() {
		modify(rt[0], 0, 0, min, max);
		for (int i = 1; i <= n; i++) modify(rt[i], rt[i - 1], s[i], min, max);
	}
}
inline int query(int i, int k)  { return seg::query(rt[r[i]], rt[l[i] - 1], k, min, max); }
...
void main() {
	...
	for (int i = 1; i <= m; i++) {
		...
		// printf(">> %d %d %d\n", u.val, u.i, u.k);
		if (r[u.i] - l[u.i] + 1 <= u.k + 1) q.push((info){s[u.i] - query(u.i, u.k + 1), u.i, u.k + 1});
	}
	print(ans, '\n');
}
