...
#define nosol print(-1,'\n'),exit(0)

int n,m,s[N],ans[N],cho[N];
std::set<int> set;
...
int main(){
	...
	for(int l,r,val,i=1;i<=m;i++){
		read(l,r,val);
		if(val>n+1)nosol;
		min[val]=min[val]&interval{l,r};
		max[val]=max[val]|interval{l,r};
		if(!val){
			s[l]++;
			s[r+1]--;
		}
	...
	sum[n+1]=interval{0,n};
	for(int i=n;i>=0;i--){
		sum[i]=sum[i+1]&min[i+1];
		if(!sum[i].check())nosol;
	}
	for(int i=sum[0].l;i<=sum[0].r;i++)
		if(s[i]||~cho[0]){
			set.insert(i);
		}else{
			ans[i]=0;
	...
	if(!~cho[0])nosol;
	// for(int i=0;i<=n;i++)printf("sum[%d %d] min[%d %d] max[%d %d]\n",sum[i].l,sum[i].r,min[i].l,min[i].r,max[i].l,max[i].r);
	// for(int i=0;i<=n;i++)printf("%d%c",ans[i]," \n"[i==n]);
	for(int i=1;i<=n;i++){
		for(int k=sum[i].l;k<sum[i-1].l;k++)set.insert(k);
		for(int k=sum[i].r;k>sum[i-1].r;k--)set.insert(k);
		// for(int x:set)printf("%d ",x);putchar('\n');
		if(!set.size())nosol;
		int key=-1;
		if(!max[i].check()||*set.begin()<max[i].l){
			key=*set.begin();
			set.erase(set.begin());
		}else if(!max[i].check()||*--set.end()>max[i].r){
			key=*--set.end();
			set.erase(--set.end());
		}
		if(!~key)nosol;
