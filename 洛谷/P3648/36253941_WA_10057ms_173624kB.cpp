...
int n, m, l[maxm], r[maxm], a[maxn], s[maxn], p[maxn][maxm];
ll q[maxm][maxn], f[maxn][maxm];
std::vector < int > ans;

inline ll sqr(ll x) { return x * x; }
inline int x(int i) { return -s[i]; }
inline int y(int i, int j) { return f[i][j] - sqr(s[i]); }
...
int main() {
	...
	for (int i = 1; i <= n; i++) {
		for (int k = 1; k <= std::min(i, m); k++) {
			while (l[k - 1] < r[k - 1] && slope(q[k - 1][l[k - 1]], q[k - 1][l[k - 1] + 1], k - 1) < s[i]) ++l[k - 1];
