...
inline poly inte(const poly &f) {
    poly g(f.size());
    for (int i = 0; i < f.size() - 1; i++) g[i] = mul(f[i + 1], i + 1);
    return g;
}

inline poly ln(const poly &f) {
    poly g = inv(f) * deri(g); g.resize(f.size());
    return inte(g);
}
...
inline poly exp(const poly &f) {
    poly g(0, 1);
    for (int len = 2; (len >> 1) < f.size(); len <<= 1) {
        poly h = ln(g), t(len); h.resize(len); h[0] = dec(h[0], 1);
        for (int i = 0; i < len; i++) h[i] = dec(f[i], h[i]);
        for (int i = 0; i < len; i++) t[i] = i < f.size() ? f[i] : 0;
        h = h * t, g.resize(len);
...
// void main() {
//     read(n), read_mod(k, mod - 1); poly f(n);
