// 我永远喜欢 jerome_wei！
// I love jerome_wei forever!

#include<bits/stdc++.h>
const int N=1e5+10,mod=998244353,wyp=mod;
int n,len;
struct z {
	int32_t x;
	z(int32_t x=0):x(x){}
...
namespace poly{
	...
	vec inte(vec a){for(int i=a.size()-1;i;i--)a[i]=a[i-1]*::inv[i];return a.front()=0,a;}
	vec deri(vec a){for(int i=0;i<a.size()-1;i++)a[i]=a[i+1]*(i+1);return a.back()=0,a;}
	vec ln(const vec &f){vec a=deri(f)*inv(f); a.resize(f.size()); return inte(a);}
	vec exp(const vec &f,int len=-1){
		if((len=~len?len:f.size())==1)return vec{1};
		vec a=exp(f,(len+1)>>1),b=ln(vec(&a[0],&a[len]));
...
int main(){
	...
	for(int i=1;i<=n;i++)h[i]=fpow(n+1,mod-2)*(lpart[i-1]+rpart[i]*(n-i+1));
	vec f,g,h;
