...
const int maxn = 1000010, inf = 2000000000000;
...
namespace seg1 {
	...
	void modify(int u, int k, int c, int v) {
		...
		if (p[u << 1].mx1 > p[u << 1 | 1].mx1) {
			...
			p[u].mx2 = std::max(std::max(p[u << 1].mx2, p[u << 1 | 1].mx2), p[u << 1].at == p[u << 1 | 1].at ? -inf : p[u << 1 | 1].mx1);
		} else {
			p[u].mx1 = p[u << 1 | 1].mx1, p[u].at = p[u << 1 | 1].at;
			p[u].mx2 = std::max(std::max(p[u << 1].mx2, p[u << 1 | 1].mx2), p[u << 1].at == p[u << 1 | 1].at ? -inf : p[u << 1].mx1);
		}
	}
	...
	for (int i = hed[u], v = to[i]; i; i = nxt[i], v = to[i])
		if (!vis[v]) {
			...
			for (iterator it = vet.begin(); it != vet.end(); it++) {
				...
				// printf("-> %d", seg2::query(seg2::root[col[v]], std::max(l - it->dis, 0), r - it->dis, 0, n) - c[col[v]]);
				other = std::max(other, seg1::query(1, std::max(l - it->dis, 0ll), r - it->dis, col[v]));
				// printf(" %d\n", seg1::query(1, std::max(l - it->dis, 0), r - it->dis, col[v]));
				// printf("%d %d %d %d => %d\n", it->node, it->col, it->dis, it->cst, other + it->cst);
				// if (other + it->cst > ans) printf("%d %d : %d %d %d\n", u, v, ans, other, it->cst);
				ans = std::max(ans, other + it->cst);
			}
			for (iterator it = vet.begin(); it != vet.end(); it++) {
				seg1::modify(1, it->dis, col[v], it->cst);
				seg2::modify(seg2::root[col[v]], it->dis, it->cst, 0, n);
				rub1.push_back(it->dis), rub2.push_back(col[v]);
			}
		}
	seg1::erase(1, 0);
	seg2::cnt = 0;
