...
bool check(ll x,ll y){
	if((fpow(x,phi,m)==1)&&!(fpow(y,phi,m)==1))return false;
	for(ll u:vet)
		for(ll v=1;v<=phi/u;v++){
			v*=u;
			if((fpow(x,phi/v,m)==1)&&!(fpow(y,phi/v,m)==1))return false;
		}
	return true;
}
ll calc(ll x){
	ll res=phi;
	for(auto it:vet)if(res%it==0&&fpow(x,phi/it,m)==1)res/=it;
	return res;
}
int main(){
#ifdef memset0
	...
	for(auto it:list)vet.push_back(it.first);//std::cout<<it.first<<std::endl;
	// printf("m(%lld) = p(%lld)^k(%d)\n",m,p,k);for(auto it:vet)print(it,' ');putchar('\n');
	for(int i=1;i<=q;i++)read(x),read(y),puts(calc(x)%calc(y)==0?"Yes":"No");
}