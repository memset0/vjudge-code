...
const int N=2e3+10,Q=1e6+10;
using namespace mem::io;

int n,m,q,a[N],p[N],bln[N],sum[N],now[N],ans[Q];
std::vector<std::pair<int,int>> qry[N];

struct Matrix{
	int a,b,c,d;
	inline bool includes(int x){
		return a<=x&&x<=c&&b<=p[x]&&p[x]<=d;
	}
}mat[N];
...
int main(){
	...
	read(m);
	for(int i=1;i<=m;i++){
		read(mat[i].a),read(mat[i].c),read(mat[i].b),read(mat[i].d);
	}
	read(q);
	...
	for(int i=1;i<=m;i++){
		...
		for(const auto &it:qry[i]){
			// printf("query[%d](%d %d) => %d\n",it.first,it.second,i,sum[it.second]);
			ans[it.first]=sum[it.second];
		}
		// for(int j=1;j<=i;j++)printf("%d%c",now[j]," \n"[j==i]);
		// for(int j=1;j<=n;j++)printf("%d%c",bln[j]," \n"[j==n]);
	}
	for(int i=1;i<=q;i++){
		print(ans[i],'\n');
	}
