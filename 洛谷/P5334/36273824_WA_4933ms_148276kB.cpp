...
char s[N]; bool del[N];
int n, siz, len, ans[N], pre_lcp[N];
int sa[N], rnk[N], tax[N], log[N], tmp[N], height[N], st[M][N];
...
void main() {
	...
	buildSA(s);
	for (int i = 1; i <= n; i++) pre_lcp[i] = query(1, i);
	...
	for (int L = 2; L <= len; L++) {
		// std::cerr << "===== L = " << L << " =====\n";
		lis = set, lis.push_back(L), lis.clear();
		for (int j = 0; j < lis.size(); j++) {
			bool flag = 1;
			for (int i = set.size() - 1; i >= 0; i--) {
				int lcp = query(set[i], lis[j]);
				if (lcp < L - lis[j] + 1) {
					if (s[set[i] + lcp] < s[lis[j] + lcp]) { flag = 0; break; }
					else { set.pop_back(); }
				}
			}
