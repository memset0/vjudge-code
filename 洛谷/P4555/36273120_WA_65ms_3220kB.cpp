...
void main() {
  ...
  for (int i = 1; i <= n; i++) mind(l[i + f[i] - 2], i - f[i] + 1);
  for (int i = 1; i <= n; i++) maxd(r[i - f[i] + 2], i + f[i] - 1);
  for (int i = 4; i <= n; i += 2) mind(l[i], l[i - 2] + 2);
  for (int i = n - 1; i >= 2; i -= 2) maxd(r[i], r[i + 2] - 2);
  // for (int i = 1; i <= n; i++)
  //   printf("%d %d : %d %d : %d\n", i, f[i], i & 1 ? (i - 1) >> 1 : i >> 1, (i + f[i] - 2) >> 1, (i - f[i] + 2) >> 1);
  // for (int i = 1; i <= n; i++) printf("%c", s[i]); puts("");
  // for (int i = 1; i <= n; i++) printf("%d", f[i]); puts("");
  // for (int i = 2; i <= n; i += 2) print(l[i], ' '); puts("");
  // for (int i = 2; i <= n; i += 2) print(r[i], ' '); puts("");
