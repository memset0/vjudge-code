...
inline bool comp[0](const node &x,const node &y){return x.x==y.x?x.y<y.y:x.x<y.x;}
inline bool comp[1](const node &x,const node &y){return x.y==y.y?x.x<y.x:x.y<y.y;}
struct matrix{
	node a,b;
	inline matrix(){}
...
ll query(int u,matrix x,int fl){
	...
	return (matrix(e[u].x).is_in(x)?e[u].val:0)+query(e[u].ch[0],x,fl^1)+query(e[u].ch[1],x,fl^1);
}
int main(){
#ifdef memset0
