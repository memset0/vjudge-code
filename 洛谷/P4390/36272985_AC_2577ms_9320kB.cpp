...
int build(int l,int r,int fl){
	...
	int u=stk[mid].id;
	// for(int i=l;i<=r;i++)printf("stk[%d] = %d %d %lld %d\n",i,stk[i].x.x,stk[i].x.y,stk[i].w,stk[i].id);
...
void dfs(int u){
	if(lc(u))dfs(lc(u));
	// printf("> %d %d %d : %d %d : %lld %lld : %d %d %d %d\n",
	// 	u,lc(u),rc(u),e[u].x.x,e[u].x.y,e[u].val,e[u].sum,e[u].mat.a.x,e[u].mat.a.y,e[u].mat.b.x,e[u].mat.b.y);
	if(rc(u))dfs(rc(u));
}
...
ll query(int u,matrix x,int fl){
	...
	// 	u,fl,e[u].sum,x.a.x,x.a.y,x.b.x,x.b.y,e[u].mat.is_in(x),e[u].mat.is_out(x),matrix(e[u].x).is_in(x));
