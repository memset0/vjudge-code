...
const int N=1e5+10,M=1e4+10,S1=300,S2=4;
...
int mempool[N<<6],*mempointer=mempool;
int* memselect(int n){
  int *res=mempointer;
...
#ifdef sanitize_combinable_vector
struct combinable_vector{
  int lim;
  int *l,*r;
  std::vector<int> vec;
  inline void assertAll(){
    assert(l<=r),assert(r<=(l+lim)),assert(vec.size()==(r-l));
    for(int i=0;i<vec.size();i++){
      if(vec[i]!=*(l+i)){
        for(int i=0;i<vec.size();i++)log("%d/%d ",vec[i],*(l+i));
        log("\n");
      }
      assert(vec[i]==*(l+i));
    }
  ...
  inline void output()const{
    log("[%d] ",lim);
    for(int *i=l;i<r;i++)log("%d ",*i);
    log("\n");
  }
  inline void outputAll()const{
    log("lim=%d siz=%lu/%lu [%d %d] => ",lim,r-l,vec.size(),l,r);
    for(int i=0;i<vec.size();i++)log("%d/%d",vec[i],*(l+i));
    log("\n");
  }
  ...
  inline void re_assign(){
    int *tl=l,*tr=r;
    lim<<=1,l=memselect(lim),r=l+(tr-tl);
    memcpy(l,tl,size()<<2);
    assertAll();
  }
