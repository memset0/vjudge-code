...
vec fpow(vec a,int b){
	vec s={1}; int len=a.size();
	for(;b;b>>=1,a=mul(a,a,len))if(b&1)s=mul(s,a,len);
	return s;
}
...
std::pair<vec,vec> solve(int l,int r){
	if(l==r){
		...
		return F.pop_back(),std::make_pair(F,G);
	}
	int mid=(l+r)>>1;
	std::pair<vec,vec> L=solve(l,mid);
	std::pair<vec,vec> R=solve(mid+1,r);
	return std::make_pair(L.first*R.second+L.second*R.first,L.second*R.second);
}
int main(){
#ifdef memset0
	...
	std::pair<vec,vec> res=solve(1,m);
	for(int i=1;i<=n;i++){
