...
void main() {
    int n, k, cnt = 0, fst, inv_fst; ll tmp = 1;
    read(n), read(k);
    if (k == 0) {
        putchar('1'), putchar(' ');
    ...
    poly f(n), g; f.in();
    fpow(f, k).out();
    for (int i = 0; i < n; i++) if (!f[i]) ++cnt; else break;
    if (cnt == n) {
        for (int i = 0; i < n; i++) putchar('0'), putchar(' ');
        return;
    }
    for (int i = cnt; i < n; i++) g.push_back(f[i]);
    fst = g[0], inv_fst = inv(fst);
    for (int i = 0; i < g.size(); i++) g[i] = mul(g[i], inv_fst);
    f = ln(g), g.clear();
    for (int i = 0; i < f.size(); i++) f[i] = mul(f[i], k);
    f = exp(f), fst = fpow(fst, k), tmp = std::min((ll)n, (ll)cnt * k);
    for (int i = 0; i < tmp; i++) g.push_back(0);
    for (int i = tmp; i < n; i++) g.push_back(mul(f[i - tmp], fst));
    g.out();
}
