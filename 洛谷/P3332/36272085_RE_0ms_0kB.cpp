...
int query(int u, int k, int l, int r) {
	...
	int left_size = e::query(p[u << 1].root, 1, t, l, r);
	// printf("left size = %d\n", left_size);
	if (k <= left_size) return query(u << 1, k, l, r);
	return query(u << 1 | 1, k - left_size, l, r);
}
...
int main() {
	...
	build(1, 1, n);
	for (int i = 1; i <= m; i++)
		if (q[i].opt == 1) q[i].k = lower_bound(val + 1, val + t + 1, q[i].k) - val;
	for (int i = 1; i <= m; i++) {
