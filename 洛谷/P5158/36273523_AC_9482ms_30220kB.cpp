...
void build(int u, int l, int r, const poly &f) {
    if (l == r) { F[u] = poly{dec(0, f[l]), 1}; return; }
    int mid = (l + r) >> 1;
    build(u << 1, l, mid, f);
    build(u << 1 | 1, mid + 1, r, f);
    F[u] = F[u << 1] * F[u << 1 | 1];
}

void MPE(int u, int l, int r, const poly &f, const poly &x, poly &a) {
    if (l == r) { a[l] = mul(x[l], inv(f[0])); return; }
    int mid = (l + r) >> 1;
    MPE(u << 1, l, mid, f % F[u << 1], x, a);
    MPE(u << 1 | 1, mid + 1, r, f % F[u << 1 | 1], x, a);
}
