...
const int N = 75, K = 1 << 13, mod = 998244353;
            ...
            for (int j = 0; j < len; j++) {
                if (s[k + 1] == '^') {
                    ...
                    if (!flag) {
                        ...
                        a[i + j] = half(inc(x, y));
                        a[i + j + len] = half(dec(x, y));
                    }
                } else if (s[k + 1] == '&') {
                    if (!flag) {
                        a[i + j] = inc(a[i + j], a[i + j + len]);
                    } else {
                        a[i + j] = dec(a[i + j], a[i + j + len]);
                    }
                } else if (s[k + 1] == '|') {
                    if (!flag) {
                        a[i + j + len] = inc(a[i + j + len], a[i + j]);
                    } else {
                        a[i + j + len] = dec(a[i + j + len], a[i + j]);
                    }
                }
            }
    }
