...
void fwt(int a[]) {
    for (int len = 1, k = 0; len < lim; len <<= 1, ++k)
        for (int i = 0; i < lim; i += (len << 1))
            for (int j = 0; j < len; j++)
                if (s[k + 1] == '^') { int x = a[i + j], y = a[i + j + len]; a[i + j] = inc(x, y); a[i + j + len] = dec(x, y); }
                else if (s[k + 1] == '&') a[i + j] = inc(a[i + j], a[i + j + len]);
                else if (s[k + 1] == '|') a[i + j + len] = inc(a[i + j + len], a[i + j]);
}
...
void ifwt(int a[]) {
    for (int len = 1, k = 0; len < lim; len <<= 1, ++k)
        for (int i = 0; i < lim; i += (len << 1))
            for (int j = 0; j < len; j++)
                if (s[k + 1] == '^') { int x = a[i + j], y = a[i + j + len]; a[i + j] = half(inc(x, y)); a[i + j + len] = half(dec(x, y)); }
                else if (s[k + 1] == '&') a[i + j] = dec(a[i + j], a[i + j + len]);
                else if (s[k + 1] == '|') a[i + j + len] = dec(a[i + j + len], a[i + j]);
}
