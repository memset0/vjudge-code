...
const int N=1e5+10;
int n,m,fa[N],ch[N][2],val[N],sum[N],rev[N];
...
inline void rotate(int x){
	...
	if(!isroot(f))ch[ff][ffs]=x;ch[x][fs]=y,ch[x][fs^1]=f,ch[f][fs]=y;
...
inline void splay(int x){clear(x);for(int f=fa[x];!isroot(x);rotate(x),f=fa[x])if(!isroot(f))rotate(getson(x)==getson(f)?f:x);}
inline void access(int x){for(int y=0;x;y=x,x=fa[x])splay(x),ch[x][1]=y,maintain(x);}
...
inline void link(int x,int y){if(getroot(x)!=getroot(y))mroot(x),fa[x]=y;}
inline void cut(int x,int y){if(getroot(x)==getroot(y)){split(x,y);if(fa[x]==y&&ch[x][0]==y&&!ch[x][1])fa[x]=ch[y][0]=0,maintain(y);}}
int main(){
	read(n),read(m);
