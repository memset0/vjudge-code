#include<bits/stdc++.h>
using namespace std;
const int N=4e6+1e5+7,p=998244353;
int n,q,k,t,lim,f[N],g[N],w[N],wn[N],rev[N];
inline int read(){
...
int main(){
	...
	for(int wn,len=1;len<lim;len<<=1){
	    wn=pow(3,(p-1)/(len<<1)),w[len]=1;
	    for(int i=1;i<len;i++)w[i+len]=(long long)w[i+len-1]*wn%p;
	}
