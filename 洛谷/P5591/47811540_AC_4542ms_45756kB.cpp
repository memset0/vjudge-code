...
int n,q,k,t,lim,f[N],g[N],wn[N],rev[N],wc[N],wq[N];
inline int read(){
...
inline void init(){
	for(int d=1;d<lim;d=d<<1){
		long long w=pows(3,(p-1)/(d<<1)),c=pows(332748118,(p-1)/(d<<1)),wn=1,ws=1;
		for(int j=0;j<d;j++,wn=wn*w%p,ws=ws*c%p) wc[j+d]=wn,wq[j+d]=ws;
	}
}
inline void ntt(int *s){
	...
	for(int d=1;d<lim;d=d<<1){
		for(int i=0,r=d<<1;i<lim;i+=r){
			for(int j=0;j<d;j++){
				int a=s[i+j],b=1ll*s[i+j+d]*wc[j+d]%p;
...
inline void intt(int *s){
	...
	for(int d=1;d<lim;d=d<<1){
		for(int i=0,r=d<<1;i<lim;i+=r){
			for(int j=0;j<d;j++){
				int a=s[i+j],b=1ll*s[i+j+d]*wq[j+d]%p;
				s[i+j]=inc(a,b),s[i+j+d]=inc(a,p-b);
			}
		}
	}
...
int main(){
	...
	lim=1,t=0; while(lim<(k<<1)) lim=lim<<1,t++; init();
