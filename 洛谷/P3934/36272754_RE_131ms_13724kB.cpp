...
void build(int l, int r, int u = 1) {
	...
	if (l == r) { p[u].sum = a[l], p[u].fst = a[l] == 1 ? l : n + 2; return; }
...
int query(int k, int u = 1) {
	...
	return query(k, k <= p[u].mid ? u << 1 : u << 1 | 1);
}
