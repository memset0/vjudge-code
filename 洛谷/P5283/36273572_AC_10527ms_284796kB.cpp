...
void main() {
    ...
    for (int i = 1; i <= k; i++) {
        auto it = q.top(); q.pop();
        ans += it.value, modify(rt[it.key - 1], rt[it.key - 1], s[it.key] ^ it.value, -1);
        // printf(">>> "), print(it.value, ' '), print(query(rt[i - 1], s[i]), '\n');
        q.push((status){query(rt[it.key - 1], s[it.key]), it.key});
    }
    print(ans, '\n');
}
