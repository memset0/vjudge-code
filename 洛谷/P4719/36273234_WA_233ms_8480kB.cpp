...
struct matrix {
	int a[2][2];
	inline int& operator [] (int k) { return a[0][k]; }
} f[N], g[N];
...
inline void modify(int k, int delta) {
	for (int u = k; u; u = fa[u]) {
		if (ch[fa[u]][0] != u && ch[fa[u]][1] != u && fa[u]) {
			g[fa[u]][0] -= std::max(f[u][0], f[u][1]), g[fa[u]][1] -= f[u][0], g[fa[u]].a[1][0] = g[fa[u]].a[0][0];
...
void main() {
	...
	dfs(1), dp(1);
	for (int i = 1; i <= n; i++) g[i].a[1][0] = g[i].a[0][0], g[i].a[1][1] = -1e9;
	rt = build(1);
	for (int i = 1, k, w; i <= m; i++) {
		read(k), read(w), modify(k, w - val[i]), val[i] = w;
