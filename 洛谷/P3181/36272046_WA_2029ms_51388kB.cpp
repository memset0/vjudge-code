...
void solve() {
	stk[top = 1] = (status){len + 1, tag[2] ^ 1};
	sum = (ll)(len + 1) * (tag[2] ^ 1);
	for (int i = 3; i <= len; i++)
		if (~tag[i]) {
			value = tag[i] ^ 1;
			while (top && stk[top].height >= height[i]) {
				sum -= (ll)stk[top].height * stk[top].value;
				// printf("sum -= %d %d\n", stk[top].height, stk[top].value);
				value += stk[top--].value;
			}
			if (value && height[i]) {
				stk[++top] = (status){height[i], value};
				// printf("sum += %d %d\n", stk[top].height, stk[top].value);
				sum += (ll)stk[top].height * stk[top].value;
			}
			if (tag[i]) ans += sum;
			// if (tag[i] && sum) printf(">>> ans += %d\n", sum);
			// printf("[%d %d] ", value, height[i]);
			// for (int i = 1; i <= top; i++)
			// 	printf("(%d %d) ", stk[i].height, stk[i].value);
			// putchar('\n');
		}
}
