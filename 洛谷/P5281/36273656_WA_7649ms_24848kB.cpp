...
inline void solve(int u) {
	...
	for (int i = hed[u], v; v = to[i], i; i = nxt[i])
		if (v != fa[u]) {
			solve(v);
			if (dep[u] & 1) {
				f[u] = mul(f[u], dec(1, f[v]));
				g[u] = mul(g[u], g[v]);
			} else {
				f[u] = mul(f[u], f[v]);
				g[u] = mul(g[u], dec(1, g[v]));
			}
		}
	(dep[u] & 1) ? f[u] = dec(1, f[u]) : g[u] = dec(1, g[u]);
}
...
void main() {
	...
	dep[1] = 1, w = pre_dp(1);
	// printf("w = %d\n", w);
	for (int d, u = 1; u <= n; u++) if (is_leaf[u]) {
		if (u > w) {
			d = u - w + 1;
			f[u] = 1, g[u] = 0;
			// printf("%d %d > %d %d %d\n", u, d, u - (d - 1) < w, u - d < w, u - (d + 1) < w);
			if (std::max(L - 1, 1) <= d && d <= std::min(R, n - 1))
				todo1[d].push_back(u);
		} else if (u < w) {
			d = w - u + 1;
			g[u] = 1, f[u] = 0;
	...
	for (int i = std::max(L - 1, 1); i <= std::min(R, n - 1); i++) {
		// printf("=== %d ===\n", i);
		if (todo1[i].size() || todo2[i].size()) {
			for (auto u : todo1[i]) g[u] = half;
			for (auto u : todo2[i]) f[u] = half;
			c = i, solve(1);
		}
		sum[i] = mul(dec(1, mul(dec(1, f[1]), dec(1, g[1]))), fpow(2, m - 1));
