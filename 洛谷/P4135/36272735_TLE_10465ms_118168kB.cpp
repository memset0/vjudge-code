...
int cnt[maxm][maxn], sum[maxm][maxm][2];
bool tag[maxn];
...
int main() {
			...
			for (int k = 1; k <= top; k++)
				if (!tag[stk[k]]) {
					if (cnt[i - 1][stk[k]] - cnt[j - 1][stk[k]])
						sum[j][i][(cnt[i - 1][stk[k]] - cnt[j - 1][stk[k]]) & 1]--;
					sum[j][i][(tmp[stk[k]] + cnt[i - 1][stk[k]] - cnt[j - 1][stk[k]]) & 1]++;
					tag[stk[k]] = 1;
				}
			clear_tag();
		}
	...
	for (int i = 1; i <= m; i++) {
		...
		if (bln[l] == bln[r]) {
			...
			ans[0] = sum[bln[l] + 1][bln[r] - 1][0];
			ans[1] = sum[bln[l] + 1][bln[r] - 1][1];
			...
			for (int i = 1; i <= top; i++)
				if (tmp[stk[i]]) {
					...
					if (cnt[bln[r] - 1][stk[i]] - cnt[bln[l]][stk[i]])
						ans[(cnt[bln[r] - 1][stk[i]] - cnt[bln[l]][stk[i]]) & 1]--;
					ans[(tmp[stk[i]] + cnt[bln[r] - 1][stk[i]] - cnt[bln[l]][stk[i]]) & 1]++;
					tmp[stk[i]] = 0;
				}
		}
		// printf(">>> %d %d\n", ans[0], ans[1]);
