...
inline poly operator + (const poly &f, const poly &g) {
	...
	rep(i, 0, h.size()) h[i] = sub(i < f.size() ? f[i] : 0, i < g.size() ? g[i] : 0);
	return h;
}

inline poly operator - (const poly &f, const poly &g) {
	poly h(std::max(f.size(), g.size()));
	rep(i, 0, h.size()) h[i] = dec(i < f.size() ? f[i] : 0, i < g.size() ? g[i] : 0);
	return h;
}
...
inline poly operator * (const poly &f, const poly &g) {
	...
	// memset(a, 0, lim * sizeof(a[0])), memset(b, 0, lim * sizeof(b[0]));
	...
	// print(f), print(g), print(h), puts("");
	return h;
}

inline poly resize(const poly &f, int n) {
	poly g = f;
	while (n > g.size()) g.push_back(0);
	while (n < g.size()) g.pop_back();
	return g;
}
...
inline poly inverse(const poly &f) {
	...
	for (int len = 2; (len >> 1) < f.size(); len <<= 1) {
		poly t;
		rep(i, 0, len) t.push_back(i < f.size() ? f[i] : 0);
		t = resize(g * t, len);
...
inline poly model(const poly &f, const poly &g) {
	...
	poly r = resize(f - q * g, g.size() - 1);
	// print(f), print(g), print(q), print(r), tputs("");
	return r;
}
...
poly divide(int l, int r) {
	poly F;
	if (l == r) F.push_back(dec(0, b[l])), F.push_back(1);
	else {
		int mid = (l + r) >> 1;
		F = divide(l, mid) * divide(mid + 1, r);
	}
	map[std::make_pair(l, r)] = F;
	// printf("[%d %d] ", l, r), print(F);
	return F;
}
...
void solve(int l, int r, poly U) {
	// printf("solve %d %d\n", l, r);
	// printf(">>> "), print(U);
	// printf(">>> "), print(map[std::make_pair(l, r)]);
