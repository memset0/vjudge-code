...
void main() {
	...
	for (int i = 0; i < n - 1; i++) a[i] = complex(q[i + 1], 0);
	for (int i = 0; i < n; i++) b[i] = complex((double)(1.0 / (i + 1) / (i + 1)), 0);
	// for (int i = 1; i <= n; i++) printf("%.3lf%c", a[i - 1].x, i == n ? '\n' : ' ');
	fft(a, 1), fft(b, 1);
	for (int i = 0; i < m; i++) a[i] = a[i] * b[i];
	fft(a, -1);
	// for (int i = 1; i <= n; i++) printf("%.3lf%c", a[i - 1].x / m, i == n ? '\n' : ' ');
	for (int i = 2; i <= n; i++) ans[i] += a[i - 2].x / m;
	// for (int i = 1; i <= n; i++) printf("%.3Lf\n", ans[i]);
	for (int i = 0; i < n - 1; i++) a[i] = complex(q[n - i], 0); a[n - 1] = complex(0, 0);
	// for (int i = 1; i <= n; i++) printf("%.3lf%c", a[i - 1].x, i == n ? '\n' : ' ');
	fft(a, 1);
	for (int i = 0; i < m; i++) a[i] = a[i] * b[i];
	fft(a, -1);
