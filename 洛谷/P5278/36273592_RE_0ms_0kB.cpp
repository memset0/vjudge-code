...
const int N = 6e5 + 10;
int n, m, tn, a[N], b[N << 1], c[N], pre[N];
std::set <int> set[N << 1];
...
//	std::set <int> ::iterator it = set[c[x]].find(x), tp;
//	tp = it; int Pre = tp == set[c[x]].begin() ? -1 : *--tp;
//	tp = it, tp = ++tp; int Nxt = tp == set[c[x]].end() ? -1 : *++tp;
//	if (~Nxt) pre[Nxt] = Pre, modify_pre(1, Nxt, Pre);
//	set[c[x]].erase(x), set[z].insert(x);
//	tp = it, Pre = tp == set[z].begin() ? -1 : *--tp;
//	tp = it, tp = ++tp, Nxt = tp == set[z].end() ? -1 : *++tp;
...
void main() {
	...
	for (int i = 1; i <= n; i++) c[i] = std::lower_bound(b + 1, b + tn + 1, a[i]) - b, set[c[i]].insert(i);
	for (int i = 1; i <= m; i++) if (q[i].opt == 1) q[i].z = std::lower_bound(b + 1, b + tn + 1, q[i].y) - b;
	for (int i = 1; i <= n; i++) {
		std::set <int> ::iterator it = set[c[i]].find(i);
		pre[i] = it == set[c[i]].begin() ? -1 : *--it;
	} build(1, 1, n);
	for (int i = 1; i <= m; i++)
		if (q[i].opt == 1) maintain(q[i].x, q[i].y, q[i].z);
