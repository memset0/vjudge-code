...
//	printf("maintain %d %d\n", x, y);
	...
	std::set <int> &Set = set[a[x]];
	it = Set.find(x);
	tp = it, Pre = tp == Set.begin() ? -1 : *--tp;
	tp = it, tp = ++tp, Nxt = tp == Set.end() ? -1 : *tp;
	if (~Nxt) pre[Nxt] = Pre, modify_pre(1, Nxt, Pre);
	Set.erase(x);
	std::set <int> &Set2 = set[y];
	Set2.insert(x), it = Set2.find(x);
	tp = it, Pre = tp == Set2.begin() ? -1 : *--tp;
	tp = it, tp = ++tp, Nxt = tp == Set2.end() ? -1 : *tp;
