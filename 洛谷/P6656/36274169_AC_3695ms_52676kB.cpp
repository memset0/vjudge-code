...
const int S=1<<15; char ibuf[S],*iS,*iT,obuf[S],*oS=obuf,*oT=oS+S-1;
...
const int N=1e6+10,p0=998244353,p1=1<<31,px=131131;
...
inline bool equal(int l0,int r0,int l1,int r1){
	if(((int64)h0[l0]*pw0[r0-l0+1]-h0[r0+1]-(int64)h0[l1]*pw0[r1-l1+1]+h0[r1+1])%p0)return false;
	if(((int64)h1[l0]*pw1[r0-l0+1]-h1[r0+1]-(int64)h1[l1]*pw1[r1-l1+1]+h1[r1+1])%p1)return false;
	return true;
}
...
int main(){
	...
	write(runs.size()),putchar('\n');
	for(int i=0;i<runs.size();i++){
		write(get<0>(runs[i])+1),putchar(' ');
		write(get<1>(runs[i])+1),putchar(' ');
