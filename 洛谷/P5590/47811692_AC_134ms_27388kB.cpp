...
// #endif
	...
	if(!fl[1]||!fl[n]||!tarjan(1)||!spfa()){
		// std::cerr<<"[std] -1"<<std::endl;
		puts("-1");
	}else{
		std::cerr<<"[std] "<<n<<" "<<m<<std::endl;
		print(n,' '),print(m,'\n');
		for(auto it:E)if(fl[it.first]&&fl[it.second]){
			print(n-it.second+1,' '),print(n-it.first+1,' '),print(dis[it.second]-dis[it.first],'\n');
		}else{
			print(n-it.second+1,' '),print(n-it.first+1,' '),print(1,'\n');
		}
	}
}