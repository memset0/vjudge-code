...
inline void update(int u) {
	...
	lmax[u] = max2(lmax[ch[u][0]], sum[ch[u][0]] + val[u] + lmax[ch[u][1]]);
	rmax[u] = max2(rmax[ch[u][1]], sum[ch[u][1]] + val[u] + rmax[ch[u][0]]);
	smax[u] = max3(smax[ch[u][0]], smax[ch[u][1]], rmax[ch[u][0]] + val[u] + lmax[ch[u][1]]);
}

inline void pushup_tag(int u, int v) {
	tag[u] = val[u] = v, rev[u] = 0;
	sum[u] = v * siz[u];
	lmax[u] = rmax[u] = smax[u] = sum[u];
}
