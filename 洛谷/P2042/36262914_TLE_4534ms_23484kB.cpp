...
int n, m, x, y, z, k, l, r, v, rt, cnt, opt, top;
int a[maxn], q[maxn], stk[maxn];
...
void bfs_ryc(int u) {
	...
	while (l <= r) {
		stk[++top] = u = q[l++];
		if (ch[u][0]) q[++r] = ch[u][0];
		if (ch[u][1]) q[++r] = ch[u][1];
	}
}
