...
int n, m, lim, val[N], son[N], fa[N], siz[N];
...
} tf, tg, ans, one, f[N], g[N], lg[N], base[M];
...
namespace bst {
	...
	int sbuild(int l, int r) {
		int sum = 0, now = 0;
		for (int i = l; i <= r; i++) sum += siz[stk[i]];
		for (int i = l; i <= r; i++) {
			now += siz[stk[i]];
			if ((now << 1) >= sum) {
...
void dp(int u) {
	// printf("DP %d\n", u);
	f[u] = base[val[u]];
	// print(" f[" + std::to_string(u) + ":" + std::to_string(val[u]) + "]", f[u], 0, lim - 1);
...
void main() {
	...
	dfs(1), dp(1);
	// for (int i = 0; i < lim; i++)
	// 	print("base[" + std::to_string(i) + "]", base[i], 0, lim - 1);
	// print("son[]", son, 1, n);
	// for (int i = 1; i <= n; i++) {
	// 	print(" f[" + std::to_string(i) + "]", f[i], 0, lim - 1);
	// 	print(" g[" + std::to_string(i) + "]", g[i], 0, lim - 1);
	// 	print("lf[" + std::to_string(i) + "]", lf[i], 0, lim - 1);
	// 	print("lg[" + std::to_string(i) + "]", lg[i], 0, lim - 1);
	// 	f[i].ifwt(), g[i].ifwt();
	// 	print("wf[" + std::to_string(i) + "]", f[i], 0, lim - 1);
	// 	print("wg[" + std::to_string(i) + "]", g[i], 0, lim - 1);
	// 	f[i].fwt(), g[i].fwt();
	// }
