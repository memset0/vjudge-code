...
//	printf(">> %d %d\n", l, r);
	...
	if (bln[l] == bln[r]) {
		...
		for (int i = l; i < fst[bln[l] + 1]; i++)
			if (better(++tmp[a[i]] + sum[a[i]][r - 1] - sum[a[i]][l], tmp[ans] + sum[ans][r - 1] - sum[ans][l], a[i], ans))
				ans = a[i];
...
//		printf("=] %d\n", ans);
		for (int i = bln[l] + 1; i <= bln[r] - 1; i++)
			if (better(sum[max[i]][r - 1] + sum[max[i]][l], tmp[ans] + sum[ans][r - 1] - sum[ans][l], max[i], ans))
				ans = max[i];
//		printf("=] %d\n", ans);
		for (int i = l; i < fst[bln[l] + 1]; i++)
			--tmp[a[i]];
