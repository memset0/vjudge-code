...
void main() {
	...
	for (int u = 1; u <= n; u++) {
		...
		for (std::map <ll, std::vector <int> > ::iterator it = map.begin(); it != map.end(); it++) {
	...
	for (int u = 1; u <= n; u++) {
		...
		for (int v = 1; v <= n; v++) nod[v] = std::make_pair(vector(p[v], p[u]), dis[u][v]);

		memset(cnt, 0, sizeof(cnt));
		for (int i = 0; i < qry[u].size(); i++) cnt[qry[u][i]]++;
		m = 1, lie[0] = std::make_pair(vector(1, 0), 0);
		...
		std::swap(nod[u], nod[n]), std::sort(nod, nod + n - 1);
