...
struct pair {
	int first, second;
	inline pair() {}
	inline pair(int _first, int _second) { first = _first, second = _second; }
};

struct pair_hash {
	inline std::size_t operator () (const std::pair <int, int> &it) const {
		return ((unsigned int)(std::hash <int> ()(it.first))) << 5 ^
			((unsigned int)(std::hash <int> ()(it.second))) >> 3;
	}
};
...
inline bool dmin(int a, int b, int c) { return dis[a][b] > dis[a][c] - dis[b][c]; }
inline bool dmax(int a, int b, int c) { return dis[a][b] < dis[a][c] - dis[b][c]; }
...
void main() {
    ...
    for (int u = 1; u <= n; u++) {
        ...
        for (std::unordered_map <int, std::vector <int> > ::iterator it = map.begin(); it != map.end(); it++) {
    ...
    // for (int u = 34; u == 34; u++) {
    // 	for (int v = 1; v <= n; v++)
    // 		printf(">> %d %d\n", -vector(p[v], p[u]).y, vector(p[v], p[u]).x);
    // }
    // exit(0);
    for (int u = 1; u <= n; u++) {
        // printf("===== u = %d (%d %d) =====\n", u, p[u].x, p[u].y);
        ...
        for (int i = 0; i < qry[u].size(); i++) map[std::make_pair(-qry[u][i].y, qry[u][i].x)]++;
        m = 1, lie[0] = std::make_pair(vector(1, 0), map[std::make_pair(1, 0)]);
        for (std::unordered_map <std::pair <int, int> , int, pair_hash> ::iterator it = map.begin(); it != map.end(); it++)
        ...
        for (int i = 1; i < m; i++) {
            // printf("=> i = %d : [%d %d]\n", i, lie[i].x, lie[i].y);
            while (hed_i < (n - 1) && lie[i].first.rev() * q_i[hed_i].first < 0 && lie[i - 1].first.rev() * q_i[hed_i].first >= 0) insert(q_i[hed_i++]);
            // printf(">> %d %d\n", lie[i] * q_d[hed_d].first, lie[i - 1] * q_d[hed_d].first);
            // printf(">> %d %d\n", lie[i] * q_d[hed_d + 1].first, lie[i - 1] * q_d[hed_d + 1].first);
            while (hed_d < (n - 1) && lie[i].first * q_d[hed_d].first <= 0 && lie[i - 1].first * q_d[hed_d].first > 0) remove(q_d[hed_d++]);
            ans += (ll)now * lie[i].second;
            tmp += (ll)now * lie[i].second;
            // if (now && lie[i].second) printf(">> %lld ans += %lld * %d (%d %d) %d %d | %d %d (%d %d)\n", ans >> 2, now >> 1, lie[i].second >> 1, hed_i, hed_d, u, i,
                // lie[i].first.x, lie[i].first.y, lie[i].first.x / std::__gcd(lie[i].first.x, lie[i].first.y), lie[i].first.y / std::__gcd(lie[i].first.x, lie[i].first.y));
        }
        // printf("%d -> %d\n", u, ans >> 2);
        // printf("end u = %d\n", u);
    }
