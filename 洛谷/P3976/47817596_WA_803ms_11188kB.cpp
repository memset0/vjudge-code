...
struct status {
	...
	il void operator += (const status &other) {
		// printf("> %d %d %d %d += %d %d %d %d\n", ans, rev, max, min, other.ans, other.rev, other.max, other.min);
		ans = std::max(ans, std::max(other.ans, other.max - min));
		rev = std::max(rev, std::max(other.rev, max - other.min));
		max = std::max(max, other.max), min = std::min(min, other.min);
	}
} ans;
...
void main() {
	...
	for (int i = 1; i <= m; i++) {
		...
		// printf("=== %d %d %d : %d ===\n", u, v, w, t);
		ans = (status){-inf, -inf, -inf, inf};
		while (top[u] != top[t]) ans += query(1, id[top[u]], id[u]).reverse(), u = fa[top[u]];
		ans += query(1, id[t], id[u]).reverse();
		while (top[v] != top[t]) ans += query(1, id[top[v]], id[v]), v = fa[top[v]];
		if (id[t] != id[v]) ans += query(1, id[t] + 1, id[v]);
		u = tu, v = tv;
