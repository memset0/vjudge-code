...
namespace polynomial{
  ...
  void dft(int *f,int lim){
    ...
    for(int len=1;len<lim;len<<=1){
      for(int i=0;i<lim;i+=(len<<1))
        for(int j=0;j<len;j++){
          unsigned long long x=a[i+j],y=a[i+j+len]*w[j+len]%mod;
          a[i+j]=x+y,a[i+j+len]=x+mod-y;
        }
      if(len==131072)for(int i=0;i<lim;i++)a[i]%=mod;
    }
    for(int i=0;i<lim;i++)f[i]=(int)(a[i]%mod);
  }
  void idft(int *f,int lim){
    dft(f,lim);
    std::reverse(f+1,f+lim);
    int inv_lim=fpow(lim,mod-2).x;
    for(int i=0;i<lim;i++)f[i]=(long long)f[i]*inv_lim%mod;
  }
  inline void dft(z *a,int lim){dft((int*)a,lim);}
  inline void idft(z *a,int lim){idft((int*)a,lim);}
  inline void dft(poly &a,int lim){a.resize(lim),dft((int*)&a[0],lim);}
  inline void idft(poly &a,int lim){a.resize(lim),idft((int*)&a[0],lim);}
  
  inline poly mul(poly a,poly b,int len=-1){
    int lim=dft_init((int)a.size()+(int)b.size()-1);
    dft(a,lim),dft(b,lim);
  ...
  struct PolynomialInit{PolynomialInit(){poly_init(N);}}_polynomial_initer;
}
...
int main(){
  ...
  read(n,m);
  poly f,g;
  f.resize(n+1),f.input();
  g.resize(m+1),g.input();
  (f*g).output();
}