...
void main() {
	...
	for (int k = 1; k <= T; k++) {
        ...
        for (int i = 0; i <= l; i++) DP[k][i] = (ll)DP[k][i] * d % mod;
    }
    for (int i = 0; i <= T; i++) {
        if (i < 1) for (int j = 0; j <= l; j++) S0[0][j] = 1;
        if (i > 0) for (int j = 0; j <= l; j++) S0[i][j] = (ll)S0[i - 1][j] * DP[i][j] % mod;
        for (int j = 0; j <= l; j++) S1[i][j] = finv(S0[i][j]);
    }
    for (int l = 1; l <= n; l++)
        for (int r = l; r <= n; r++) {
            for (int i = 0; i <= l; i++) F[i] = (ll)S[r][i] * S[l - 1][i] % mod;
