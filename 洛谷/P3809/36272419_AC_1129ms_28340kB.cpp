...
void SA_debug() {
	printf("SA : "); for (int i = 1; i <= n; i++) print(sa[i], " \n"[i == n]);
	printf("RNK: "); for (int i = 1; i <= n; i++) print(rnk[i], " \n"[i == n]);
}
...
void SA() {
	...
	SA_sort();
	for (int len = 1, cnt = 0; cnt < n; siz = cnt, len <<= 1) {
		cnt = 0;
		for (int i = 1; i <= len; i++) tmp[++cnt] = n - len + i;
		// for (int i = n; i >= n - len + 1; i--) tmp[++cnt] = i;
		for (int i = 1; i <= n; i++) if (sa[i] > len) tmp[++cnt] = sa[i] - len;
		SA_sort(), std::swap(tmp, rnk);
		rnk[sa[1]] = cnt = 1;
