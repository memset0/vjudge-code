...
int m, v, x, y, z, k, w, l, r, op, cnt, ans, rt[N];
...
inline int new_node(int v) { int u = ++cnt; p[u].val = v; return update(u), u; }
...
void pushup(int u) {
	int t; p[u].rev ^= 1;
	if (p[u].ch[0]) p[t = ++cnt] = p[p[u].ch[0]], p[u].ch[0] = t;
	if (p[u].ch[1]) p[t = ++cnt] = p[p[u].ch[1]], p[u].ch[1] = t;
	std::swap(p[u].ch[0], p[u].ch[1]);
}

void pushdown(int u) {
	int t; if (!p[u].rev || !u) return;
	if (p[u].ch[0]) p[t = ++cnt] = p[p[u].ch[0]], p[u].ch[0] = t;
	if (p[u].ch[1]) p[t = ++cnt] = p[p[u].ch[1]], p[u].ch[1] = t;
