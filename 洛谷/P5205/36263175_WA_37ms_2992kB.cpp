...
void main_init(int lim) {
	for (int len = 1, wn, __lim = std::min(lim + 100, N << 2); len < __lim; len <<= 1) {
		wn = fpow(3, (mod - 1) / (len << 1)), w[len] = 1;
		for (int i = 1; i < len; i++) w[i + len] = (ll)w[i + len - 1] * wn % mod;
	}
}

inline poly operator + (int a, poly f) { f[0] = sub(f[0], a); return f; }
inline poly operator - (int a, poly f) { f[0] = dec(f[0], a); return f; }
...
inline poly operator + (poly f, const poly &g) {
	...
	for_each(i, f) f[i] = sub(f[i], g[i]);
	return f;
}
inline poly operator - (poly f, const poly &g) {
	f.resize(std::max(f.size(), g.size()));
	for_each(i, f) f[i] = dec(f[i], g[i]);
	return f;
}
...
inline poly sqrt(const poly &f) {
	poly g(1, 1);
	for (int len = 2; (len >> 2) < f.size(); len <<= 1)
		g = resize((g * g + f) * inv(2 * g), len);
	return g.resize(f.size()), g;
}

// void main() {
// 	poly A, F; read(n), read(m), ++n; main_init(n << 2);
// 	for (int i = 1; i <= m; i++) read(a[i]), b[a[i]] = 1;
// 	for (int i = 0; i < n; i++) A.push_back(b[i]);
// 	F = resize(2 * inv(1 + sqrt(1 - 4 * A)), n);
// 	for_each(i, F) print(F[i], " \n"[i == __lim - 1]);
// }

// void main() {
// 	read(n), read(m), ++n, ++m, main_init(n + m); poly A, B;
