...
void main() {
	...
	for (int i = 1, top; i <= m; i++) {
		...
		que[++til] = 1, std::sort(que + 1, que + til + 1, cmp);
		for (int i = 1; i <= til; i++) {
			u = que[i];
			while (top && in[u] < in[stk[top]] && out[stk[top]] < out[u])
				G[u].push_back(stk[top--]);
			t = lca(stk[top], u);
			while (top && in[t] < in[stk[top]] && out[stk[top]] < out[t])
		...
		dp(1), print(f[1], '\n');
		for (int i = 2; i <= til; i++) tag[que[i]] = 0;
	}
}
