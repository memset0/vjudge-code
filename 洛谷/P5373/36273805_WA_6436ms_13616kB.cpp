...
const int N=2e4+10,M=150,mod=998244353;
...
void mul(int *f,int *g,int *h,int n,int m){
	...
	int lim=1,k=0; while(lim<(n+m-1))lim<<=1,++k;
	for(int i=0;i<lim;i++)rev[i]=(rev[i>>1]>>1)|((i&1)<<(k-1));
	int ilim=fpow(lim,mod-2);
	memcpy(a,f,n<<2),memset(a+n,0,(lim-n)<<2),ntt(a,lim);
	memcpy(b,g,m<<2),memset(b+n,0,(lim-n)<<2),ntt(b,lim);
...
int main(){
	...
	for(int i=0;i<=sqn;i++){
		...
		for(int j=0;j<sqn&&i*sqn+j<n;j++){
			for(int k=0;k<m;k++)T[k]=(T[k]+(ll)F[i*sqn+j]*Q[j][k])%mod;
		}
