...
int query(int ql, int qr, bool flag, int l, int r, int u, int v) {
	if (ql == l && qr == r) return flag ? p[u].sum : p[u].cnt;
	int mid = (l + r) >> 1;
	if (qr <= mid) return query(ql, mid, flag, l, mid, p[u].lc, p[v].lc);
	else return query(mid + 1, qr, flag, mid + 1, r, p[u].rc, p[v].rc);
}
...
void solve2() {
	...
	for (int i = 1; i <= q; i++) {
		...
		if (!~ans) puts("Poor QLW");
		else get_cnt(l, r, ans) - (get_sum(a, b, ans) - w) / ans;
	}
}
