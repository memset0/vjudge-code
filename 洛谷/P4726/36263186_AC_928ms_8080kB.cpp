...
poly mul(poly a, poly b, int len=-1) {
  int rlen=~len?len:a.size() + b.size() - 1,lim = init(a.size() + b.size() - 1);
  dft(a, lim), dft(b, lim);
  for (int i = 0; i < lim; i++) mul_up(a[i], b[i]);
  idft(a, lim);
  return a.resize(rlen), a;
}
...
poly exp(poly f, int len = -1) {
  ...
  for (int i = 0; i < tmp.size() && i < f.size(); i++) tmp[i] = inc(tmp[i], f[i]);
  ans = mul(ans, tmp, len);
  return ans;
}
...
void main() {
  read(n), f.resize(n), f.in(), exp(f).out();
}
