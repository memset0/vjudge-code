...
namespace g{
	...
	int cnt,sum[L];
	pointer_t lc[L],rc[L];
	void build(pointer_t &u,int l,int r){
		u=++cnt;
		if(l==r)return;
		int mid=(l+r)>>1;
		build(lc[u],l,mid),build(rc[u],mid+1,r);
	}
	void modify(pointer_t &u,int v,int x,int l,int r){
	...
	int query(pointer_t u,int ql,int qr,int l,int r){
