...
const int N=3e5+10,sqn=1200,S=sqn+10;
int n,m,a[N],bln[N];
...
int bas;
struct FC{
  int n,k,arr[S],l[S],r[S];
  inline void loc(int x){while(k<n&&arr[k+1]<=x)++k;}
  inline void source(int i){n=s[i].n,memcpy(arr+1,s[i].val+1,n<<2);}
...
void locate(int x,int l,int r){
  for(int i=1;i<=bln[n];i++)tr[bas+i].k=l<=i&&i<=r?-1:0;
  for(int i=bas;i>=1;i--)tr[i].k=tr[i<<1].k||tr[i<<1|1].k?-1:0;
  tr[1].k=0;
