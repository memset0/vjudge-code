...
std::set<int> set[N];
int fac[N], ifac[N], root[N];
int W, n, m, cnt, nod, counter, ans = 1;
...
void build(int u) {
	if (p[u].left + 1 < p[u].right) {
		...
		p[u].lson = ++nod, p[nod].init(p[u].left, mid), build(nod);
		p[u].rson = ++nod, p[nod].init(mid, p[u].right), build(nod);
...
void main() {
	...
	int last = -1;
	for (auto u : set[n]) {
		if (~last) root[++cnt] = ++nod, p[nod].init(last, u);
		last = u;
	}
	for (int i = 1; i <= cnt; i++)
		build(root[i]);
	print(n - 3 - counter, ' '), print(ans, '\n');
}
