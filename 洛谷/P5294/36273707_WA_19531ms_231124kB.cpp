...
inline bool checkL(int l, int r, int L) {
...
inline bool checkR(int l, int r) {
	frac mid(s3[r] - s3[l - 1] + y - a[x], r - l + 1);
...
int searchL(int R) {
	...
	while (l <= r) {
		int mid = (l + r) >> 1, now = mid == t1.top[x - 1] ? x : t1.query(t1.rt[x - 1], mid + 1).l;
		checkL(now, R, mid) ? (l = mid + 1, ans = now) : r = mid - 1;
	} return ans;
}

std::pair<int, int> searchR() {
	int l = x, r = n; std::pair<int, int> ans(-1, -1);
	while (l <= r) {
		int mid = (l + r) >> 1, now = searchL(mid);
		checkR(now, mid) ? (ans = std::pair<int, int>(now, mid), r = mid - 1) : l = mid + 1;
	} return ans;
}
