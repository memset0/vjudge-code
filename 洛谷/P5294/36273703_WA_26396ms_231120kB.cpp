...
inline int solve(int L, int R, int x, int y) {
	...
	if (L > 1) {
		left = t1.query(t1.rt[x - 1], L - 1).val;
		if (mid < left) return -1;
		cost = inc(cost, t1.querySum(t1.rt[x - 1], 1, L - 1));
	}
	if (r < n) {
...
void main() {
	...
	for (int i = 1, x, y, lL, lR, rL, rR, mL, mR, now, ans, flag; i <= m; i++) {
		read(x), read(y), ans = -1;
		lR = 0, rR = t2.top[x + 1];
		while (lR <= rR) {
			mR = (lR + rR) >> 1;
			lL = 0, rL = t1.top[x - 1], flag = false;
			while (lL <= rL) {
				mL = (lL + rL) >> 1;
				now = solve(mL, mR, x, y);
				// printf(">> solve %d %d %d %d => %d\n", mL, mR, x, y, now);
				(~now) ? (lL = mL + 1, flag = 1, ans = now) : rL = mL - 1;
			}
			flag ? lR = mR + 1 : rR = mR - 1;
		}
		print(ans, '\n');
	}
}
