...
inline bool checkL(int l, int L, int r) {
	frac mid(s3[r] - s3[l - 1] + y - a[x], r - l + 1);
	if (L && mid <= t1.query(t1.rt[x - 1], L).val) return false;
	return true;
}

inline bool checkR(int l, int r, int R) {
	frac mid(s3[r] - s3[l - 1] + y - a[x], r - l + 1);
	// if (r < n && frac(a[r + 1], 1) <= mid) return false;
    if (r < n && mid >= t2.query(t2.rt[x + 1], R).val) return false;
	return true;
}
...
int searchL(int R) {
	...
	while (l <= r) {
		int mid = (l + r) >> 1;
        int now = mid == t1.top[x - 1] ? x : t1.query(t1.rt[x - 1], mid + 1).l;
		checkL(now, mid, R) ? (l = mid + 1, ans = now) : r = mid - 1;
	} return ans;
}

std::pair<int, int> searchR() {
	int l = 0, r = t2.top[x + 1]; std::pair<int, int> ans(-1, -1);
    // printf("search right [%d %d]\n", l, r);
	while (l <= r) {
		int mid = (l + r) >> 1;
        int now = mid == t2.top[x + 1] ? x : t2.query(t2.rt[x + 1], mid + 1).r;
        int res = searchL(now);
        // printf("R : l=%d mid=%d r=%d | now=%d check=%d\n", l, mid, r, now, checkR(now, mid));
		checkR(res, now, mid) ? (ans = std::make_pair(res, now), l = mid + 1) : r = mid - 1;
	} return ans;
}

int calc(const std::pair<int, int> &it) {
    // printf(">> %d %d\n", it.first, it.second);
	int l = it.first, r = it.second;
	int res = calc(r - l + 1, inc(dec(s1[r], s1[l - 1]), dec(y, a[x])), inc(dec(s2[r], s2[l - 1]), dec(sqr(y), sqr(a[x]))));
	if (l > 1) res = inc(res, t1.querySum(t1.rt[l - 1], 1, t1.top[l - 1]));
	if (r < n) res = inc(res, t2.querySum(t2.rt[r + 1], 1, t2.top[r + 1]));
	return res;
}
