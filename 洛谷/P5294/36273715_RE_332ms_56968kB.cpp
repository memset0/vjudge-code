...
struct seg {
	...
	int querySum(int u, int l, int r) {
		...
		return inc(querySum(u << 1, l, mid), querySum(u << 1 | 1, mid + 1, r));
	}
	...
	void play(int k) {
		if (flag == 1) {
			...
			top = topBack[k + 1];
			for (auto it = mdf[k].begin(); it != mdf[k].end(); it++)
				if (it->first <= top) {
					stk[it->first] = it->second;
					modify(1, it->first, it->second);
				}
...
int calc(const std::pair<border, border> &it) {
	...
	if (l > 1) res = inc(res, t1.querySum(1, 1, it.first.block));
	if (r < n) res = inc(res, t2.querySum(1, 1, it.second.block));
	return res;
}
