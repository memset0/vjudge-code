...
int main(){
  ...
  dij((n+1)*(m+1));
  // for(int i=1;i<=n+1;i++)for(int j=1;j<=m+1;j++)printf("%lld%c",dis[id(i,j)]," \n"[j==m+1]);
  ...
  // for(int i=1;i<=n;i++)for(int j=0;j<=m;j++)printf("%d%c",bt[i][j]," \n"[j==m]);
  // for(int i=0;i<=n;i++)for(int j=1;j<=m;j++)printf("%d%c",ct[i][j]," \n"[j==m]);
  // for(int k=0;k<4;k++)for(int i=1;i<=n+1;i++)for(int j=1;j<=m+1;j++)assert(!G[id(i,j,k)].size());
  ...
  dij((n+1)*(m+1)*4);
  // for(int u=id(1,1,3);u!=1;u=pre[u])printf("%d (%d %d %d)\n",u,idx((u-1)%((n+1)*(m+1))+1),idy((u-1)%((n+1)*(m+1))+1),(u-1)/(n+1)/(m+1));
  // for(int k=0;k<4;k++){for(int i=1;i<=n+1;i++)for(int j=1;j<=m+1;j++)printf("%lld%c",dis[id(i,j,k)]>100?-1:dis[id(i,j,k)]," \n"[j==m+1]);puts("");}
  printf("%lld\n",dis[id(1,1,3)]);
}