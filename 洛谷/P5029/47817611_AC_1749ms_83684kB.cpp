...
const int N = 1e5 + 10, M = 2e4 + 10, inf = 1e9, Nd = 8e5, Ed = 2e6;
int n, m, k, s, e, cnt;
...
void build(int l, int r, int u) {
	...
	p[u].l = l, p[u].r = r, p[u].mid = (l + r) >> 1, p[u].id[0] = ++cnt, p[u].id[1] = ++cnt, add(p[u].id[1], p[u].id[0], inf);
	if (l == r) { at[l][0] = p[u].id[0], at[l][1] = p[u].id[1]; return; }
