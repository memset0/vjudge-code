...
long long check(long long mid){
	...
	for(int i=1;i<=n;i++){
		...
		while(l<=r&&tr(i,s[r].l)<=tr(s[r].t,s[r].l))r--;
		if(l<=r){
			int ql=s[r].l,qr=s[r].r,mid,res=-1;
			while(ql<=qr){
				mid=(ql+qr)>>1;
				tr(s[r].t,mid)<=tr(i,mid)?(res=mid,ql=mid+1):(qr=mid-1);
			}
			s[r].r=res;
		}
		if(s[r].r>=n)continue;
...
long long wqs(){
	...
	long long ql=-1e12,qr=1e12,mid,res=-1;
	while(ql<=qr){
		mid=(ql+qr)>>1; int cnt=check(mid);
		if(cnt==m){res=mid; break;}
		(cnt>m)?(res=mid,ql=mid+1):(qr=mid-1);
	}
	check(res);
