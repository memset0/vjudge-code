	...
	for (int i = 1; i <= Q; i++) {
		...
		for (int i = 1, u = 1, c; c = T[i] - 'a', i <= lenT; i++) {
			...
			while (u) {
				...
				int from = u;
				u = samS.ch[u][c];
				debug("u=%d from=%d lim=%d | %d %d | %d %d\n", u, from, lim[i],
					dfn[u], dfn[u] + siz[u] - 1, L + lim[i] - 1, R);
				debug(">>> query => %d\n", query(rt[dfn[u] - 1], rt[dfn[u] + siz[u] - 1], L + lim[i], R, 1, lenS));
				if (query(rt[dfn[u] - 1], rt[dfn[u] + siz[u] - 1], L + lim[i], R, 1, lenS)) { lim[i]++; break; }
				if (lim[i] <= samS.len[samS.fa[from]]) {
					u = samS.fa[from], lim[i] = std::min(lim[i], samS.len[u]);
				} else {
					lim[i]--, u = from;
				}
			}
		...
		for (int i = 2; i <= samT.nod; i++) {
			debug("=> i=%d lim=%d len=%d fa_len=%d val=%d\n", i, lim[samT.end[i]], samT.len[i], samT.len[samT.fa[i]], samT.len[i] - lim[samT.end[i]]);
			ans += std::max((int)0, std::min(samT.len[i] - samT.len[samT.fa[i]], samT.len[i] - lim[samT.end[i]]));
		}
		print(ans, '\n');
		samT.clear();
	}
