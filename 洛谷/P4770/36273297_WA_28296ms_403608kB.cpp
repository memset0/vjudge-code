...
void main() {
	...
	for (int i = 1; i <= Q; i++) {
		...
		for (int i = 1, u = 1, c; c = T[i] - 'a', i <= lenT; i++) {
			...
			lim[i] = lim[i - 1];
			while (u) {
				if (!samS.ch[u][c]) { u = samS.fa[u], lim[i] = std::min(lim[i], samT.len[u]); continue; }
				int from = u;
				u = samS.ch[u][c], ++lim[i];
				// printf("u=%d from=%d lim=%d | %d %d | %d %d | %d %d | %d %d\n", u, from, lim[i],
				// 	dfn[u], dfn[u] + siz[u] - 1, rt[dfn[u] - 1], rt[dfn[u] + siz[u] - 1], L, R);
				if (query(rt[dfn[u] - 1], rt[dfn[u] + siz[u] - 1], L + lim[i] - 1, R, 1, lenS)) break;
				u = samS.fa[from], lim[i] = std::min(lim[i] - 1, samT.len[u]);
			}
			if (!u) u = 1, lim[i] = 0;
			// printf(">>> %d | %d %d\n", i, u, lim[i]);
		}
		for (int i = 2; i <= samT.nod; i++) {
			// printf("%d : %d %d %d | %d %d\n", i, samT.len[i], samT.len[samT.fa[i]], lim[samT.end[i]],
			// 	samT.len[i] - samT.len[samT.fa[i]], samT.len[i] - lim[samT.end[i]]);
			ans += std::max(0, std::min(samT.len[i] - samT.len[samT.fa[i]], samT.len[i] - lim[samT.end[i]]));
		}
		print(ans, '\n');
		samT.clear();
	}
