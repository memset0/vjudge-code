...
const int K = 131072, P = 4, p = 1e9 + 7;
int T, sum;
...
void main() {
	read(T), Mker::init();
	base.a[0][0] = 233, base.a[1][0] = 666, base.a[0][1] = 1;
    for (register int p = 0; p < P; p++) {
		f[p][0].a[0][0] = f[p][0].a[1][1] = 1;
        for (int k = 1; k < K; k++) f[p][k] = f[p][k - 1] * base;
        base = base * f[p][K - 1];
    }
	vector temp, ans;
    for (register int i = 1; i <= T; i++) {
        x = Mker::rand(); if (x == 0) continue;
		--x; ans.a = 1, ans.b = 0;
        for (register int i = 0; x; i++) {
			matrix g = f[i][x % K]; temp = ans;
			ans.a = ((ll)temp.a * g.a[0][0] + (ll)temp.b * g.a[1][0]) % p;
			ans.b = ((ll)temp.a * g.a[0][1] + (ll)temp.b * g.a[1][1]) % p;
			x >>= 17;
		}
		sum ^= ans.a;
    }
