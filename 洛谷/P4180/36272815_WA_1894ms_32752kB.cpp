...
const int inf = 0;
int n, m, u, v, t, sum, ans, ans1, ans2;
int used[maxm], dep[maxn];
int f[maxn][20], w1[maxn][20], w2[maxn][20];
...
int lim_max_3(int lim, int a, int b, int c) {
...
int lim_max_4(int lim, int a, int b, int c, int d) {
	if (a == lim) a = -inf;
	if (b == lim) b = -inf;
	if (c == lim) c = -inf;
	if (d == lim) d = -inf;
