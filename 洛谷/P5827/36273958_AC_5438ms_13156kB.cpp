...
std::vector<int> evalBCC(std::vector<int> set){
  int max=*max_element(set.begin(),set.end()); init(max+5);
  poly::vec D(max),G(max+1),A,B,C;
  for(int i=0;i<=max;i++)G[i]=fpow(2,(int64_t)i*(i-1)/2%(mod-1))*ifac[i];
  G=poly::ln(G);
  for(int i=0;i<max;i++)D[i]=G[i+1]*(i+1);
  A=poly::deri(D),B=poly::ln(D);
  std::vector<int> res;
  for(int n:set)switch(n){
    case 1:res.push_back(1);break;
    default:res.push_back(((A*poly::exp(poly::vec{mod-n}*B))[n-2]*fac[n-1]*inv[n-1]).x);
  }
  return res;
}
int main(){
  ...
  for(int x:evalBCC(std::vector<int>{a,b,c,d,e}))printf("%d\n",x);
}