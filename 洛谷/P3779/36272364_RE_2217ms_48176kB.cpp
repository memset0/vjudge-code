...
inline void fft(complex *a, complex *w) {
...
inline void mul(poly &f, poly &g, poly &h) {
	...
	h.n = f.n + g.n - 1, lim = 1, k = 0;
	while (lim < h.n) lim <<= 1, ++k;
	...
	// printf(">> mul n=%d m=%d l=%d lim=%d k=%d | %u %u %u \n", f.n, g.n, h.n, lim, k, &f, &g, &h), f.print(), g.print();
	for (int i = 0; i < lim; i++) {
		a[i] = { i < f.n ? f.a[i] : 0, 0 };
		b[i] = { i < g.n ? g.a[i] : 0, 0 };
	}
	...
	fft(a, w2);
	for (int i = 0; i < h.n; i++) h.a[i] = a[i].x / lim;
    // h.print();
}
