...
int n, m, u, v, x, y, c, opt;
int fa[maxn], siz[maxn], ch[maxn][2];
int sum[maxn], val[maxn], plus[maxn], mul[maxn], rev[maxn];
...
inline void pushup_plus(int u, int c) {
...
inline void pushup_mul(int u, int c) {
...
void cut(int x, int y) { split(x, y), ch[y][0] = fa[x] = 0, update(y); }
