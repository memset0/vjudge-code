...
inline int id2(int x) { return x <= sqm ? x : nod2 - n / x + 1; }
...
inline int S(int n, int k) {
	if (k == 1) {
		// printf("%d : %d\n", n, id(n));
		if (map1.find(n) != map1.end()) return s1[id1(n)] + 1;
		if (map2.find(n) != map2.end()) return s2[id2(n)] + 1;
		puts("!");
	}
	int res = 0;
...
void main() {
	...
	for (int i = 1; i <= n; i = a1[nod1] + 1) a1[++nod1] = n / (n / i), map1[a1[nod1]] = nod1;
	for (int i = 1; i <= m; i = a2[nod2] + 1) a2[++nod2] = m / (m / i), map2[a2[nod2]] = nod2;
	cnt = 0;
