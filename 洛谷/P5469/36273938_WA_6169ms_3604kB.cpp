...
const int N = 310, M = 2300, mod = 1e9 + 7;
std::vector<int> rec;
bool tag[N], mrk[N][N];
...
void main() {
	...
	for (int i = 1; i <= n; i++) ++cnt, l[cnt] = r[cnt] = i, mrk[i][i] = 1;
	init_dfs(1, n);
	...
	for (int i = 0; i < rec.size(); i++) {
		...
		for (int x = 1; x <= n + 2; x++) {
			for (int i = 1; i <= n; i++) {
				f[i][x] = f[i][x - 1] + (l[i] == r[i] && tag[l[i]]);
				for (int k = L[i]; k <= R[i]; k++) if (tag[k])
					f[i][x] = (f[i][x] + (ll)f[id[l[i]][k - 1]][x] * f[id[k + 1][r[i]]][x - 1]) % mod;
			}
