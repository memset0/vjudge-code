...
int n, ans, a[N], cnt[N], fac[N], ifac[N], pow[N];
...
inline int solve(int k) {
    ...
    for (int i = 1; i <= k; i++) cost = mul(cost, pow[a[i] >> 1]);
    for (int i = 1; i <= k; i++)		
        for (int j = i + 1; j <= k; j++)
            cost = mul(cost, pow[std::__gcd(a[i], a[j])]);
    // printf(">> cost = %d\n", cost);
