...
int cnt;
int siz[maxm], l[maxm], r[maxm], ch[maxm][2], rnd[maxm], root[maxn];

void update(int x) { siz[x] = siz[ch[x][0]] + siz[ch[x][1]] + r[x] - l[x] + 1; }

int newnode(int nl, int nr) {
	rnd[cnt] = rand();
	cnt++, l[cnt] = nl, r[cnt] = nr;
	ch[cnt][0] = ch[cnt][1] = 0;
	return cnt;
}
...
void split(int u, int k, int &x, int &y) {
	...
	if (k <= siz[ch[u][0]]) {
		...
		if (k - siz[ch[u][0]] < r[u] - l[u] + 1) {
			int want = l[u] + k - siz[ch[u][0]] - 1;
			r[u] = want;
			ch[u][1] = merge(newnode(l[u], want), ch[u][1]);
			update(u);
		}
		x = u;
...
//	freopen("INPUT", "r", stdin);
	...
	for (int i = 1; i <= n; i++)
		root[i] = newnode((i - 1) * m + 1, (i - 1) * m + m - 1);
	for (int i = 1; i <= n; i++)
