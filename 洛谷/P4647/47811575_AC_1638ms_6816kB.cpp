...
int query_bound(int _l, int _r, int k) {
	...
	for (int l = _l, r = _r, mid; mid = (l + r) >> 1, l <= r; )
		if (query(1, mid) == k) ans = mid, r = mid - 1;
		else l = mid + 1;
	return ans;
}

int query_bound2(int _l, int _r, int k) {
	int ans = _l;
	for (int l = _l + 1, r = _r, mid; mid = (l + r) >> 1, l <= r; )
		if (query(1, mid) == k) ans = mid, l = mid + 1;
		else r = mid - 1;
	return ans;
}
...
void main() {
	...
	for (int i = 1, pos, end, limit = m + 1, length; i <= n; i++) {
		limit -= a[i].h - a[i - 1].h;
		// printf("main [%d %d] %d\n", a[i].h, a[i].k, limit);
		modify(1, limit, limit + a[i].k - 1, 1);
		// for (int i = 1; i <= m; i++)
		// 	print(query(1, i), " \n"[i == m]);
		if (limit + a[i].k - 1 < m && query(1, limit + a[i].k - 1) > query(1, limit + a[i].k)) {
			pos = query_bound(limit, limit + a[i].k - 1, query(1, limit + a[i].k) + 1);
			end = query_bound2(limit + a[i].k, m, query(1, limit + a[i].k));
			// printf("========> %d %d\n", query(1, m) + 1, query(1, limit + a[i].k - 1));
			length = limit + a[i].k - pos;
			// printf("len=%d [%d %d] [%d %d]\n", length, pos, pos + length - 1, end - length + 1, end);
			modify(1, pos, pos + length - 1, -1);
			modify(1, end - length + 1, end, 1);
		}
		// for (int i = 1; i <= m; i++)
		// 	print(query(1, i), " \n"[i == m]);
		// puts("");
