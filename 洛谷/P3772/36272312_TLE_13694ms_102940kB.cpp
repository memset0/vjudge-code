...
inline double calc(int l,int r){
	...
	double L=l==0?1:(query(1,1,l)*B[0]).a[2+tag[l]][0];
	double R=r==n+1?1:M.a[2+tag[r]][0];
	double S=(M.a[tag[r]][0])/(R)-(tag[r]^1);
	// printf("calc %d[%d] %d[%d] : %.2lf %.2lf %.2lf\n",l,tag[l],r,tag[r],L,R,S);
	// M.out(),query(1,l+1,n).out();
	return S;
}
int main(){
#ifdef memset0
