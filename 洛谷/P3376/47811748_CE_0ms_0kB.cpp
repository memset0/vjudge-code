...
struct MaxFlow{
  ...
  inline void add(int u,int v,int w){
    ...
    G[v].push_back(E.size()),E.push_back((Edge){u,w});
  }
  int bfs(){
    std::queue<int> q;
  ...
  int dfs(int u,int lim){
    ...
    for(int &k=cur[u];k<G[u].size();k++){
      ...
      if(E[i].val&&dep[E[i].to]==dep[u]+1)
        if(int flow=dfs(E[i].to,std::min(lim,E[i].val))){
          E[i].val-=flow;
          E[i^1].val+=flow;
          return flow;
        }
  ...
  void clear(){E.clear(),G.clear();}
  void init(int n){s=n+1,e=n+2,G.resize(n+3),cur.resize(n+3),dep.resize(n+3);}
}flow;
