...
int n,m,l,ans,a[N],c[N],s[N],v[N],pinv[N],w[N<<2],rev[N<<2],f[N],g[N],fac[N],ifac[N];
...
void solve(std::multiset<int> &set,int *res){
	m=0,res[0]=1;poly k,F,G,H;
	if(!set.size())return;
	while(set.size()>1)
		if((*set.begin())*(*--set.end())<=l){
			a[++m]=*set.begin(),c[m]=1,set.erase(set.begin());
		}else{
			a[++m]=*--set.end(),c[m]=-1,set.erase(--set.end());
		}
	a[++m]=*set.begin();
	for(int i=2;i<=m;i++)s[i]=s[i-1]+c[i-1];
	for(int i=1;i<=n;i++)k.push_back(i);
	F=MPE(dc(1,m),k),G.resize(n);
	// for(int i=0;i<n;i++)printf("%d%c",F[i]," \n"[i==n-1]);
	for(int i=0;i<n;i++)F[i]=(ll)(i&1?mod-F[i]:F[i])*ifac[i]%mod;
	for(int i=0;i<n;i++)G[i]=ifac[i];
	// for(int i=0;i<n;i++)printf("%d%c",G[i]," \n"[i==n-1]);
	H=mul(F,G,n);
	for(int i=0;i<n;i++)res[i+1]=(ll)H[i]*fac[i]%mod*(i&1?mod-1:1)%mod;
}
int main(){
#ifdef memset0
	...
	for(int i=1;i<=n;i++)read(v[i]),(v[i]>=0?s1:s2).insert(v[i]);
	solve(s1,f),solve(s2,g);
	// for(int i=0;i<=n;i++)printf("%d%c",f[i]," \n"[i==n]);
	// for(int i=0;i<=n;i++)printf("%d%c",g[i]," \n"[i==n]);
	for(int i=1;i<=n;i++)ans=(ans+(ll)f[i]*g[i]%mod+(ll)f[i]*g[i-1]%mod+(ll)f[i-1]*g[i]%mod)%mod;
