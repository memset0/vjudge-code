...
namespace mem{ //v1.8.7 => size: 13.43KiB
  #define MEM_IO
  #define MEM_STR
  #define MEM_MATH
  #define MEM_UTILS
...
int n,m,ans,cnt[N],a[N],lst[N],at[N];

inline long long calc(int x){return (long long)x*(x+1)/2;}
...
std::vector<std::pair<interval,int>> max,min;

struct Info{
  int min,cnt;
};
const Info empty{INT_MAX,0};
...
struct Node{
  int l,r,mid,lazy,tag;
  Info val,cur;
  inline void debug(){
    printf("%d %d %d %d\n",val.min,val.cnt,cur.min,cur.cnt);
  }
}p[N<<2];
...
inline void deploy(int k,int w){
  if(at[k]>=k){
    const interval &it=vec[k][at[k]-k];
    // printf("deploy %d %d [%d %d] [%d ~]\n",k,w,it.l,it.r,lst[k]);
    if(1<=it.l-1)update(1,1,it.l-1,w);
    if(it.r+1<=lst[k])update(1,it.r+1,lst[k],w);
  }
}
...
int main(){
  ...
  for(int i=1;i<=n;i++){
    ...
    deploy(a[i],-1);
    if(at[a[i]]){
      int lst=vec[a[i]][at[a[i]]-1].r;
      vec[a[i]][at[a[i]]++]={lst+1,i};
    }else{
      vec[a[i]][at[a[i]]++]={1,i};
    }
    lst[a[i]]=i;
    ...
    max.push_back({{i,i},calc(a[i])});
    while(max.size()>=2){
      auto a=max[max.size()-2];
      auto b=max[max.size()-1];
      if(a.second<b.second){
        modify(1,a.first.l,a.first.r,b.second-a.second);
        max.pop_back();
        max.pop_back();
        max.push_back({{a.first.l,b.first.r},b.second});
      }else{
        break;
      }
    }
    min.push_back({{i,i},-calc(a[i]-1)});
    modify(1,i,i,calc(a[i])-calc(a[i]-1));
    while(min.size()>=2){
      auto a=min[min.size()-2];
      auto b=min[min.size()-1];
      // printf("! [%d] %d %d\n",(int)min.size(),a.second,b.second);
      if(a.second<b.second){
        modify(1,a.first.l,a.first.r,b.second-a.second);
        min.pop_back();
        min.pop_back();
        min.push_back({{a.first.l,b.first.r},b.second});
      }else{
        break;
      }
    }
