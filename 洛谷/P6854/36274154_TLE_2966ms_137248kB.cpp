...
int n,m,ans,LST,cnt[N],a[N],lst[N],at[N];
...
Stack<std::pair<interval,int>> max,min;
...
inline void deploy(int k,int w){
  if(at[k]>=k){
    ...
    if(LST+1<=it.l-1)update(1,LST+1,it.l-1,w);
    if(it.r+1<=lst[k])update(1,it.r+1,lst[k],w);
  }
}

inline void applyMerge(Stack<std::pair<interval,int>> &max){
  while(max.size()>=2){
    auto a=max.stk[max.top-2];
    auto b=max.stk[max.top-1];
    if(a.second<b.second){
      modify(1,a.first.l,a.first.r,b.second-a.second);
      max.pop();
      max.pop();
      max.push({{a.first.l,b.first.r},b.second});
    }else{
      break;
    }
  }
...
int main(){
  ...
  for(int i=1;i<=n;i++){
    ...
    max.push({{i,i},calc(a[i])});
    applyMerge(max);
    min.push({{i,i},-calc(a[i]-1)});
    applyMerge(min);
