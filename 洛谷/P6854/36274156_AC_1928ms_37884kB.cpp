...
int main(){
  ...
  for(int i=1;i<=n;i++){
    ...
    auto it=std::find(q.begin(),q.end(),a[i]);
    if(it!=q.end()){
      q.erase(it);
    }
    lst[a[i]]=i;
    ...
    // for(int i=1;i<=m;i++){
    //   printf("[%d] ",i);
    //   for(interval x:vec[i])printf("(%d,%d)",x.l,x.r);
    //   printf("\n");
    // }

    int now=i,min=INT_MAX,max=INT_MIN;
    interval cur={lim,i},tmp;
    for(int i=0;i<q.size();i++){
      if(at[q[i]]<q[i]){
        break;
      }else{
        cur=cur&vec[q[i]][at[q[i]]-q[i]];
      }
      tmp=cur;
      if(i+1!=q.size()){
        tmp=tmp&interval{lst[q[i+1]]+1,now};
      }
      min=std::min(min,q[i]);
      max=std::max(max,q[i]);
      if(tmp.l<=tmp.r&&i+1==max-min+1){
        ++ans;
      }
      // printf("i=%d qi=%d lst=%d cur=(%d,%d) tmp=(%d,%d) ans=%d\n",i,q[i],lst[q[i]],cur.l,cur.r,tmp.l,tmp.r,ans);
    }
  }
  printf("%d\n",ans);
}