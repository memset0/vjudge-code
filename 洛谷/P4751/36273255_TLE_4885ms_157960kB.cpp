...
void main() {
	...
	dfs(1), dp(1);
	for (int i = 1; i <= n; i++) g[i].a[0][1] = g[i].a[0][0], g[i].a[1][1] = -1e18;
	rt = build(1);
	for (int i = 1, k, w, lastans = 0; i <= m; i++) {
		read(k), read(w), k ^= lastans, modify(k, w - val[k]), val[k] = w;
		print(lastans = std::max(f[rt][0], f[rt][1]), '\n');
		// for (int i = 1; i <= n; i++)
