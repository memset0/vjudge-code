...
const int N = 3e6 + 10;
int n, m, k, tn, tm, sqn, ans, tmp;
int out[N], cnt[N], bln[N];
unsigned int a[N], b[N], h[N];
char s[N];
...
inline void add(unsigned int x) { debug("add %u %d\n", x, cnt[x]); ++cnt[x]; ans += (cnt[x] << 1) - 2; }
inline void del(unsigned int x) { debug("del %u %d\n", x, cnt[x]); ans -= (cnt[x] << 1) - 2; --cnt[x]; }
