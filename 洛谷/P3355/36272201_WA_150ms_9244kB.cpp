...
const int maxn = 210, maxm = 40010, maxe = 640010, inf = 1e9;
...
int main() {
	...
	rep(i, 1, n) rep(j, 1, n) if (!a[i][j]) rep(t, 0, 7) {
		x = i + mov[t][0], y = j + mov[t][1];
		if (!a[x][y] && x > 0 && y > 0 && x <= n && y <= n) add_edge(at(i, j), at(x, y), 1);
	}
	rep(i, 1, n) rep(j, 1, n) if (!a[i][j]) {
		if ((i + j) & 1) add_edge(s, at(i, j), 1);
