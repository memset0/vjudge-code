...
const int N = 8e5 + 10, mod = 167772161;
int n, len, lim = 1, k, inv_lim;
int f[N], g[N], fac[N], ifac[N], w[N], rev[N];
...
void main() {
    ...
    for (int i = 0; i < len; i++) f[i] = mul(fpow(i, n), ifac[i]), g[i] = i & 1 ? mod - ifac[i] : ifac[i];
    while (lim < (len << 1) - 1) lim <<= 1, ++k;
    ...
    for (int i = 0; i < len; i++) print(mul(f[i], inv_lim), " \n"[i == n]);
}
