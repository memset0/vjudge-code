...
const int N = 5e5 + 10, sqn = 650, M = N / sqn + 10;
int n, m, l, r, tn, ans;
...
void main() {
    ...
    for (int l = 1; l <= bln[n]; l++)
        for (int r = l; r <= bln[n]; r++) {
            for (int i = fst[l]; i < fst[r + 1]; i++) ++cnt[a[i]], maxd(pre[l][r], cnt[a[i]]);
            for (int i = fst[l]; i < fst[r + 1]; i++) --cnt[a[i]];
        }
    for (int i = 1; i <= n; i++) vet[a[i]].push_back(a[i]), pos[i] = vet[a[i]].size() - 1;
    for (int i = 1; i <= m; i++) {
        read(l), read(r), l ^= ans, r ^= ans, ans = 0;
        if (bln[l] == bln[r]) {
            for (int i = l; i <= r; i++) ++cnt[a[i]], maxd(ans, cnt[a[i]]);
            for (int i = l; i <= r; i++) --cnt[a[i]];
        } else {
            if (bln[l] + 1 <= bln[r] - 1) ans = pre[bln[l] + 1][bln[r] - 1];
            for (int i = l; l < fst[bln[l] + 1]; l++)
