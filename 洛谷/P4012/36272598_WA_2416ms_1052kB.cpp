...
const int maxn = 18, maxm = 250, maxe = 100010;
const int inf = 1e9;

#define at(x,y) ((x) * (m - 1) + (y))
...
int main() {
	...
	rep(i, 1, n) rep(j, 1, m - 1) add_edge(at(i, j), at(i, j + 1), inf, read());
	rep(j, 1, m) rep(i, 1, n - 1) add_edge(at(i, j), at(i + 1, j), inf, read());
	rep(i, 1, tn) { c = read(), x = read() + 1, y = read() + 1, add_edge(s, at(y, x), c, 0); }
	rep(i, 1, tm) { c = read(), x = read() + 1, y = read() + 1, add_edge(at(y, x), e, c, 0); }
	while (spfa()) {
		flow = inf;
