...
//			log(">> %d %d\n",o,v);
			...
			if(o==2)split(rt,v-1,x,y),split(y,v,y,z),rt=merge(x,merge(merge(ch[y][0],ch[y][1]),z));
		if(o==3)split(rt,v-1,x,y),printf("%d\n",siz[x]+1),rt=merge(x,y);
		if(o==4)printf("%d\n",kth(rt,v));
		if(o==5)split(rt,v-1,x,y),printf("%d\n",kth(x,siz[x])),rt=merge(x,y);
		if(o==6)split(rt,v,x,y),printf("%d\n",kth(y,1)),rt=merge(x,y);
		}
	}
}
int main(){
