...
//			printf("%d <= %d\n", u, v);
			...
			for (int i = 0; i <= std::min(m, siz[u]); i++) {
				for (int j = 0; i + j <= std::min(m, siz[u]); j++) {
					(f[u][i + j][0][0] += 1ll * t[i][0][0] * f[v][j][0][0] % mod) %= mod;
					(f[u][i + j][0][1] += 1ll * (t[i][0][0] + t[i][0][1]) * (f[v][j][1][0] + f[v][j][1][1]) % mod) %= mod;
					(f[u][i + j][1][0] += 1ll * t[i][1][0] * f[v][j][0][1] % mod) %= mod;
					(f[u][i + j][1][1] += 1ll * t[i][1][1] * f[v][j][1][1] % mod) %= mod;
				}
			}
		}
}
