...
//			printf("%d <= %d\n", u, v);
			...
			for (int i = 0, lim1 = std::min(m, siz[u]); i <= lim1; i++) {
				for (int j = 0, lim2 = std::min(m, siz[u]); j <= siz[v] && i + j <= lim2; j++) {
					...
					f[u][i + j][0][1] = (f[u][i + j][0][1] + 1ll * t[i][0][0] * f[v][j][1][1] % mod + 1ll * t[i][0][1] * (f[v][j][1][1] + f[v][j][0][1])) % mod;
					f[u][i + j][1][0] = (f[u][i + j][1][0] + 1ll * t[i][1][0] * (f[v][j][0][0] + f[v][j][0][1])) % mod;
					f[u][i + j][1][1] = (f[u][i + j][1][1] + 1ll * t[i][1][0] * (f[v][j][1][0] + f[v][j][1][1]) + 1ll * t[i][1][1] * ((ll)f[v][j][0][0] + f[v][j][0][1] + f[v][j][1][0] + f[v][j][1][1]) % mod) % mod;
				}
			}
		}
}
