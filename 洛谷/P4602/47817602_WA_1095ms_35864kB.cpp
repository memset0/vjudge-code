...
int ans[N], val[N], cst[N], rt[N], cnt[M], sum[M], lc[M], rc[M];

struct info { int val, cst, lim; } a[N];
struct query { int max, lim; } b[N];
...
void modify(int &u, int v, int k, int w, int l, int r) {
	...
	if (l == r) { cnt[u] += w, sum[u] += w * cst[l]; return; }
	int mid = (l + r) >> 1;
