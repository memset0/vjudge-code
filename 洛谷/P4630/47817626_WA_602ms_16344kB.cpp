...
void dfs(int u, int father) {
  ...
  vis[u] = 1;
  f1[u] = siz[u];
  f2[u] = (siz[u] - 1) * (siz[u] - 1);
  ans += (ll)siz[u] * (siz[u] - 1) * (siz[u] - 2);
  for (int i = hed[u], v = to[i]; i; i = nxt[i], v = to[i])
    if (v != father) {
      dfs(v, u);
      f1[u] += f1[v];
      f2[u] += siz[u] * f1[v] + f2[v];
      // printf("%d -> %d >> %lld %lld %lld\n", u, v, 2ll * tmp * f1[v], 2ll * f2[v] * siz[u], 2ll * (siz[u] - 1) * (siz[u] - 1) * f1[v]);
      ans += 2ll * tmp * f1[v], tmp += f1[v];
      ans += 2ll * f2[v] * siz[u];
      ans += 2ll * (siz[u] - 1) * (siz[u] - 1) * f1[v];
    }
}
...
void main() {
  ...
  // for (int i = 1; i <= cnt; i++) print(f1[i], " \n"[i == cnt]);
  // for (int i = 1; i <= cnt; i++) print(f2[i], " \n"[i == cnt]);
  print(ans, '\n');
