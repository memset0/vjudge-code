...
struct atom{
  ...
  inline bool div(const atom &rhs)const{
    return (rhs.x?x%rhs.x==0:!x)&&(rhs.y?y%rhs.y==0:!y);
  }
};
atom q[M],d[N][N];
set<atom> vis;
...
bool query(int x,int y){
  ...
  // log<BLUE>("query %d %d > %d %d > %lld\n",x,y,xx,yy,u);
  if(type==1){
    if(!vec[xx][yy].size())return false;
    // printf("(%d,%d) (%d,%d) (%d,%d)\n",vec[xx][yy][0].x,vec[xx][yy][0].y,u.x,u.y,d[xx][yy].x,d[xx][yy].y);
  ...
  if(r==lim){
    ...
    for(int i=0;i<n;i++)
      for(int j=0;j<m;j++)if(vec[i][j].size()){
        ...
        // assert(d[i][j]);
        // printf("%d %d > %lld %lld %lld\n",i,j,vec[i][j][1]-vec[i][j][0],vec[i][j][2]-vec[i][j][1],vec[i][j][3]-vec[i][j][2]);
