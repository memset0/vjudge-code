...
inline int id(int x,int y){return (x-1)*6+y-1;}
inline void mov(int x,int y,int w,int c){
  print(x,' ',y,' ',w?'D':'R',' ',c,'\n');
}
...
vector<int> dfs(int x,int y){
  vector<int> rsp;
  if(x==1&&y==1){
    if(n)rsp.push_back(d[--n]);
  }else if((x==1&&y==2)||(x==2&&y==1)){
    if(n<=1){if(n)rsp.push_back(d[--n]);}
    else{
      if(d[n-2])mov(1,1,y==1,2);
      else mov(1,1,y==1,1),mov(1,1,y==1,1);
    }
  }else{
    vector<int> p[35];
    for(int i=x;i>=1;i--)
