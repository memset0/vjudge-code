...
namespace mem{ //v2.9.2 => size: 15.67KiB
  #ifdef memset0
  #else
    #define MEM_FASTIO
  #endif
...
const int N=1e5+10;
int n,m,ans[N],arc[N],el[N],er[N],ql[N],qr[N],dis[2][N];
vector<int> G[N];
pair<int,int> e[N],q[N];

void bfs(int u,int n,int *dis){
  queue<int> q;
  memset(dis,0x3f,n<<2),dis[u]=0,q.push(u);
  while(q.size()){
    u=q.front(),q.pop();
...
inline pair<int,int> trans(int s,pair<int,int> x){
  x.first=((x.first-s)%n+n)%n;
  x.second=((x.second-s)%n+n)%n;
  if(x.first>x.second)swap(x.first,x.second);
  return x;
}

void solve(const vector<int> &id,const vector<int> &e,const vector<int> &q){
  int n=id.size();
  log<PURPLE>("solve %d [%d] [%d]\n",n,(int)e.size(),(int)q.size());
  if(!e.size()){
    for(int x:q)ans[x]=1;
    return;
  }
  if(!q.size())return;
  vector<int> le,re,lq,rq,lid,rid;
  int res=INT_MAX,pos;
  for(int i=0;i<id.size();i++)arc[id[i]]=i;
  for(int i=0;i<n;i++){
    G[i].clear();
    G[i].push_back((i+1)%n);
  ...
  for(int i=0;i<e.size();i++){
    el[i]=arc[::e[e[i]].first];
    er[i]=arc[::e[e[i]].second];
    if(el[i]>er[i])swap(el[i],er[i]);
    log("e[%d] = %d, %d\n",e[i],el[i],er[i]);
    int len=(er[i]-el[i]+n)%n;
    if(max(len,n-len)<res){
      res=max(len,n-len),pos=i;
    }
    G[el[i]].push_back(er[i]);
    G[er[i]].push_back(el[i]);
  }
  bfs(el[pos],n,dis[0]);
  bfs(er[pos],n,dis[1]);
  for(int i=0;i<e.size();i++)if(i!=pos){
    if(el[pos]<=el[i]&&er[i]<=er[pos])le.push_back(e[i]);
    if(er[i]<=el[pos]||el[i]>=er[pos]||el[i]<=el[pos]&&er[pos]<=er[i])re.push_back(e[i]);
  }
  log<RED>(">> res=%d pos=%d\n",res,pos);
  for(int i=0;i<q.size();i++){
    ql[i]=arc[::q[q[i]].first];
    qr[i]=arc[::q[q[i]].second];
    if(ql[i]>qr[i])swap(ql[i],qr[i]);
    log("q[%d] = %d, %d\n",q[i],ql[i],qr[i]);
    if(el[pos]<ql[i]&&qr[i]<er[pos])lq.push_back(q[i]);
    else if(qr[i]<el[pos]||ql[i]>er[pos]||ql[i]<el[pos]&&er[pos]<qr[i])rq.push_back(q[i]);
    else ans[q[i]]=min(dis[0][ql[i]]+dis[0][qr[i]],dis[1][ql[i]]+dis[1][qr[i]]);
  }
  int ln=er[pos]-el[pos]+1,rn=rn=n+el[pos]-er[pos]+1;
  log<RED>(">> ln=%d rn=%d\n",ln,rn);
  lid.resize(ln),rid.resize(rn);
  for(int i=0;i<ln;i++)lid[i]=id[(el[pos]+i)%n];
  for(int i=0;i<rn;i++)rid[i]=id[(er[pos]+i)%n];
  lid.shrink_to_fit(),le.shrink_to_fit(),lq.shrink_to_fit();
  rid.shrink_to_fit(),re.shrink_to_fit(),rq.shrink_to_fit();
  solve(lid,le,lq);
  vector<int>().swap(lid),vector<int>().swap(le),vector<int>().swap(lq);
  solve(rid,re,rq);
}

int main(){
  judge_file_io("distance"),local_file_input("1.in");
  vector<int> id,edg,qry;
  read(n);
  for(int i=0;i<n-3;i++){
    read(e[i].first,e[i].second),--e[i].first,--e[i].second;
    edg.push_back(i);
  }
  for(int i=0;i<n;i++)G[i].sort(),id.push_back(i);
  read(m);
  for(int i=0;i<m;i++){
    read(q[i].first,q[i].second),--q[i].first,--q[i].second;
    if(q[i].first!=q[i].second)qry.push_back(i);
  }
  solve(id,edg,qry);
