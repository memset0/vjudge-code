...
void solve(int s,int n,const vector<int> &e,const vector<int> &q){
  ...
  le.shrink_to_fit(),lq.shrink_to_fit();
  solve((s+el[pos])%(::n),er[pos]-el[pos]+1,le,lq);
  vector<int>().swap(le),vector<int>().swap(lq);
  re.shrink_to_fit(),rq.shrink_to_fit();
  solve((s+er[pos])%(::n),n+el[pos]-er[pos]+1,re,rq);
  vector<int>().swap(re),vector<int>().swap(rq);
}
