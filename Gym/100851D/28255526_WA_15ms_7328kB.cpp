...
const int N=1e5+10;
int n,m,ans[N],el[N],er[N],ql[N],qr[N],dis[2][N];
vector<int> G[N];
...
void solve(int s,int n,const vector<int> &e,const vector<int> &q){
  log<PURPLE>("solve %d %d [%d] [%d]\n",s,n,(int)e.size(),(int)q.size());
  if(!e.size()){
    for(int x:q)ans[x]=1;
    return;
  }
  if(!q.size())return;
  vector<int> le,re,lq,rq;
  int res=INT_MAX,pos;
  for(int i=0;i<n;i++){
    G[i].clear();
    G[i].push_back((i+1)%n);
  ...
  // for(int i=0;i<n;i++)log("%d%c",dis[0][i]," \n"[i+1==n]);
  // for(int i=0;i<n;i++)log("%d%c",dis[1][i]," \n"[i+1==n]);
  log<RED>(">> %d %d\n",res,pos);
  for(int i=0;i<q.size();i++){
    tie(ql[i],qr[i])=trans(s,::q[q[i]]);
    // log("q[%d] = %d, %d\n",q[i],ql[i],qr[i]);
    if(el[pos]<ql[i]&&qr[i]<er[pos])lq.push_back(q[i]);
    else if(qr[i]<el[pos]||ql[i]>er[pos]||ql[i]<el[pos]&&er[pos]<qr[i])rq.push_back(q[i]);
    else{
      // log<RED>("> %d %d %d %d\n",dis[0][ql[i]],dis[0][qr[i]],dis[1][ql[i]],dis[1][qr[i]]);
      ans[q[i]]=min(dis[0][ql[i]]+dis[0][qr[i]],dis[1][ql[i]]+dis[1][qr[i]]);
    }
  }
  int ls=(s+el[pos])%(::n),ln=er[pos]-el[pos]+1;
  int rs=(s+er[pos])%(::n),rn=n+el[pos]-er[pos]+1;
  le.shrink_to_fit(),lq.shrink_to_fit(),solve(ls,ln,le,lq),vector<int>().swap(le),vector<int>().swap(lq);
  re.shrink_to_fit(),rq.shrink_to_fit(),solve(rs,rn,re,rq),vector<int>().swap(re),vector<int>().swap(rq);
}
