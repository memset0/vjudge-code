...
void solve(int s,int n,const vector<int> &e,const vector<int> &q){
  ...
  if(!e.size()){
    for(int x:q)ans[x]=::q[x].first==::q[x].second?0:1;
    return;
  }
  if(!q.size())return;
  auto trans=[&](pair<int,int> x){
  ...
  solve((s+el[pos])%(::n),er[pos]-el[pos],le,lq);
  vector<int>().swap(le),vector<int>().swap(lq);
  re.shrink_to_fit(),rq.shrink_to_fit();
  solve((s+er[pos])%(::n),n+el[pos]-er[pos],re,rq);
