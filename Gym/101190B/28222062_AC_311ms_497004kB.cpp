...
namespace mem{ //v2.8.4 => size: 15.65KiB
  #ifdef memset0
  #else
    #define MEM_FASTIO
  #endif
...
const int N=5e6+1e3+5,M=5e5+5;
int n,id,tot,top,cnt,col,fa[M<<1],ch[M<<1][2],stk[N],hed[N],dfn[N],low[N],bel[N];
char s[M],t[M<<1];
int len[M];
vector<int> v[M<<1];
...
inline void addedge(int u,int v){c[++tot]={v,hed[u]},hed[u]=tot;}

void tarjan(int x){
  dfn[x]=low[x]=++id;
  stk[++top]=x;
  for(int i=hed[x];i;i=c[i].nxt){
    int v=c[i].v;
  ...
  if(dfn[x]==low[x]){
    bel[x]=++col;
    while(stk[top]!=x)bel[stk[top--]]=col;
    --top;
  }
}
...
int main(){
  ...
  for(int i=0;i<=cnt;++i)
    for(int j=0;j<2;++j)
      if(ch[i][j]){
        fa[ch[i][j]]=i;
        addedge(i,ch[i][j]);
        addedge(ch[i][j]+M*2,i+M*2);
      }
  int tmp=0;
  for(int i=0;i<=cnt;++i){
    bool fl=true;
    for(auto it:v[i]){
      int t=it>M*5?it-M:it+M;
      if(!fl){
        addedge(t,tmp+M*6);
        addedge(tmp+M*6+1,tmp+M*6);
      }
      fl=false;
      ++tmp;
      addedge(tmp+M*6,it);
      if(~fa[i])addedge(t,fa[i]+M*2);
      if(ch[i][0])addedge(t,ch[i][0]);
      if(ch[i][1])addedge(t,ch[i][1]);
    }
    fl=true;
  ...
  cur=1;
  for(int i=1;i<=n;++i){
    char *p=find(t+cur,t+cur+len[i],'?');
    if(p!=t+cur+len[i]){
      *p=bel[i+M*4]<bel[i+M*5]?'1':'0';
    }
