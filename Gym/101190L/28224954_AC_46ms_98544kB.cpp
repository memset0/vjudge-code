...
int main(){
  ...
  for(int i=pcnt;i;i--){
    int p=prime[i];
    for(int j=0;j<=lim;j++){
      way[i][j]=way[i+1][j],ans[i][j]=ans[i+1][j];
    }
    for(int j=lim;j;j--)
      if(j>=p){
        way[i][j]+=way[i+1][j-p];
        ans[i][j]+=1ll*way[i+1][j-p]*(lg[p])+ans[i+1][j-p];
      }
  }
  read(L,R);
  long long len=1;
