...
const int N=2500,lim=2100;
int pcnt,prime[N],lg[N];
long long L,R,ways[N][N],lenth[N][N];
vector<int> cur;

inline void sieve(int n=lim){
  for(int i=2;i<=n;i++){
    int fl=0;
    for(int j=2;j<i;j++)if(i%j==0)++fl;
    if(!fl)prime[++pcnt]=i;
  }
}
...
inline string get(){
  ...
  for(int i=0;i<(int)cur.size();i++){
    s+=to_string(cur[i]);
    if(i+1<(int)cur.size())s+=", ";
    else s+="], ";
  }
  return s;
}

inline void solve(long long pos,int lst,int res,int fix){
  if(res==0){
    string s=get();
  ...
  long long tem=pos+lenth[lst+1][res]+1ll*fix*(ways[lst+1][res]);
  if(tem<=L||pos>R)return;
  for(int i=lst+1;i<=pcnt&&prime[i]<=res;i++){
    cur.push_back(prime[i]);
    solve(pos,i,res-prime[i],fix+lg[prime[i]]);
    cur.pop_back();
    pos+=lenth[i+1][res-prime[i]]+1ll*(fix+lg[prime[i]])*ways[i+1][res-prime[i]];
  }
}
...
int main(){
  ...
  lg[0]=2;
  for(int i=1;i<=lim;i++)lg[i]=lg[i/10]+1;
  sieve();
  ways[pcnt+1][0]=1;
  lenth[pcnt+1][0]=2;
  for(int i=pcnt;i;i--){
    int p=prime[i];
    for(int j=0;j<=lim;j++){
      ways[i][j]=ways[i+1][j],lenth[i][j]=lenth[i+1][j];
    }
    for(int j=lim;j;j--)
      if(j>=p){
        ways[i][j]+=ways[i+1][j-p];
        lenth[i][j]+=1ll*ways[i+1][j-p]*(lg[p])+lenth[i+1][j-p];
      }
  }
  read(L,R);
  long long len=1;
