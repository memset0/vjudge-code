...
void dfs(int u, int fa) {
    vis[u] = tms;
    for (auto [v, i] : e[u]) {
        if (v == fa || vis[v] == tms) { continue; }
        dfs(v, u);
        if (f[v] == 1) { f[v] ^= 1; f[u] ^= 1; dir[i] ^= 1; }
    }
}
...
void run() {
    ...
    num = 0;
    for (auto& i : xmap) { i.second = ++num; f[num] = 0; e[num].clear(); }
    for (auto& i : ymap) { i.second = ++num; f[num] = 0; e[num].clear(); }
    std::vector<std::vector<int>> ans(num + 1);
    for (int i = 1; i <= n; i++) {
        x[i] = xmap[a[i] - i]; y[i] = ymap[a[i] + i];
        dir[i] = 0; f[x[i]] ^= 1;
    ...
    for (int i = 1; i <= n; i++) {
        if (dir[i] == 0) { ans[x[i]].push_back(i); }
        if (dir[i] == 1) { ans[y[i]].push_back(i); }
    }
    out << "Yes\n";
    for (int u = 1; u <= num; u++) {
