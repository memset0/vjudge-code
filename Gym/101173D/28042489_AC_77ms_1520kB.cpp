...
inline void mov(int si,int sj,int ei,int ej,int c){
  log<BLUE>("mov (%d,%d) (%d,%d) %d\n",si,sj,ei,ej,c);
#ifndef memset0
  while(si<ei)print(si++,' ',sj,' ','D',' ',c,'\n');
  while(sj<ej)print(si,' ',sj++,' ','R',' ',c,'\n');
#endif
}

void solve(int si,int sj,int ei,int ej,vector<int> a){
  if(!a.size()||(si==ei&&sj==ej))return;
  log<GREEN>("solve (%d,%d) (%d,%d) %d   ",si,sj,ei,ej,a.size());
  for(int x:a)log("%d ",x);log("\n");
  if(ei-si+ej-sj==1){
    if(a.size()==1){
      mov(si,sj,ei,ej,1);
    }else{
    ...
    vector<int> id(a.size()),col(a.size());
    int p=0;
    ...
    for(int i=ei;i>=si;i--)
      for(int j=ej;j>=sj;j--)
        if(!(i==si&&j==sj)){
          for(int k=p;k<a.size()&&k<p+f[ei-i+1][ej-j+1];k++){
            col[id[k]]=i*10+j;
          }
          p+=f[ei-i+1][ej-j+1];
        }
    for(int i=0;i<a.size();i++){
      mov(si,sj,col[i]/10,col[i]%10,1);
    }
    for(int i=ei;i>=si;i--)
      for(int j=ej;j>=sj;j--)
        if(!(i==si&&j==sj)&&!(i==ei&&j==ej)){
          ...
          for(int k=0;k<a.size();k++){
            if(col[k]==i*10+j)b.push_back(a[k]);
          }
...
int main(){
  ...
  reverse(d,d+n);
  solve(1,1,6,6,vector<int>(d,d+n));
}
