...
struct Seg{
	...
	int q1(int x,int l,int r,int ql,int qr,int v){ //最大的位置满足<=v
		if(ql>qr || ql>r || qr<l)return 0; if(mn[x] > v)return 0;
		if(l==r)return l;
	...
	int q2(int x,int l,int r,int ql,int qr,int v){ //最小的位置满足>=v
		if(ql>qr || ql>r || qr<l)return n+1; if(mx[x] < v)return n+1;
		if(l==r)return l;
...
void qry(int L,int R){
	...
	int l = 1,r = R-L+1,len = 0; ans = 1e9,cnt = 0;
	while(l<=r){
		int mid = (l+r)>>1;
		if(H(L,L+mid-1) == rH(R-mid+1,R))len = mid,l = mid+1;
		else r = mid-1;
	}
	int lp = L+len,rp = R-len,rest = (R-L+1)-2*len; assert(lp <= rp);
	//
	...
	if(j){
		...
		else pchk(rest-2*(j-lp)-1,lp-max(j-f[j],L)+1);
	}
	...
	if(j!=n+1){
		...
		else pchk(rest-2*(rp-j),min(j+g[j],R)-rp+1);
	}
