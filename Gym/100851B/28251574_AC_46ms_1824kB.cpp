...
struct BigNum{
  ...
  inline bool at(int x){
    return 1&(a[x/20]>>(x%20));
  }
  inline void mul(int x){
    for(int i=0;i<64;i++)a[i]*=10;
    for(int i=0;i+1<64;i++)if(a[i]>=1<<20)a[i+1]+=a[i]>>20,a[i]&=(1<<20)-1;
  }
  inline BigNum operator+(const BigNum &b)const{
    BigNum ret;
    for(int i=0;i<64;i++)ret.a[i]=a[i]+b.a[i];
    for(int i=0;i+1<64;i++)if(ret.a[i]>=1<<20)ret.a[i+1]+=ret.a[i]>>20,ret.a[i]&=(1<<20)-1;
    return ret;
  }
};

vector<pair<string,BigNum>> res;
...
int main(){
  ...
  BigNum pow;
  pow.a[0]=1;
  res.push_back(make_pair("",BigNum()));

  for(int i=0;;i++){
    vector<pair<string,BigNum>> cur,nxt;
    for(const auto &v:res){
      BigNum tem=v.second+pow;
      if(tem.at(i)==1){
        cur.push_back(make_pair(v.first+"1",tem));
        nxt.push_back(make_pair(v.first+"0",v.second));
      }
    }
    nxt.concat(cur);
    res=nxt;
    if(n<=cur.size()){
      string output=cur[n-1].first;
