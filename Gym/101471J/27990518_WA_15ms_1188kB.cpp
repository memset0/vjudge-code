...
int main(){
  ...
  for(int u,v,c,i=1;i<=m;i++){
    cin>>u>>v>>c;
    eu[i]=u,ev[i]=v;
    id[i][0]=F.add(u,v,c),G.add(u,v,c);
    id[i][1]=F.add(v,u,c),G.add(v,u,c);
  }
  F.add(F.s,1,inf),F.add(3,F.e,inf),f=F.dinic(),F.add(F.s,2,inf);
  G.add(G.s,2,inf),G.add(3,G.e,inf),g=G.dinic(),G.add(G.s,1,inf);
  s=f+F.dinic(),G.dinic();
  auto structure=[&](MaxFlow<double> &F,int s){
    for(int i=0;i<F.E.size();i+=2){
      F.E[i].val=F.E[i^1].val;
      F.E[i^1].val=0;
    }
    F.pop(),F.pop(),F.pop();
    F.add(F.s,s,inf),F.add(3,F.e,inf);
    F.dinic();
    for(int i=1;i<=m;i++){
      log("%d %d : %.6lf %.6lf %.6lf %.6lf : %.6lf %.6lf\n",
        eu[i],ev[i],
        F.E[id[i][0]].val,F.E[id[i][0]^1].val,F.E[id[i][1]].val,F.E[id[i][1]^1].val,
        F.E[id[i][0]].val-F.E[id[i][1]].val,F.E[id[i][0]^1].val-F.E[id[i][1]^1].val);
    }
    log("\n");
  };
  structure(F,2);
  ...
  for(int i=1;i<=m;i++){
    double wf=F.E[id[i][0]].val*x+G.E[id[i][0]^1].val*(1-x)-F.E[id[i][1]].val*x-G.E[id[i][1]^1].val*(1-x);
    double wg=F.E[id[i][0]^1].val*x+G.E[id[i][0]].val*(1-x)-F.E[id[i][1]^1].val*x-G.E[id[i][1]].val*(1-x);
    log("[%10.6lf] ",wf+wg);
    cout<<(wf/v)<<" "<<wg<<endl;
  }
  double rf=x*f+(1-x)*(s-g);
  double rg=x*(s-f)+(1-x)*g;
  double ans=pow(rf,a)*pow(rg,1-a)*pow(v,-a);
  cout<<ans<<endl;
}