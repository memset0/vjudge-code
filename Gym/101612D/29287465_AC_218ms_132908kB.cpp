...
const int lim=1<<22,N=lim+9;
int T,n,a[30],d,d1,d2,d3,d4,ans[N],ppc[N];
vector<int> vec[N];

void dfs(int d){
	for(int i=0;i<d&&a[i]+a[d-1]<=lim;i++){
		a[d]=a[i]+a[d-1];
		if(ppc[a[d]]<=4&&d<ans[a[d]]){
			ans[a[d]]=d;
			vec[a[d]]=vector<int>(a,a+d);
		}
		if(d>=2&&a[d]<(1<<d-2))continue;
		dfs(d+1);
	}
}

int main(){
	judge_file_io("dividing");
	...
	while(T--){
		...
		print(ans[n],'\n');
		print(n,' ',vec[n].back(),' ',n-vec[n].back(),'\n');
		for(int i=(int)vec[n].size()-1;i>=1;i--)print(vec[n][i],' ',vec[n][i-1],' ',vec[n][i]-vec[n][i-1],'\n');
	}
}//dsafsadfsadfsadf