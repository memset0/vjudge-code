...
int main(){
	...
	if(n<=44){
		...
		auto calcR=[](u64 a,u64 b){
			int q=0;
			while(a&1^1){
				if(b&1)return make_pair(0ull,-1);
				a>>=1,b>>=1,q++;
			}
		...
		auto verifyR=[&](u64 r,int q){
			// assert(r!=4789227339117263789ull);
			u64 x=inv(r,(ic8)(1)<<(64-q));
			for(int i=0;i<n;i++)a[i]=calc(b[i],x,q);
			u64 tmp=0;
			// printf("check r=%llu x=%llu :: %llu\n",r,x,((1ull<<(64-q))-1));
		...
		auto verify=[&](u64 f){
			...
			if(t){
				for(u64 p=0;p<(1<<t);p++){
					if(verifyR(r+(1ull<<(64-t))*p,q))return true;
				}
			}else{
				return verifyR(r,q);
			}
