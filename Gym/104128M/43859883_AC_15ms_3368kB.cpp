...
void run() {
    ...
    for (auto _ : map) {
        ...
        if (p[ls[u]].y > p[u].y && p[u].y < p[rs[u]].y) {
            ...
            if ((p[l].x < p[u].x && p[u].x < p[r].x) || (p[l].x > p[u].x && p[u].x > p[r].x) || p[r].x == p[u].x) {if (p[l].x < p[u].x) {
                    if (check((p[l].x + p[u].x) / 2 + eps, (p[l].y + p[u].y) / 2 + eps) & 1) { ans++; }
                } else {
                    if (check((p[l].x + p[u].x) / 2 - eps, (p[l].y + p[u].y) / 2 + eps) & 1) { ans++; }
                }
            } else if (p[l].x == p[u].x) {
                if (p[r].x < p[u].x) {
                    if (check((p[r].x + p[u].x) / 2 + eps, (p[r].y + p[u].y) / 2 + eps) & 1) { ans++; }
                } else {
                    if (check((p[r].x + p[u].x) / 2 - eps, (p[r].y + p[u].y) / 2 + eps) & 1) { ans++; }
                }
