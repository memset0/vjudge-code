...
void run() {
    ...
    for (auto x : set) {
        ...
        vec.push_back({vec.back().first + 1, n});
        for (int t = has[x], i = 0; t; t--) {
            for (; vec[i].second == 0; i++);
            if (i > 0 && vec[i].first > vec[i - 1].first + 1) {
                vec[i].first = vec[i - 1].first + 1; vec[i].second = n;
            }
            sum -= vec[i].first; vec[i].second--;
        }
    }
    out << ((sum % 2 == 0) ? "FuuFuu" : "Pico") << "\n";
}
