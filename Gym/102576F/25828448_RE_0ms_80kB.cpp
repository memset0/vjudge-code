...
int main(){
	...
	for(read(T);T--;){
		...
		for(int i=0;i<=m;i++)sum[i]=(i?sum[i-1]:0)+vet[i].first;
		for(int i=0;i<=m;i++)cnt[i]=(i?cnt[i-1]:0)+vet[i].second;
		bool fl=false;
		for(int i=0;i<=m;i++){ // 0...i 直接走 i+1...m 刷新
			x=frac{sum[i],c}.filter()-fac[n]-c*cnt[i];
			if(vet[i].first<=x+frac{c}){fl=true;break;}
		}
		assert(fl);
