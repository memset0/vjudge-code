...
template<const int N> struct matrix{
  ...
  inline pair<int,int> find()const{
    for(int i=0;i<N;i++)
      for(int j=0;j<N;j++)
        if(x[i][j]){
          return {i,j};
...
bool check(const matrix<N> &a,const matrix<N> &b,const matrix<N> &c,int type){
  pair<int,int> x=a.find()-b.find();
  d.clear();
  for(int i=0;i<a.n;i++)
    for(int j=0;j<a.m;j++){
      d.x[(N<<1)+i][(N<<1)+j]^=a.x[i][j];
    }
  for(int i=0;i<b.n;i++)
    for(int j=0;j<b.m;j++){
      d.x[(N<<1)+i+x.first][(N<<1)+j+x.second]^=b.x[i][j];
    }
  pair<int,int> y=d.find()-make_pair(N<<1,N<<1)-c.find();
  for(int i=0;i<c.n;i++)
    for(int j=0;j<c.m;j++){
      d.x[(N<<1)+i+y.first][(N<<1)+j+y.second]^=c.x[i][j];
    }
  ...
  println("YES");
  if(type==0){
    println(x.second,' ',x.first);
  }else if(type==1){
    println(-y.second,' ',-y.first);
  }else if(type==2){
    println(y.second-x.second,' ',y.first-x.first);
  }
  return true;
}
...
int main(){
  ...
  b.input(),y=b.find();
  c.input(),z=c.find();
  if(check(a,b,c,0))return 0;
  if(check(b,c,a,1))return 0;
  if(check(c,a,b,2))return 0;
  println("NO");
}