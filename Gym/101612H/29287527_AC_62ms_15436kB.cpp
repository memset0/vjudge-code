...
const int N=1e5+10;
int n,m,fa[N],vis[N];
vector<int> rt,vec[N],G[N];

inline int pop(vector<int> &v){
  int x=v.back();
  return v.pop_back(),x;
}
...
void dfs(int u){
  for(int v:G[u]){
    dfs(v);
    if(!vis[u]&&!vis[v])vis[u]=vis[v]=true;
  }
}
...
int main(){
  ...
  sort(rt.begin(),rt.end(),[&](int a,int b){
    return vec[a].size()>vec[b].size();
  });
  for(int u:rt){
    if(vec[1].size()){
      link(pop(vec[1]),pop(vec[u]));
    }
    vec[1].concat(vec[u]);
  }
  print((n-vec[1].size())>>1,'\n');
  for(int i=2;i<=n;i++)print(fa[i]?fa[i]:1," \n"[i==n]);
}//dfasdfsadf