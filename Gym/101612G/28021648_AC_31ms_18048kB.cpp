...
const int N=2e5+9;
int T,flag,n,m,s,cnt,a[N],b[N],f[N],fa[N],dfn[N];
vector<int> ans[3],G[N];

void solve(vector<int> &a,int x,int y){
  while(y!=fa[x])a.push_back(y),y=fa[y];
}
...
void work(int x,int y){
  ...
  int l=0,r=0;
  while(y!=x){
    if(f[y]&&(!l||f[y]==f[l]))r=fa[y],l=l?l:y,y=fa[y];
    else f[y]=s,y=fa[y];
  }
  if(l){
...
int main(){
  ...
  while(T--){
    ...
    for(int i=1;i<=n&&!flag;++i)if(!dfn[i])dfs(i);
    if(!flag){
      println("-1");
    }
  }
