...
int T,n,a[N],b[N],d[N],dp[N][N];
vector<int> tmp;
int DP(int l,int r){
  if(dp[l][r]!=inf)return dp[l][r];
  int id=-1,max=-1;
  for(int i=1;i<=n;i++)if(l<=a[i]&&b[i]<=r&&d[i]>max)max=d[i],id=i;
  if(id==-1)return dp[l][r]=0;
  for(int i=a[id];i<=b[id];i++)dp[l][r]=min(dp[l][r],DP(l,i-1)+DP(i+1,r)+d[id]);
  // printf("dp[%d][%d]=%d {%d}\n",l,r,dp[l][r],id);
  return dp[l][r]==inf?++dp[l][r]:dp[l][r];
}
int main(){
#ifdef memset0
