...
namespace quad_residue{
  int a,b,s,d;
  std::mt19937 rng;
  struct vec2{int r,i;} x,y;
  inline vec2 cross(const vec2 &a,const vec2 &b,int mod){
    return {(int)(((long long)a.r*b.r+(long long)a.i*b.i%mod*d)%mod),(int)(((long long)a.r*b.i+(long long)a.i*b.r)%mod)};
  }
  inline int legendre(int a,int mod){
    for(b=(mod-1)>>1,s=1;b;b>>=1,a=(long long)a*a%mod)if(b&1)s=(long long)s*a%mod;
    return s==mod-1?-1:s;
  }
  inline int cipolla(int n,int mod){
    do a=rng()%mod; while(legendre((d=((long long)a*a-n+mod)%mod,d),mod)!=-1);
    x={a,1},y={1,0};
    for(b=(mod+1)>>1;b;b>>=1,x=cross(x,x,mod))if(b&1)y=cross(y,x,mod);
...
constexpr int N=36,L=1e5+9,K=10;
constexpr int pri[K]={
  922702951,
  914296711,
  930243719,
...
struct bigint{
  ...
  inline int operator%(int mod)const{
    int res=0;
    for(int i=0;i<len;i++)res=(res*10+a[i])%mod;
    return res;
  }
}a[N];
...
struct pack{
  int a,b,c;
  template<int p> static inline int dec(int a,int b){return (a-=b)<0?a+p:a;}
  template<int p> static inline int inc(int a,int b){return (a+=b)>=p?a-p:a;}
  friend inline pack operator+(const pack &a,const pack &b){
    return {inc<pri[0]>(a.a,b.a),inc<pri[1]>(a.b,b.b),inc<pri[2]>(a.c,b.c)};
  }
  friend inline pack operator-(const pack &a,const pack &b){
    return {dec<pri[0]>(a.a,b.a),dec<pri[1]>(a.b,b.b),dec<pri[2]>(a.c,b.c)};
  }
  inline bool operator<(const pack &rhs)const{
...
//    }
    ...
    for(int i=0;i<3;i++)
      for(int j=0;j<idx.size();j++){
        b[i][j]=cipolla(rst[idx[j]][i],pri[i]);
//        fprintf(stderr,"%d %d >> %d %d %d\n",idx[j],i,rst[idx[j]][i],pri[i],b[i][j]);
      }
    for(int i=0;i<idx.size();i++){
      c[i]={b[0][i],0,0};
//      c[i]={b[0][i],b[1][i],b[2][i]};
