...
namespace mem{ //v2.8.0 => size: 15.00KiB
  #ifdef memset0
  #else
    #define MEM_FASTIO
  #endif
...
int ml[N<<2],ad[N<<2];
inline void pushtag(int x,int a,int b){ml[x]*=a,ad[x]=ad[x]*a+b;}
inline void pushdown(int x,int l,int r){
	if(l^r){
		pushtag(x<<1,ml[x],ad[x]);
		pushtag(x<<1|1,ml[x],ad[x]);
		ml[x]=1,ad[x]=0;
	}
}
inline void Mul(int x,int l,int r,int ql,int qr,int s){
	if(ql>qr)return;
	pushdown(x,l,r);
	if(ql<=l&&qr>=r){
		pushtag(x,s,0);
	...
	int mid=(l+r)>>1;
	if(ql<=mid)Mul(x<<1,l,mid,ql,qr,s);
	if(qr>mid)Mul(x<<1|1,mid+1,r,ql,qr,s);
}
...
int top=1000002;
vector<int> flow[N];
vector<pair<int,int>> cow[N],op2[N];
vector<pair<pair<int,int>,int>> op1[N];
int n,m;
int ans[N],prefetch[N];
int main(){
	read(m);
  fences.insert(10);
  for(int x1,y1,x2,y2,i=1;i<=m;i++){
    ...
    op1[x2+1].push_back(make_pair(make_pair(y1,y2),i));
    op1[x1].push_back(make_pair(make_pair(y1,y2),-i));
  }
	read(m);
  ...
  for(int x,y,i=1;i<=n;i++){
		read(x,y);
    cow[x].push_back(make_pair(y,i));
  }
  for(int i=top;i;i--){
    for(auto v:flow[i]){
      int h=v;
      h=-*fences.lower_bound(-h);
      h=max(h,1);
      Add(1,1,top,h,v,1);
    }
    for(auto v:cow[i]){
      ans[v.second]=query(1,1,top,v.first);
    }
    for(auto v:op1[i]){
      int L=v.first.first,R=v.first.second;
      if(v.second>0){
        fences.insert(-L);
        fences.insert(-(R+1));
        Mul(1,1,top,L,R,0);
        prefetch[v.second]=query(1,1,top,R+1);
      }
    }
    for(auto v:op1[i]){
      int L=v.first.first,R=v.first.second;
      if(v.second<0){
        assert(fences.find(-L)!=fences.end());
        assert(fences.find(-R-1)!=fences.end());
        fences.erase(-L);
        fences.erase(-(R+1));
        Mul(1,1,top,L,R,0);
        int _v2=query(1,1,top,R+1);
        int _v=prefetch[-v.second];
        int H=-*fences.lower_bound(-L);
        H=max(H,1);
        Add(1,1,top,H,R,_v2);
        Add(1,1,top,H,L-1,-_v);
      }
    }
  }
