...
namespace mem{ //v2.9.4 => size: 16.02KiB
  #ifdef memset0
  #else
    #define MEM_FASTIO
  #endif
...
void modify(int u,int l,int r,int x){
  if(p[u].l==l&&p[u].r==r)return p[u].tag^=x,void();
...
void update(int u,int k,int x){
  ...
  update(k<=p[u].mid?u<<1:u<<1|1,k,x);
}

int query(int u,int l,int r,int x){
  x^=p[u].tag;
  if(p[u].l==l&&p[u].r==r)return p[u].set[x];
  if(r<=p[u].mid)return query(u<<1,l,r,x);
  if(l>p[u].mid)return query(u<<1|1,l,r,x);
  return query(u<<1,l,p[u].mid,x)+query(u<<1|1,p[u].mid+1,r,x);
}

int main(){
  judge_file_io("hack"),local_file_input("1.in");
  read(n);
  for(int i=1;i<=n;i++){
    read(a[i]);
  }
  ...
  for(int i=1;i<=n;i++){
    for(auto &it:lis)it.first&=a[i];
    for(int i=0;i<lis.size();i++)if(i+1==lis.size()||lis[i].first!=lis[i+1].first)tmp.push_back(lis[i]);
    lis.swap(tmp),tmp.clear();
    int lst=1;
    if(i>1)modify(1,1,i-1,a[i]);
    for(const auto &it:lis){
      // log("<%d> [%d %d] and=%d cnt=%d\n",i,lst,it.second,it.first,query(1,lst,it.second,it.first));
      ans+=query(1,lst,it.second,it.first);
      lst=it.second+1;
    }
    update(1,i,a[i]);
    lis.push_back({a[i],i});
