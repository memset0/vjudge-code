...
int T,n,m,fa[N],col[N],rnk[N],lca[N][N],dp[N][N],status[1<<10],trans[10][10],last[10];
...
void solve(int u){
  ...
  for(int x=0;x<(1<<lim);x++)if(status[x]>=0){
    for(int i=0;i<lim;i++)if(!(x>>i&1)){
      last[i]=max(last[i],status[x]);
    }
    dp[u][0]=max(dp[u][0],status[x]);
  ...
  for(int x:direct[u]){
    // log("%d->%d:%d:%d\n",u,x,dp[col[x]][x],last[rnk[col[x]]]);
    dp[u][0]=max(dp[u][0],dp[col[x]][x]+last[rnk[col[x]]]+1);
  }
