...
int T,n,m,fa[N],col[N],rnk[N],lca[N][N],dp[N][N],status[1<<10],trans[10][10],at[10],last[10];
vector<int> G[N],child[N],direct[N];
...
void dfs(int u){
  ...
  for(int v:G[u])if(v!=fa[u]){
    fa[v]=u;
    dfs(v);
    child[u].concat(child[v]);
  }
}
void solve(int u){
  for(int v:G[u])if(v!=fa[u])solve(v);
  fill(dp[u],dp[u]+n+1,-inf);
  int lim=G[u].size()-(u!=1),cnt=0;
  for(int v:G[u])if(v!=fa[u]){
    // log<GREEN>("%d->%d\n",u,v);
    rnk[v]=cnt,at[cnt]=v,cnt++;
    for(int t:child[v])col[t]=v;
  }
  for(int x:direct[u]){
    dp[col[x]][0]=max(dp[col[x]][0],dp[col[x]][x]+1);
  }
  for(int i=0;i<lim;i++)fill(trans[i],trans[i]+lim,-inf);
  for(const auto &it:parade[u]){
    int w=dp[col[it.first]][it.first]+dp[col[it.second]][it.second]+1;
    int &f=trans[min(rnk[col[it.first]],rnk[col[it.second]])][max(rnk[col[it.first]],rnk[col[it.second]])];
    // log<YELLOW>("[%d] %d %d w=%d f=%d i=%d j=%d\n",u,it.first,it.second,w,f,rnk[col[it.first]],rnk[col[it.second]]);
    f=max(f,w);
  }
  fill(status,status+(1<<lim),-inf);
  status[0]=0;
  for(int x=0;x<(1<<lim);x++)if(status[x]>=0){
    for(int i=0;i<lim;i++)if(!(x>>i&1))
      for(int j=i+1;j<lim;j++)if(!(x>>j&1)){
        status[x|1<<i|1<<j]=max(status[x|1<<i|1<<j],status[x]+trans[i][j]);
      }
  }
  fill(last,last+lim,-inf);
  for(int x=0;x<(1<<lim);x++)if(status[x]>=0){
    int s=0;
    for(int i=0;i<lim;i++)if(!(x>>i&1)){
      s+=dp[at[i]][0];
    ...
    for(int i=0;i<lim;i++)if(!(x>>i&1)){
      last[i]=max(last[i],status[x]+s-dp[at[i]][0]);
    }
    dp[u][0]=max(dp[u][0],status[x]+s);
    // if(status[x]>=0)log("STATUS[%d][%d]=%d\n",u,x,status[x]);
  }
  // for(int i=0;i<=n;i++)if(dp[u][i]>=0)log("DP[%d][%d]=%d\n",u,i,dp[u][i]);
  for(int v:G[u])if(v!=fa[u])
    for(int i:child[v]){
      dp[u][i]=max(dp[u][i],dp[v][i]+last[rnk[v]]);
    }
  dp[u][u]=max(dp[u][u],dp[u][0]);
  // log("dp[%d][0]=%d|%d\n",u,dp[u][0],child[u].size());
  // for(int i=0;i<=n;i++)if(dp[u][i]>=0)log("DP[%d][%d]=%d\n",u,i,dp[u][i]);
}
int main(){
  ...
  while(T--){
    ...
    dfs(1);
    for(int u=1;u<=n;u++){
      for(int i:G[u])if(i!=fa[u])
        for(int j:G[u])if(j!=fa[u]&&i<j)
          for(int x:child[i])
            for(int y:child[j]){
              lca[x][y]=lca[y][x]=u;
            }
    ...
    // for(int i=1;i<=n;i++){
    //   log<GREEN>("child[%d]",i),log(" = ");
    //   for(int x:child[i])log("%d ",x);log("\n");
    // }
    read(m);
    for(int u,v,i=1;i<=m;i++){
      read(u,v);
      // log("lca(%d,%d)=%d\n",u,v,lca[u][v]);
      if(u==lca[u][v]||v==lca[u][v]){
        direct[lca[u][v]].push_back(u^v^lca[u][v]);
      }else{
        parade[lca[u][v]].push_back({u,v});
      }
    }
    solve(1);
    println(dp[1][0]);
