...
const int N=55;
int n,a[3][N];
vector<tuple<int,bool,int,bool>> ans;

inline void add(int i,bool x,int j,bool y){
  ans.push_back(make_tuple(i,x,j,y));
}

void out(string prefix,const vector<int> &p){
#ifdef memset0
  print(prefix," => ");
  for(int x:p)print(x,' ');
  print('\n');
...
int main(){
  ...
  for(int u=0;u<3;u++){
    for(int i:p[u]){
      ...
      add(p[u].front(),a[u][p[u].front()],i,a[u][i]);
      add(p[u].front(),a[u][p[u].front()]^1,i,a[u][i]^1);
    }
  }
  for(int i=0,j=1;i<3;i++,j=(j+1)%3){
    if(p[i].size()&&p[j].size()){
      int x=p[i].front(),y=p[j].front();
      add(x,a[i][x],y,a[j][y]^1);
      add(y,a[j][y],x,a[i][x]^1);
    }
  }
