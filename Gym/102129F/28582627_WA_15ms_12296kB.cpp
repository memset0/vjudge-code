...
using namespace mem;
const int N=1e5+9;
int n,m,val[N],mute[N<<2];
int pos,siz[N],fa[N],dep[N],dfn[N],idfn[N],top[N],son[N];
vector<int> ans,vec[N];
vector<pair<int,int>> T[N];
struct edge{
  int o,u,v,w;
  inline bool operator<(const edge &rhs)const{
    return w==rhs.w?o>rhs.o:w>rhs.w;
  }
}e[N];
struct dsu{
  int anc[N];
  dsu(){for(int i=0;i<N;i++)anc[i]=i;}
  inline int find(int x){return x==anc[x]?x:anc[x]=find(anc[x]);}
  inline void merge(int u,int v){anc[find(v)]=find(u);}
}a,b;
void dfs(int u){
  siz[u]=1;
  for(const auto &it:T[u]){
...
void modify(int u,int ql,int qr,int l,int r){
  ...
  int mid=(l+r)>>1;
  if(qr<=mid)modify(u<<1,ql,qr,l,mid);
  else if(ql>mid)modify(u<<1|1,ql,qr,mid+1,r);
  else modify(u<<1,ql,mid,l,mid),modify(u<<1|1,mid+1,qr,mid+1,r);
}
void scan(int u,int l,int r){
  if(mute[u])return;
  if(l==r){
    ans.push_back(idfn[l]);
    return;
  }
  int mid=(l+r)>>1;
  scan(u<<1,l,mid);
  ...
  for(int i=0;i<m;i++)
    if(e[i].o){
      ...
      if(b.find(e[i].u)!=b.find(e[i].v)){
        vec[b.find(e[i].u)].concat(vec[b.find(e[i].v)]);
        b.merge(e[i].u,e[i].v);
      }
    }else{
      a.merge(e[i].u,e[i].v);
      if(b.find(e[i].u)!=b.find(e[i].v)){
        ...
        auto modifySubtree=[&](int u){
          if(u==t){
            modify(1,1,dfn[u],1,n);
            if(dfn[u]+siz[u]<=n)modify(1,dfn[u]+siz[u],n,1,n);
          }else{
            modify(1,dfn[u],dfn[u]+siz[u]-1,1,n);
          }
        };
        modifySubtree(e[i].u);
        modifySubtree(e[i].v);
