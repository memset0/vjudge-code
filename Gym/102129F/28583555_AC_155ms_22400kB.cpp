...
struct edge{
  int o,u,v,w;
  inline bool operator<(const edge &rhs)const{return w==rhs.w?o>rhs.o:w>rhs.w;}
  inline void destory(){
    log("destory !!\n");
    for(int x:vec[b.find(u)])vis[x]=true;
    for(int x:vec[b.find(v)])vis[x]=true;
    vec[b.find(u)].clear();
    vec[b.find(v)].clear();
  }
}e[N];
...
int main(){
  ...
  for(auto it=p.rbegin();it!=p.rend();it++){
    log("w=%d\n",it->first);
    for(int i:it->second){
      log("%d %d %d %d\n",e[i].o,e[i].u,e[i].v,e[i].w);
      if((!e[i].o||a.find(e[i].u)==a.find(e[i].v))&&b.find(e[i].u)!=b.find(e[i].v)){
        e[i].destory();
      }
    }
    for(int i:it->second){
      a.merge(e[i].u,e[i].v);
      if(e[i].o&&b.find(e[i].u)!=b.find(e[i].v)){
        if(vec[b.find(e[i].v)].size()>vec[b.find(e[i].u)].size()){
          swap(e[i].u,e[i].v);
        }
