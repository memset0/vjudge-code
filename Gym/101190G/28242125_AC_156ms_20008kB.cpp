...
const int N=2e5+5;
int n,m,deg[N];
bool vis[N],ans[N],cov[N];
vector<int> G[N],P[N];
int main(){
  judge_file_io("game"),local_file_input("1.in");
  read(n,m);
  for(int u,v,i=1;i<=m;i++){
    read(u,v);
    G[u].push_back(v+n),P[v+n].push_back(u),deg[u]++;
    G[u+n].push_back(v),P[v].push_back(u+n),deg[u+n]++;
  }
  queue<int> q;
  memset(ans,true,sizeof(ans));
  for(int i=1;i<=(n<<1);i++){
    if(!deg[i])q.push(i),ans[i]=cov[i]=true;
  }
  while(q.size()){
    int u=q.front(); q.pop();
    ...
    for(int v:P[u]){
      --deg[v];
      cov[v]|=true;
      ans[v]=min<int>(ans[v],ans[u]^1);
      // log("AAA %d[%d] -> %d[%d]\n",u,ans[u],v,ans[v]);
      if(!vis[v]&&(!deg[v]||v<=n&&!ans[v]))q.push(v);
    }
  }
  for(int i=1;i<=n;i++)if(cov[i])q.push(i);
  while(q.size()){
    int u=q.front(); q.pop();
    if(vis[u])continue; vis[u]=true;
    for(int v:P[u]){
      --deg[v];
      ans[v]=v<=n?ans[v]:min<int>(ans[v],ans[u]^1);
      // log("BBB %d[%d] -> %d[%d]\n",u,ans[u],v,ans[v]);
      if(!vis[v]&&(!deg[v]||v<=n))q.push(v);
    }
  }
  for(int i=n+1;i<=(n<<1);i++)print(vis[i]?"WL"[ans[i]]:'D');
  print('\n');
  for(int i=1;i<=n;i++)print(vis[i]?"WL"[ans[i]]:'D');
  print('\n');
}