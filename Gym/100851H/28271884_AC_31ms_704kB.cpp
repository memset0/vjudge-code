...
int main(){
  judge_file_io("hypercube"),local_file_input("1.in");
  cin>>x>>y>>z;
  for(int k=1;k<=z;k++)
    for(int i=1;i<=y;i++)
      for(int j=1;j<=x;j++){
        char t=getchar();
        while(t!='x'&&t!='.')t=getchar();
        s[i][j][k]=t=='x';
        if(s[i][j][k]&&!q.size()){
          q.push(0);
          a[0]=(dat){i,j,k,{2,3,4,5,6,7}};
        }
      }
  vis[0]=1;
  while(q.size()){
    int u=q.front();
    q.pop();
    s[a[u].x][a[u].y][a[u].z]=0;
    for(int d=0;d<6;d++){
      int nx=a[u].x+dir[d][0],ny=a[u].y+dir[d][1],nz=a[u].z+dir[d][2];
      if(s[nx][ny][nz]){
        int v=a[u].tow[d];
        if(vis[v]){
          println("No");
