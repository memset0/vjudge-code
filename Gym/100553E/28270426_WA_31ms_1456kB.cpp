...
inline char trans(int c){return "RPS"[c];}
inline int trans(char c){return c=='S'?2:c=='P';}

int main(){
  judge_file_io("epic"),local_file_input("1.in");
  read(n);
  for(int i=1;i<=n;i++){
    a[i].cho=trans((char)read_upper());
    read(a[i].nxt[0],a[i].nxt[1],a[i].nxt[2]);
  }
  for(int i=1;i<=n;i++){
    id[(i-1<<2)+1]=i;
    id[(i-1<<2)+2]=i;
  ...
  for(int i=1;i<=(n<<2);i++){
    ...
    vector<int> cir(n+1);
    for(int j=1;j<=(n<<1);j++){
      b[lst].nxt[0]=b[lst].nxt[1]=b[lst].nxt[2]=i%(n<<2)+1;
      b[lst].cho=(a[u].cho+1)%3;
      int v=a[u].nxt[(a[u].cho+1)%3];
      // log("u=%d v=%d i=%d j=%d lst=%d (%d)\n",u,v,i,j,lst,cir[v]);
      if(cir[u]){
        b[lst].nxt[a[u].cho]=cir[v];
        break;
      }else{
        b[lst].nxt[a[u].cho]=++cnt;
        u=v,cir[u]=lst=cnt;
      }
    }
  }
  println(cnt);
  for(int i=1;i<=cnt;i++){
    println(trans(b[i].cho),' ',b[i].nxt[0],' ',b[i].nxt[1],' ',b[i].nxt[2]);
  }
}