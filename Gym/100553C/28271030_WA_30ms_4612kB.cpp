...
graph* build(int l,int r,bool fl){
  ...
  if(s[l]=='r'){
    ...
    if(a>b)swap(a,b);
    graph *u=nullptr,*lst=nullptr;
    for(var[nam-'A']=a;var[nam-'A']<=b;var[nam-'A']++){
      graph *v=build(p[l+1]+2,r,1);
      (u?lst->next:u)=v,lst=v;
    }
    return u;
  }else if(fl){
    graph *u=nullptr,*lst=nullptr;
    ...
    while(k<=r+1)if(s[k]==','||k>r){
      graph *v=build(t,k-1,0);
      t=++k,(u?lst->next:u)=v,lst=v;
    }else{
      k=(s[k]=='('?p[k]:k)+1;
    }
    return u;
    ...
    if(l==r&&s[l]=='c')s[l]='1';
    if(islower(s[l])){
      graph *u=new graph(),*c=build(l+2,r-1,1);
      vector<graph*> vec;
      ...
      graph *lst=newNode(),*src=lst;
      for(int i=1;i<v;i++)lst->next=newNode(),lst=lst->next;
      return src;
    }
  }
}
...
void euler(int u){
  cir[now].push_back(u);
  bool fl=false;
  for(int &i=hed[u];i<G[u].size();i++)
    if(!mrk[G[u][i].second]){
      fl=true;
      mrk[G[u][i].second]=true;
      euler(G[u][i].first);
    }
  if(!fl){
    // assert(G[u].size()&1);
