...
namespace mem{ //v2.9.3 => size: 15.91KiB
  #ifdef memset0
  #else
    #define MEM_FASTIO
  #endif
...
int n,cnt,tmp,now,p[N],anc[M],id[M],hed[N];
bool vis[N],tag[N],mrk[M],slv[N];
string s;
queue<int> que;
vector<int> cir[N>>1];
vector<vector<int>> tir[N];
vector<pair<int,int>> edg,G[N];
...
inline int parse(int l,int r){return l==r&&isupper(s[l])?var[s[l]-'A']:atoi(s.slice(l,r+1).c_str());}

inline graph* newNode(){
  graph *res=new graph();
  res->s=++cnt,anc[cnt]=cnt;
  res->t=++cnt,anc[cnt]=cnt;
  edg.push_back({res->s,res->t});
  return res;
}

inline int count(graph *u){
  int cnt=0;
  while(u)++cnt,u=u->next;
  return cnt;
}

typedef pair<graph*,graph*> list;
inline list twice(graph *r){return make_pair(r,r);}
inline void concat(list &a,const list &b){
  if(a.first){
...
list build(int l,int r,bool fl){
  log<PURPLE>("build %d %d %d >> %s\n",l,r,fl,s.slice(l,r+1).c_str());
  if(l>r){
    return twice(newNode());
  }
  list res=make_pair(nullptr,nullptr);
  if(s[l]=='r'){
    int t=-1,nam=s[l+2];
    for(int i=l+4;i<=p[l+1];i++)if(s[i]==',')t=i;
    assert(~t);
    int a=parse(l+4,t-1),b=parse(t+1,p[l+1]-1);
    log("a=%d b=%d\n",a,b);
    if(a>b)swap(a,b);
    for(var[nam-'A']=a;var[nam-'A']<=b;var[nam-'A']++){
      concat(res,build(p[l+1]+2,r,1));
    }
  }else if(isupper(s[l])||isdigit(s[l])){
    int t=l;
    while(s[t]!=','&&t<=r)t++;
    int v=parse(l,t-1);
    log("v=%d t=%d [%d %d]\n",v,t,t+1,r);
    for(int i=0;i<v;i++){
      concat(res,build(t+1,r,1));
    }
  }else if(fl){
    int t=l,k=l;
    while(k<=r+1)if(s[k]==','||k>r){
      concat(res,build(t,k-1,0));
      t=++k;
    }else{
      k=(s[k]=='('?p[k]:k)+1;
    }
    ...
    if(l==r&&s[l]=='c'){
      ...
      graph *u=new graph(),*c=build(l+2,r-1,1).first;
      vector<graph*> vec;
      while(c)vec.push_back(c),c=c->next;
      log("%c(%d)\n",s[l],(int)vec.size());
      if(s[l]=='c'){
        for(int i=1;i<vec.size();i++)merge(vec[i-1]->t,vec[i]->s);
        u->s=vec.front()->s,u->t=vec.back()->t;
      }else if(s[l]=='l'){
        assert(vec.size()>1);
        for(int i=0;i<vec.size();i++)merge(vec[i]->t,vec[(i+1)%vec.size()]->s);
        u->s=vec[0]->s,u->t=vec[0]->t;
      }else{
        for(int i=1;i<vec.size();i++)merge(vec[0]->s,vec[i]->s);
        u->s=vec[0]->s,u->t=vec[0]->t;
      }
      res=twice(u);
    }
  }
  log<GREEN>("endbd %d %d %d >> [%d] %lu %lu\n",l,r,fl,count(res.first),res.first,res.second);
  return res;
}
...
int main(){
  judge_file_io("cactus"),local_file_input("1.in");
  read(s);
  string tem;
  for(int i=0;i<s.length();i++){
    if(islower(s[i])&&s[i]!='c'&&s[i]!='t'&&s[i]!='l'&&s[i]!='r')continue;
    tem.push_back(s[i]);
  }
  s.swap(tem);
  vector<int> stk;
  for(int i=0;i<s.length();i++)if(s[i]=='('){
    ...
    p[stk.back()]=i,p[i]=stk.back();
    stk.pop_back();
  }
  graph* g=build(0,(int)s.length()-1,1).first;
  id[find(g->s)]=n=1;
  for(int i=1;i<=cnt;i++)if(find(i)==i&&i!=find(g->s)&&i!=find(g->t))id[i]=++n;
  id[find(g->t)]=++n;
  int tmp=0;
  for(auto &it:edg){
    int u,v; tie(u,v)=it;
  ...
  while(que.size()){
    ...
    while(true){
      cir[now].clear();
      euler(i);
      if(cir[now].size()==1)break;
      log<RED>(">> ");for(int x:cir[now])log("%d ",x);log("\n");
      tir[i].push_back(cir[now]);
    }
  }
  --now;
  vector<int> res;
  function<void(int)> add=[&](int u){
    if(tir[u].size()&&!slv[u]){
      slv[u]=true;
      while(tir[u].size()){
        const vector<int> &tmp=tir[u].back();
        for(int i=0;i+1<tmp.size();i++)add(tmp[i]);
        tir[u].pop_back();
      }
      res.push_back(u);
    }else{
  ...
  for(int i=1;i<=now;i++){
    res.clear();
    for(int u:cir[i])add(u);
    cir[i].swap(res);
  }
  println(n,' ',now);
  for(int i=1;i<=now;i++){
    print((int)cir[i].size(),' ');
    for(int j=0;j<cir[i].size();j++)print(cir[i][j]," \n"[j+1==cir[i].size()]);
  }
}
