...
const int N=1e5+5;
int n,m,id,cnt,dfn[N],in[N],out[N],rev[N],fa[N],bel[N],tfa[N];
bool vis[N];
vector<int> path,nd[N],e[N],t[N];
pair<int,int> E[N];

inline int find(int x){return x==fa[x]?x:fa[x]=find(fa[x]);}
inline int tfind(int x){return x==tfa[x]?x:tfa[x]=tfind(tfa[x]);}

inline void pushup(int x){
  if(!vis[x]){
    dfn[x]=++id,vis[x]=true;
    for(auto v:e[x])pushup(v);
  }
}

void dfs(int x){
  if(vis[x]){
    if(x==1&&path.size()==cnt){
      for(auto it:path) for(int x:nd[it])print(x,' ');
      printf("1\n");
      exit(0);
    }
    return;
  }
  vis[x]=true,path.push_back(x);
  for(auto v:t[x]) dfs(v);
  vis[x]=false,path.pop_back();
}
int main(){
  judge_file_io("king"),local_file_input("1.in");
  read(n,m);
  for(int i=1;i<=n;i++)fa[i]=tfa[i]=i;
  for(int x,y,i=1;i<=m;++i){
    read(x,y),E[i]={x,y};
    rev[y]=x;
    ++in[y],++out[x],e[x].push_back(y);
    tfa[tfind(x)]=tfind(y);
  }
  int tem=0;
  for(int i=1;i<=n;++i) if(tfa[i]==i) ++tem;
  if(tem!=1){
    println("There is no route, Karl!");
    return 0;
  }
  for(int i=2;i<=n;i++)
    if(in[i]==1&&out[i]<=1&&in[rev[i]]==1&&out[rev[i]]==1){
      fa[find(i)]=find(rev[i]);
    }
  for(int i=1;i<=n;i++){
  ...
  for(int i=1;i<=n;i++){
    bel[i]=bel[find(i)];
    nd[bel[i]].push_back(i);
  }
  for(int i=1;i<=n;i++)if(!vis[i])pushup(i);
  memset(vis,0,sizeof(vis));
  for(int x,y,i=1;i<=m;++i){
    tie(x,y)=E[i];
    if(bel[x]!=bel[y])t[bel[x]].push_back(bel[y]);
  }
  for(int i=1;i<=cnt;++i){
    nd[i].sort([&](int a,int b)->bool{return dfn[a]<dfn[b];});
    t[i].sort();
    t[i].erase(unique(t[i].begin(),t[i].end()),t[i].end());
  }
  if(cnt==1){
    for(auto x:nd[1])print(x,' ');
    println(1);
  }else{
    dfs(1);
    println("There is no route, Karl!");
