...
const int N = 1e5 + 5;
constexpr __int128_t INF = (__int128_t) (1ll << 62) * (1ll << 62);
...
bool check(ll mid) {
    for (int i = 0; i < n; i++) {
        c[i] = (__int128_t) a[i] * mid;
    }
    __int128_t need = 0;
    ll sup = 0;
    for (int i = 86; i >= 0; i--) { d[i] = b[i]; }
    for (int i = 86; i >= 0; i--) {
        std::sort(c, c + n, std::greater<__int128_t>());
        need *= 2;
        for (int j = 0; j < n; j++) {
            if (c[j] >> i & 1) {
                if (sup) { sup--; c[j] = 0; }
                else if (d[i]) { d[i]--; c[j] ^= ((__int128_t) 1) << i; }
                else { need++; c[j] ^= ((__int128_t) 1) << i; }
            }
        }
        if (need > d[i]) { need -= d[i]; d[i] = 0; }
        else { d[i] -= need; need = 0; }
        sup += d[i];
        if (need > INF) { return false; }
    }
    return need == 0;
}
...
void run() {
    ...
    ll ans = 0;
    for (ll l = 1, r = 1048575000000000ll, mid; l <= r; ) {
        mid = l + r >> 1;
        if (check(mid)) {
            l = mid + 1; ans = mid;
        } else {
