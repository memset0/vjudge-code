...
void run() {
    int n;
    in >> n;
    std::vector<std::array<bool, 3>> a(n * 10 + 10);
    for (int i = 0; i < n * 10 + 10; i++) {
        a[i][0] = a[i][1] = a[i][2] = i < n;
    }
    for (int i = 0; i < n * 3 - 3; i++) {
        int p, q, u, v;
        in >> p >> u >> v; p--; u--; v--;
        q = (n * 3 + i) / 3;
        a[p][u] = false; a[q][v] = true;
        if ((a[p + 1][0] || a[p + 1][1] || a[p + 1][2]) && !(a[p][1] || (a[p][0] && a[p][2]))) {
            out << ((i & 1) ? "Sheauhaw" : "Nocriz") << "\n"; return;
        }
        if ((a[q][0] || a[q][1] || a[q][2]) && !(a[q - 1][1] || (a[q - 1][0] && a[q - 1][2]))) {
            out << ((i & 1) ? "Sheauhaw" : "Nocriz") << "\n"; return;
        }
        if (i == n * 3 - 3 - 1) {
            i++;
