...
struct Atom{
  char a[7][21];
  inline char* operator[](const size_t i){return a[i];}
  inline const char* operator[](const size_t i)const{return a[i];}

  template<class T> inline void source(T s){
    for(int i=0;i<7;i++){
      for(int j=0;j<21;j++){
        a[i][j]=s[i][j]=='X';
      }
...
}a[N],ans,tim[1440],bas;

inline Atom check(const Atom &c,const Atom &u){
  Atom res;
  for(int i=0;i<7;i++){
    for(int j=0;j<21;j++){
      res[i][j]=c[i][j]==u[i][j]?2:c[i][j];
    }
  }
  return res;
}

inline pair<Atom,bool> operator&&(const Atom &a,const Atom &b){
  Atom c;
  for(int i=0;i<7;i++)
    for(int j=0;j<21;j++)
      if(a[i][j]<2&&b[i][j]<2){
...
inline Atom operator||(const Atom &a,const Atom &b){
  ...
  for(int i=0;i<7;i++)
    for(int j=0;j<21;j++)
      if(a[i][j]==3||b[i][j]==3||a[i][j]!=b[i][j]){
        c[i][j]=3;
      }else{
        c[i][j]=a[i][j];
      }
...
int main(){
  ...
  for(int i=0;i<24;i++){
    for(int j=0;j<60;j++){
      Time s={i,j};
      bool fl=true,tem;
      Atom cur=check(tim[s.h*60+s.m],a[0]);
      for(int i=1;i<n&&fl;i++){
        s.inc();
        tie(cur,tem)=cur&&check(tim[s.h*60+s.m],a[i]),fl&=tem;
      }
      if(fl){
        printf("> %d %d\n",i,j);
        ans=ans||cur;
