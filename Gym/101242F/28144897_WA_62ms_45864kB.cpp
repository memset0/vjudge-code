...
const int N=5e5+10;
int n,m,pos,w[N],f[N],fa[N],id[N],dep[N],dfn[N],siz[N],ans[N];
string name[N];
vector<int> G[N];

namespace bit{
  int s[N];
  inline void add(int k,int x){for(++k;k<N;k+=k&-k)s[k]+=x;}
  inline int ask(int k){int r=0;for(++k;k;k-=k&-k)r+=s[k];return r;}
}

struct q_comparer{
  inline bool operator()(int x,int y)const{return f[x]>f[y];}
};
priority_queue<int,vector<int>,q_comparer> q;
...
int main(){
...
#endif
  read(n,m);
  for(int i=1;i<=n;i++)read(name[i],fa[i],w[i]);
  for(int i=1;i<=m;i++)read(fa[n+i],w[n+i]);
  for(int i=1;i<=n+m;i++){
    if(fa[i])fa[i]+=n;
    G[fa[i]].push_back(i);
    // log("%d %d %d\n",fa[i],i,w[i]);
  ...
  // for(int i=1;i<=n+m;i++)log<RED>("%d%c",f[i]," \n"[i==n+m]);
  // for(int i=1;i<=n+m;i++)log<RED>("%d%c",dep[i]," \n"[i==n+m]);
  for(int i=1;i<=n;i++){
    q.push(i);
    bit::add(dfn[i],1);
  }
  for(int i=1;i<=n;i++){
    function<void(int)> check=[&](int u){
      if(!u)return;
      if(bit::ask(dfn[u]+siz[u]-1)-bit::ask(dfn[u]-1)==0){
        if(f[u]>dep[id[i]]){
          bit::add(dfn[u],1);
          q.push(u);
        }else{
    ...
    // log<GREEN>("%d %d\n",id[i],dep[id[i]]);
    while(q.size()&&f[q.top()]<=dep[id[i]]){
      int u=q.top(); q.pop();
      bit::add(dfn[u],-1),check(fa[u]);
    }
    ans[id[i]]=q.size()+1;
  }
  for(int i=1;i<=n;i++)println(name[i],' ',ans[i]);
}