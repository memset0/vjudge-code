...
inline long long lim(int i){return (1ll*(c[i]+1)*s+a[i]-1)/a[i];}

int main(){
  local_file_input("1.in");
  read(n,k),src=k;
  for(int i=1;i<=n;i++)read(a[i]),s+=a[i];
  for(int i=1;i<=k;i++)read(b[i]),c[b[i]]++;
  for(int i=1;i<=n;i++)q.push({lim(i),i});
  while(q.size()&&k<=300000){
    int t=q.top().t,u=q.top().u; q.pop();
    ++c[u],++k;
    q.push({lim(u),u});
    if(t<k){
      println(k-src-2);
