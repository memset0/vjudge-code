...
int main(){
	...
	for(read(T);T--;){
		...
		for(int i=1;i<=n;i++){
			...
			log("insert %d\n",i); for(int i=1;i<=n;i++)log("%lld%c",a[i]," \n"[i==n]); log(">> %lld\n",ans);
			log("u=%d | %d %d\n",u,inc[u].size(),dec[u]);
			while(inc[u].size()>=dec[u]){
				long long delta=std::min(inc[u].top()-mov[u],lst[u]);
				log("delta %lld : min %lld %lld\n",delta,inc[u].top()-mov[u],lst[u]);
		...
		print(ans,'\n');
		for(int i=1;i<=n;i++)while(inc[i].size())inc[i].pop();
		memset(dec+1,0,n<<2),memset(pre+1,0,n<<2),memset(mov+1,0,n<<3),memset(lst+1,0,n<<3);
	}
}