...
inline long long cross(int i,int j){
	return b[j].x>a[i].x?(long long)(b[j].x-a[i].x)*(b[j].y-a[i].y):-LLONG_MAX+1;
}
void solve(int al,int ar,int bl,int br){
	while(al<=ar&&a[ar].x>b[br].x)ar--;
	while(bl<=br&&b[bl].x<a[al].x)bl++;
	...
	// for(int i=bl;i<=br;i++)printf("%lld%c",cross(am,i)," \n"[i==br]);
	ans=max(ans,mx);
	solve(al,am-1,bl,bm);
	solve(am+1,ar,bm,br);
}
