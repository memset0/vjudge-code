...
struct segment{
  int l,r,mid,min,submin,val,cnt,right;
}p[N<<2];
...
inline void maintainJ(int u){
  if(p[u<<1].min==p[u<<1|1].min){
    p[u].min=p[u<<1].min;
    p[u].submin=min(p[u<<1].submin,p[u<<1|1].submin);
  }else if(p[u<<1].min<p[u<<1|1].submin){
    p[u].min=p[u<<1].min;
    p[u].submin=min(p[u<<1].submin,p[u<<1|1].min);
  }else{
    p[u].min=p[u<<1|1].min;
    p[u].submin=min(p[u<<1].submin,p[u<<1|1].min);
  }
...
void build(int u,int l,int r){
  ...
  p[u].min=n,p[u].submin=1919810;
  p[u].tag=n,p[u].cnt=r-l+1;
  if(l==r){
    p[u].right=r;
    return;
  }
...
void modify(int u,int l,int r,int x){
  ...
  p[u].k=p[u<<1|1].cnt?p[u<<1|1].k:p[u<<1].k;
}
...
int query(int u,int l,int r,int x){
  ...
  // log("query [%d %d %d] >> %d %d %d\n",p[u].l,p[u].mid,p[u].r,p[u<<1].cnt,x+p[u<<1].cnt-1,p[u<<1].k);
  if(p[u<<1].cnt&&x+p[u<<1].cnt-1>p[u<<1].k){
...
int at(int u,int k){
  if(p[u].l==p[u].r)return p[u].cnt?p[u].k:-1;
  pushdown(u);
...
bool check(int mid){
  ...
  for(int i=1;i<=n;i++){
    ...
    int k=!t?first(1,1,n):(t<n&&first(1,t+1,n)?first(1,t+1,n):n);
    int u=find(1,1,k);
    cho[i]=u;
    if(at(1,u)<i){
