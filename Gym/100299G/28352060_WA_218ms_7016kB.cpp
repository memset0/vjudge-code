...
struct segment{
  int l,r,mid,k,tag,cnt,min;
}p[N<<2];

inline void pushup(int u,int x){p[u].k=min(p[u].k,x),p[u].tag=min(p[u].tag,x);}
inline void pushdown(int u){pushup(u<<1,p[u].tag),pushup(u<<1|1,p[u].tag);}

void build(int u,int l,int r){
  p[u].l=l,p[u].r=r,p[u].mid=(l+r)>>1;
  p[u].k=n,p[u].tag=n,p[u].cnt=r-l+1;
  if(l==r){
    p[u].min=r;
    return;
  }
  ...
  p[u].min=better(p[u<<1].min,p[u<<1|1].min);
}
...
void modify(int u,int l,int r,int x){
  ...
  p[u].k=p[u<<1|1].cnt?p[u<<1|1].k:p[u<<1].cnt;
}

int query(int u,int l,int r,int x){
  if(!p[u].cnt)return 0;
  // log("query %d %d %d %d >> %d\n",u,l,r,x,p[u].l==p[u].r?(x<=p[u].k?l:0):-1);
  if(p[u].l==p[u].r){
    return x<=p[u].k?l:0;
  }
  pushdown(u);
  if(r<=p[u].mid)return query(u<<1,l,r,x);
  if(l>p[u].mid)return query(u<<1|1,l,r,x);
  int tmp=0;
  if(p[u<<1].cnt&&x+p[u<<1].cnt-1>p[u<<1].k)tmp=query(u<<1,l,p[u].mid,x);
  if(!tmp)tmp=query(u<<1|1,p[u].mid+1,r,x+p[u<<1].cnt);
  if(!tmp)tmp=query(u<<1,l,p[u].mid,x);
  return tmp;
}
...
int first(int u,int l,int r){
  if(p[u].l==p[u].r)return p[u].cnt?p[u].l:0;
  if(r<=p[u].mid)return first(u<<1,l,r);
  if(l>p[u].mid)return first(u<<1|1,l,r);
  return p[u<<1].cnt?first(u<<1,l,p[u].mid):first(u<<1|1,p[u].mid+1,r);
}

int at(int u,int k){
  if(p[u].l==p[u].r){
    return p[u].cnt?p[u].k:-1;
  }
  pushdown(u);
  ...
  while(T--){
    ...
    while(sl<=sr){
      ...
      for(int i=1;i<=n;i++){
        ...
        int t=query(1,1,lim,i+1);
        int k=t<n?first(1,t+1,n):0;
        int u=find(1,1,k?k:n);
        cho[i]=u;
        if(at(1,u)<i){
          log<RED>("[mid=%d] fir=%d lim=%d k=%d u=%d\n",mid,first(1,1,n),lim,k,u);
          flag=false;
          break;
        }
        int wav=upper_bound(l+1,l+n+1,r[u])-l-1;
        log<GREEN>("[mid=%d] fir=%d lim=%d k=%d u=%d wav=%d flag=%d\n",mid,first(1,1,n),lim,k,u,wav,flag);
        flip(1,u);
