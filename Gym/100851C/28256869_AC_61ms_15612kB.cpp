...
const int N=5e4+5;
int n,m,top,cnt,fa[N],col[N],tem[N],stk[N],dep[N],siz[N];
bool vis[N];
long long ans[3],dp[N];
vector<int> G[N],T[N],cir[N];
  ...
  for(int v:G[u])if(v!=fa[u]){
    if(vis[v]&&dep[v]<dep[u]){
      ...
      for(int i=dep[v];i<=dep[u];i++){
        cir[cnt].push_back(stk[i]);
        if(i!=dep[v])col[stk[i]]=cnt;
      }
    }
    if(!vis[v]){
      fa[v]=u;
...
void calc2(int u,long long ext){
  dp[u]+=ext;
  for(int v:T[u])if(col[v]){
    calc2(v,0);
  }else{
    calc2(v,dp[u]-dp[v]);
    ans[1]+=siz[v]*(n-siz[v])-1;
  }
}
...
int main(){
  ...
  for(int t,i=1;i<=m;i++){
    read(t);
    for(int j=1;j<=t;j++)read(tem[j]);
    for(int j=1;j<t;j++){
      G[tem[j]].push_back(tem[j+1]);
      G[tem[j+1]].push_back(tem[j]);
    }
  }
  dfs(1);
  calc(1);
  ...
  for(int i=1;i<=cnt;i++){
    ...
    for(int u:cir[i]){
      all+=dp[u];
      log("dp[%d] = %lld\n",u,dp[u]);
      ans[2]-=dp[u]*dp[u]*cir[i].size();
    }
    ans[2]+=all*all*cir[i].size();
    ans[2]-=2*cir[i].size()*cir[i].size();
    for(int x:cir[i])log<BLUE>("%d ",x);
    log(">> "),log<RED>("ans=%lld\n",ans[2]);
  }
  long long all=0;
  for(int i=1;i<=cnt;i++)all+=cir[i].size();
  println(ans[0]*all+ans[1]+(ans[2]>>1));
}