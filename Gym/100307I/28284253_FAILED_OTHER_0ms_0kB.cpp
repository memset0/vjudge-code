...
const int N=1e5+10;
int n,m,k,limL[N],limR[N];
string rsp;

int main(){
  read(n,m);
  for(int i=0;i<=m;i++)limR[i]=n;
  long long all=(long long)(n+1)*(m+1);
  for(k=0;all>1;k++){
    long long l=0,r=0,res=-1,cnt;
    for(int i=0;i<=m;i++){
      r=max(r,limR[i]+(long long)k*i);
    }
    log("l=%lld r=%lld\n",l,r);
    while(l<=r){
      long long mid=(l+r)>>1,now=0;
      for(int i=0;i<=m;i++){
        now+=max(0ll,min<long long>(limR[i],mid-(long long)k*i)-limL[i]+1);
      }
      log("calc(%lld)=%lld\n",mid,now);
      if((now<<1)<=all){
        l=mid+1,res=mid,cnt=now;
      }else{
        r=mid-1;
      }
    }
    log<RED>("all=%lld res=%lld cnt=%lld\n",all,res,cnt);
    println("check ",0,' ',res);
    flush();
    read(rsp);
    all=0;
    for(int i=0;i<=m;i++){
      if(rsp=="Yes"){
        limR[i]=min<long long>(limR[i],res-(long long)k*i);
      }else{
        limL[i]=max<long long>(limL[i],res-(long long)k*i+1);
      }
      all+=max(0,limR[i]-limL[i]+1);
      log("v=%d p=%d~%d\n",i,limL[i],limR[i]);
    }
  }
  for(int i=0;i<=m;i++)if(limL[i]==limR[i]){
    println("answer ",limL[i]+(long long)i*k);
  }
}