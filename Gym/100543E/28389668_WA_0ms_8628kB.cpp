...
inline int highbit(int x){return x?1<<32-__builtin_clz(x):0;}
void solve(int i,int j){
  ...
  int h=max(highbit(j),highbit(s[i]-j));
  return solve(i,(j&(j^h))+(h>>1));
}
int main(){
  ...
  while(T--){
    ...
    dp[0][0]=true;
    for(int i=1;i<=n;i++){
      for(int j=0;j<=s[i];j++)dp[i][j]=false;
      for(int j=0;j<=s[i-1];j++)
        if(dp[i-1][j]){
          if(validate(j,a[i]))dp[i][j+a[i]]=true;
          if(validate(s[i-1]-j,a[i]))dp[i][j]=true;
        }
      for(int h,j=0;j<=s[i];j++)
        if(dp[i][j]&&(h=highbit(j))==highbit(s[i]-j)){
          dp[i][j]=false;
          dp[i][j^h]=dp[i][(j^h)+(h<<1)]=true;
        }
