...
using namespace mem;
const int N=1e3+9,M=1<<13|9;
int T,n,a[N],s[N],dp[N][M];
string ans;
inline bool validate(int x,int y){return x==0||y<=(x&-x);}
inline int lowbit(int x){return x&-x;}
inline int highbit(int x){return x?1<<31-__builtin_clz(x):0;}
void solve(int i,int j){
  // log("solve %d %d\n",i,j);
  if(i==1){ans+=j?'l':'r';return;}
  if(dp[i][j]>0)return solve(i,dp[i][j]);
  if(j>=a[i]&&validate(j-a[i],a[i])&&dp[i-1][j-a[i]]){ans+='l';return solve(i-1,j-a[i]);}
  if(j<=s[i-1]&&validate(s[i-1]-j,a[i])&&dp[i-1][j]){ans+='r';return solve(i-1,j);}
  assert(0);
}
int main(){
  ...
  while(T--){
    ...
    dp[0][0]=-1;
    for(int i=1;i<=n;i++){
      fill(dp[i],dp[i]+s[i]+1,0);
      for(int j=0;j<=s[i-1];j++)
        if(dp[i-1][j]){
          if(validate(j,a[i]))dp[i][j+a[i]]=-1;
          if(validate(s[i-1]-j,a[i]))dp[i][j]=-1;
        }
      for(int h,j=0;j<=s[i];j++)
        if(dp[i][j]&&(h=highbit(j))==highbit(s[i]-j)){
          dp[i][j]=0;
          log("%d,%d !! %d\n",i,j,h);
          dp[i][j^h]=dp[i][(j^h)+(h<<1)]=j;
        }
      for(int j=1;j<s[i];j++)
        if(dp[i][j]){
          if(lowbit(j)==j&&j>s[i]-j)dp[i][0]=j,log("%d,%d -> %d\n",i,j,0);
          if(lowbit(s[i]-j)==s[i]-j&&s[i]-j>j)dp[i][s[i]]=j,log("%d,%d -> %d\n",i,j,s[i]);
        }
      for(int j=0;j<=s[i];j++)if(dp[i][j])log("%d,%d\n",i,j);
    }
    if(dp[n][0]){
      ans.clear();
      solve(n,0);
