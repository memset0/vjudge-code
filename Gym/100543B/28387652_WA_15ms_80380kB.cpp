...
using namespace mem;
const int N=1e5+9;
int T,n,cnt,rt[N],ans[N];
struct node{
  int x,y;
}a[N];
struct line{
  int k,b;
  inline double operator[](double x)const{return k*x+b;}
};
struct segment{
  int lc,rc;
  line x;
}p[N*50];
void modify(int &u,int v,line x,double l,double r,int dep=0){
  if(dep>50)return;
  if(!v)return u=++cnt,p[u].x=x,p[u].lc=p[u].rc=0,void();
	if(x[l]>=p[v].x[l]&&x[r]>=p[v].x[r])return;
  p[u=++cnt]=p[v];
	if(x[l]<=p[v].x[l]&&x[r]<=p[v].x[r])return p[u].x=x,void();
  double mid=(l+r)/2;
  if(x[mid]<p[u].x[mid])swap(p[u].x,x);
  if(x[l]<p[u].x[l]){
    modify(p[u].lc,p[v].lc,x,l,mid,dep+1);
  }else{
    modify(p[u].rc,p[v].rc,x,mid,r,dep+1);
  }
}
double query(int u,double k,double l,double r){
  if(!u)return std::numeric_limits<double>::max();
  double mid=(l+r)/2;
  if(k<=mid){
    return min(p[u].x[k],query(p[u].lc,k,l,mid));
  }else{
    return min(p[u].x[k],query(p[u].rc,k,mid,r));
  }
}
int main(){
#ifdef memset0
  ...
  while(T--){
    ...
    for(int i=n;i>=1;i--)modify(rt[i],rt[i+1],{a[i].x,-a[i].y},-1e9,1e9);
    for(int l,r,mid,i=1;i<n;i++){
      l=i+1,r=n,ans[i]=0;
      double k=(a[i].y-a[i+1].y)/(double)(a[i].x-a[i+1].x);
      double b=a[i].y-k*a[i].x;
      while(l<=r){
        mid=(l+r)>>1;
        if(query(rt[mid],k,-1e9,1e9)<=-b){
          r=mid-1,ans[i]=mid;
        }else{
          l=mid+1;
        }
