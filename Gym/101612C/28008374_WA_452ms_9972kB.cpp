...
const int N=1e7+9;
const char *f="bcdfghjklmnpqrstvxz";
int n,ans=-1,con;
char s[N],chr[26],a[26][26];

int main(){
  judge_file_io("consonant");
  local_file_input("1.in");
  read(s),n=strlen(s);
  for(int i=0;i+1<n;i++)a[s[i]-'a'][s[i+1]-'a']++;
  for(int x=0;x<(1<<19);x++){
    ...
    for(int i=0;i<19;i++)
      for(int j=0;j<19;j++)
        if(((x>>i)&1)^((x>>j)&1))
          cnt+=a[f[i]-'a'][f[j]-'a'];
    if(cnt>ans)ans=cnt,con=x;
  }
  for(int i=0;i<26;i++)chr[i]='a'+i;
  for(int i=0;i<19;i++)if((con>>i)&1)chr[f[i]-'a']-='a'-'A';
  for(int i=0;i<n;i++)putc(chr[s[i]-'a']);
  putc('\n');
}