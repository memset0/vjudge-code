...
template<class T> inline void updmin(T &a,const T &b){
  if(b<a)a=b;
}

inline long long getdis1(int x,int y){
  auto it=c1[x].find(y);
  if(it==c1[x].end()){
    return INF;
  }else{
...
inline long long getdis2(int x,int y){
  auto it=c2[x].find(y);
  if(it==c2[x].end()){
    return INF;
  }else{
...
int st[N],top;
info solve(const vector<int> &o,int a,int b){
  int cnt=0;
  top=0;
  for(auto it:o){
    if(it==b){
  ...
  return make_pair(cnt,vector<int>{st+1,st+top+1});
}

inline long long calc(int s,int t,vector<int> o){
  long long cost0=getdis1(s,t),cost1=getdis1(t,s);
  long long cost01=getdis2(s,t),cost10=getdis2(t,s);
  auto eval=[&](const vector<int> &v){
    int c1=accumulate(v.begin(),v.end(),0);
    int c0=v.size()-c1;
    if(cost0>=INF&&c0||cost1>=INF&&c1){
      return INF;
    }else{
      return (long long)cost0*c0+(long long)cost1*c1;
    }
  };
  info a=solve(o,0,1),b=solve(o,1,0);
  info c=solve(a.second,1,0),d=solve(b.second,0,1);
  long long s0=eval(o);
  long long s1=a.first&&cost01==INF?INF:(long long)a.first*cost01+eval(a.second);
  long long s2=b.first&&cost10==INF?INF:(long long)b.first*cost10+eval(b.second);
  long long s3=(a.first&&cost01==INF)||(c.first&&cost10==INF)?INF:(long long)a.first*cost01+(long long)c.first*cost10+eval(c.second);
  long long s4=(b.first&&cost10==INF)||(d.first&&cost01==INF)?INF:(long long)b.first*cost10+(long long)d.first*cost01+eval(d.second);
  return min({s0,s1,s2,s3,s4});
}
...
int main(){
  ...
  for(int i=1;i<=d;++i){
    ...
    if(i>1){
      int x=a[i-1],y=a[i];
      mp[min(x,y)][max(x,y)].push_back(x>y);
    }
  }
  read(m);
  while(m--){
  ...
  for(int tem=1;tem<=n;++tem){
    for(const auto &it:mp[tem]){
      ans+=calc(tem,it.first,it.second);
    }
  }
  println(ans);
}//jji