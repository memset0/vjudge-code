...
const int N=1e5+10,inf=1<<25;
int n,m,c[N],f[N],pos[N],ans[N],rev[N][2],cst[N][2];
...
inline void maintain(int x){
  ...
  if((x<<1)<=n&&f[x<<1]+cst[x<<1][1]<f[x]){
    f[x]=f[x<<1]+cst[x<<1][1];
    pos[x]=pos[x<<1];
  }
  if((x<<1|1)<=n&&f[x<<1|1]+cst[x<<1|1][1]<f[x]){
    f[x]=f[x<<1|1]+cst[x<<1|1][1];
    pos[x]=pos[x<<1|1];
  }
}

inline int move(int u,int w){
  if(rev[u][w]){
    if(!--rev[u][w])cst[u][w]=1;
    return -1;
  }else{
    if(++rev[u][w^1])cst[u][w^1]=-1;
    return 1;
  }
}
...
int main(){
  ...
  for(int cas=1;cas<=m;cas++){
    int u,v,lca,tmp=inf;
    read(u);
    for(int t=u,sum=0;t;sum+=cst[t][0],t>>=1){
      if(f[t]+sum<tmp){
        tmp=f[t]+sum;
        v=pos[t];
        lca=t;
    ...
    assert(tmp<inf&&c[v]);
    c[v]--;
    while(u!=lca)ans[cas]+=move(u,0),maintain(u),u>>=1;
    while(v!=lca)ans[cas]+=move(v,1),maintain(v),v>>=1;
    while(lca)maintain(lca),lca>>=1;
  }
  for(int i=1;i<=m;i++){
    ans[i]+=ans[i-1];
    print(ans[i]," \n"[i==m]);
