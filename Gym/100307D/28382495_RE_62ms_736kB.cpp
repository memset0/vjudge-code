...
public:
  ...
  pair<long long,vector<pair<int,int>>> chuliu(int n,int rt=1){
    vector<int> id(n+1),anc(n+1),bel(n+1),pre(n+1),cst(n+1),u(e.size()),v(e.size());
    vector<pair<int,int>> res;
    int orig_n=n,orig_rt=rt;
    long long ans=0;
    ...
    while(true){
      ...
      for(int x,i=1;i<=n;i++)if(i!=rt){
        ...
        ans+=cst[i],x=i;
        log("choose %d,%d,%d => %d %d\n",e[pre[i]].u,e[pre[i]].v,e[pre[i]].w,pre[i],anc[v[pre[i]]]);
        anc[v[pre[i]]]=pre[i];
        while(!bel[x]&&x!=rt){
          bel[x]=i;
          x=e[pre[x]].u;
    ...
    for(int i=1;i<=orig_n;i++)if(i!=orig_rt){
      res.push_back(make_pair(u[anc[i]],v[anc[i]]));
    }
