...
public:
  ...
  pair<long long,vector<pair<int,int>>> chuliu(int n,int rt=1){
    vector<int> id(n+1),bel(n+1),pre(n+1),cst(n+1),trs(e.size(),-1),u(e.size()),v(e.size());
    ...
    while(true){
      ...
      for(int i=0;i<e.size();i++)
        if(e[i].u!=e[i].v&&(!~cst[e[i].v]||e[i].w<cst[e[i].v])){
          cst[e[i].v]=e[i].w;
          pre[e[i].v]=i;
        }
      int cnt=0;
      for(int x,i=1;i<=n;i++)if(i!=rt){
        ...
        ans+=cst[i],x=i;
        cho[pre[i]]=true;
        if(~trs[pre[i]]&&trs[pre[i]]!=pre[i])cho[trs[pre[i]]]=false;
        while(!bel[x]&&x!=rt){
          bel[x]=i;
          x=e[pre[x]].u;
        }
      ...
      for(int i=0;i<e.size();i++){
        e[i].w-=cst[e[i].v],trs[i]=pre[e[i].v];
        e[i].u=id[e[i].u],e[i].v=id[e[i].v];
      }
      rt=id[rt],n=cnt;
    }
    for(int i=0;i<e.size();i++)if(cho[i]){
      res.push_back(make_pair(u[i],v[i]));
    }
