...
long long dec(long long l,long long r,long long t){
  if((r-l+1)>2*t/(l+r)+1)return -1;
  return t-(r-l+1)*(l+r)/2;
}
...
bool check(long long cur){
  ...
  for(int i=1;i<=n;i++){
    long long lst=max<long long>(cur-a[i],0);
    if(i+lst<=n){
      l[i+lst]=max(l[i+lst],i);
    }
    if(i-lst>=1){
      r[i-lst]=min(r[i-lst],i);
    }
  }
  for(int i=2;i<=n;i++){
    l[i]=max(l[i-1],l[i]);
  ...
  for(int i=1;i<=n;i++){
    if(l[i]>=1&&r[i]<=n){
      long long lim=::lim+s[r[i]]-s[l[i]-1]-cur;
      if(l[i]<i){
        lim=dec(cur-i+l[i],cur-1,lim);
      }
      if(r[i]>i){
        lim=dec(cur+i-r[i],cur-1,lim);
      }
      if(lim>=0){
        return true;
      }
...
int main(){
  ...
  long long l=*max_element(a+1,a+n+1),r=l+lim,mid,ans;
  while(l<=r){
    mid=(l+r)>>1;
    if(check(mid)){
      l=mid+1;
