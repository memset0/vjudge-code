...
const int N=3e4+10,M=400,OL=5;
int n,m,len[N],cst[N],dep[N],fa[N],dp[N][M<<1],tem[M<<1],pat[N][2];
long long ans,cho;
vector<int> G[N];
...
void dfs(int u){
  ...
  for(int v:G[u])if(v!=fa[u]){
    fa[v]=u,dep[v]=dep[u]+1,cst[v]=1,dfs(v);
    len[u]=max(len[u],len[v]+1);
    for(int i=-len[v];i<=len[v];i++)dp[u][M+i+1]+=dp[v][M+i];
    for(int i=0;i<2;i++)cho+=(long long)pat[u][i]*pat[v][i];
    for(int i=0;i<2;i++)pat[u][i^1]+=pat[v][i];
  }
  dp[u][M]+=1;
}

inline int query(int u,int *res){
  int lim=0;
  res[M]=0;
  function<void(int,int)> dfs=[&](int u,int c){
    ...
    if(!fa[u])return;
    c+=cst[u];
    if(len[fa[u]]+abs(c)>lim){
      for(int i=lim+1;i<=len[fa[u]]+abs(c);i++)res[M+i]=res[M-i]=0;
      lim=len[fa[u]]+abs(c);
    }
    for(int i=-len[u];i<=len[u];i++)tem[M+i+c+cst[u]]-=dp[u][M+i];
    for(int i=-len[fa[u]];i<=len[fa[u]];i++)tem[M+i+c]+=dp[fa[u]][M+i];
    dfs(fa[u],c);
  };
  dfs(u,-cst[u]);
  return lim;
...
inline void modify(int u){
  function<void(int)> pushdown=[&](int u){
    if(!fa[u])return;
    pushdown(fa[u]);
    for(int i=-len[u];i<=len[u];i++)dp[fa[u]][M+i+cst[u]]-=dp[u][M+i];
  };
...
int main(){
  judge_file_io("hidden"),local_file_input("1.in");
  read(n);
  for(int i=1;i<n;i++){
  ...
  dfs(1);
  for(int i=1;i<n;i++){
    if(fa[e[i].u]==e[i].v)swap(e[i].u,e[i].v); // => fa[v]=u
    // log<GREEN>("=== u=%d v=%d w=%d ===\n",e[i].u,e[i].v,e[i].w);
    // for(int i=1;i<=n;i++){
    //   log("dp[%d] = ",i);
    //   for(int j=-OL;j<=OL;j++)log("%d%c",dp[i][M+j]," \n"[j==OL]);
    // }
    int lim=min(len[e[i].v],query(e[i].v,tem));
    // log("  tem = ",query(e[i].v,tem));
    // for(int j=-OL;j<=OL;j++)log("%d%c",tem[M+j]," \n"[j==OL]);
    for(int j=-lim;j<=lim;j++)ans+=(long long)dp[e[i].v][M+j]*tem[M-j]*e[i].w;
    modify(e[i].v);
  }
  log<RED>("ans=%lld cho=%lld\n",ans,cho);
  printf("%.12lf\n",ans/(double)cho);
}