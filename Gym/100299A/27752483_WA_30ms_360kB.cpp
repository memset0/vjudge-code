...
inline void apply(const vector<int> &v,int i,int j){
  // cout<<"apply";for(int x:v)cout<<" "<<x;cout<<endl;
...
int main(){
  ...
  for(atom x:atom_list)
    for(atom y:atom_list){
      ...
      // cout<<to_str(x)<<" "<<to_str(y)<<" "<<to_str(trans[x][y])<<endl;
    }
  cin>>T;
  while(T-->0){
    cin>>m>>n;
    ...
    //   for(int j=1;j<=m;j++)
    //     printf("%d%c",a[i][j]," \n"[j==m]);
    if(ask(four(a,1,1),123_s).size()){
      apply(ask(four(a,1,1),123_s),1,1);
    }else{
      apply(ask(four(a,1,1),132_s),1,1),R(n);
    }
    for(int i=2;i<n-i+1;i++){
      if(ask(four(a,i,1),123_s).size()){
        apply(ask(four(a,i,1),123_s),i,1);
      }else{
        apply(ask(four(a,i,1),132_s),i,1),R(n-i+1);
      }
    }
    for(int j=2;j<m-j+1;j++){
      if(ask(four(a,1,j),123_s).size()){
        apply(ask(four(a,1,j),123_s),1,j);
      }else{
        apply(ask(four(a,1,j),321_s),1,j),C(m-j+1);
      }
    }
    for(int i=2;i<n-i+1;i++)
      for(int j=2;j<m-j+1;j++){
        const auto &sol=ask(four(a,i,j),123_s);
