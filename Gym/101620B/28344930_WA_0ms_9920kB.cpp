...
int main(){
  ...
  read(m);
  for(int i=0;i<m;i++)read(a[i].first,a[i].second);
  read(n);
  for(int i=0;i<n;i++)read(b[i].first,b[i].second);
  b[n]={1<<30,1<<30};
  for(int i=0;i<=n;i++)id[i]=i,anc[i]=i;
  sort(id,id+n+1,[&](int x,int y){return b[x]>b[y];});
  st.insert({0,-1});
  sort(a,a+m);
  int r=m-1;
  for(int k=0;k<=n;k++){
    int i=id[k];
    pair<int,int> c(b[i].second,i);
    while(true){
      auto it=--st.lower_bound(c);
      if(it->second>i)st.erase(it);else break;
    }
    if(i!=n)p[i]=st.lower_bound(c)->second;
    st.insert(c);
    while(r>=0&&a[r].first>b[id[k+1]].first){
      ans[st.lower_bound(make_pair(a[r--].second,-1))->second]++;
    }
  }
  for(int i=n-1;i>=0;i--)p[i]=find(p[i]),anc[i]=p[i];
  for(int i=n;i>=0;i--)ans[p[id[i]]]+=ans[id[i]];
  for(int i=0;i<n;i++)println(ans[i]);
}