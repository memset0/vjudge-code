...
typedef pair<int,long long> answer_t;
const int N=5e4+9,sqn=300,M=sqn+9,L=N/sqn+9;
int n,m,a[N],bln[N];
answer_t ans={0,LLONG_MIN};

struct node{
  int x,y;
  inline node(){}
  inline node(int a){a>0?(x=a,y=0):(x=0,y=-a);}
...
inline void update(answer_t &a,const answer_t &b){if(b.second>a.second)a=b;}
inline long long calc(const node &t){return (long long)t.x*U.y-(long long)t.y*U.x;}
inline long long cross(const node &u,const node &a,const node &b){return (long long)(a.x-u.x)*(b.y-u.y)-(long long)(b.x-u.x)*(a.y-u.y);}

struct block{
  int l=N,r,m,s[M],t[M];
  node mov,a[M];
  pair<int,long long> query(){
    int ll=2,rr=m,res=1,mid;
    // log<BLUE>("mov (%d,%d)\n",mov.x,mov.y);
    // for(int i=1;i<=r-l+1;i++)log<RED>("(%d,%d)%lld%c",a[i].x,a[i].y,calc(a[i]+mov)," \n"[i==r-l+1]);
    // for(int i=1;i<=m;i++)log<RED>("(%d,%d)%lld%c",a[s[i]].x,a[s[i]].y,calc(a[s[i]]+mov)," \n"[i==m]);
    while(ll<=rr){
      mid=(ll+rr)>>1;
      if(calc(a[s[mid]]+mov)-calc(a[s[mid-1]]+mov)<=0)rr=mid-1;
      else ll=mid+1,res=mid;
    }
    // printf("> %d %d\n",res,s[res]);
    return {s[res]+l-1,calc(a[s[res]]+mov)};
  }
  void build(){
    for(int i=1;i<=r-l+1;i++)t[i]=i;
    m=0,sort(t+1,t+(r-l+2));
    for(int i=1;i<=r-l+1;i++){
      while(m>1){
        long long x=cross(a[s[m-1]],a[s[m]],a[t[i]]);
        if(x<0||x==0&&t[i]<t[m])m--; else break;
      }
      s[++m]=t[i];
    }
  }
  void rebuild(int k,const node &x){
    for(int i=l;i<=r;i++)a[i-l+1]=a[i-l+1]+mov;
    for(int i=k;i<=r;i++)a[i-l+1]=a[i-l+1]+x;
    mov=0,build();
  }
}b[L];
...
int main(){
  ...
  for(int i=1;i<=n;i++){
    read(a[i]);
    bln[i]=(i-1)/sqn+1;
    b[bln[i]].l=min(b[bln[i]].l,i),b[bln[i]].r=i;
  }
  for(int i=1;i<=n;i++){
    s[i]=s[i-1]+a[i],U=U+a[i];
    b[bln[i]].a[i-b[bln[i]].l+1]=s[i];
  }
  // log<GREEN>("%d %d\n",U.x,U.y);
  for(int i=1;i<=bln[n];i++){
    b[i].build();
    update(ans,b[i].query());
  }
  print(ans.first,'\n');
  for(int k,x,i=1;i<=m;i++){
    read(k,x),ans.second=LLONG_MIN;
    U=U-a[k],D=node(x)-a[k],a[k]=x,U=U+a[k];
    // log<GREEN>("%d %d\n",U.x,U.y);
    b[bln[k]].rebuild(k,D);
    for(int i=bln[k]+1;i<=bln[n];i++)b[i].mov=b[i].mov+D;
    for(int i=1;i<=bln[n];i++)update(ans,b[i].query());
    print(ans.first,'\n');
  }
}