...
inline long long calc(const node &t){return t.x*U.y-t.y*U.x;}
...
struct block{
	...
	long long query(){
		...
		// for(int i=1;i<=r-l+1;i++)log<RED>("(%d,%d)%lld%c",a[i].x,a[i].y,calc(a[i])," \n"[i==r-l+1]);
		// for(int i=1;i<=m;i++)log<RED>("(%d,%d)%lld%c",t[s[i]].x,t[s[i]].y,calc(t[s[i]])," \n"[i==m]);
		while(ll<=rr){
			mid=(ll+rr)>>1;
			if(calc(t[s[mid]])-calc(t[s[mid-1]])<0)rr=mid-1;
			else ll=mid+1,res=mid;
		}
		return s[res]+l-1;
	}
	void build(){
		memcpy(t+1,a+1,(r-l+1)<<4),m=0;
		sort(t+1,t+r-l+2);
		for(int i=1;i<=r-l+1;i++){
			while(m>1&&cross(t[s[m-1]],t[s[m]],t[i])<=0)m--;
			s[++m]=i;
		}
	}
...
int main(){
	...
	for(int k,x,i=1;i<=m;i++){
		...
		// log<GREEN>("%d %d\n",U.x,U.y);
		b[bln[k]].rebuild(k,D);
		for(int i=bln[k]+1;i<=bln[n];i++)b[i].mov=b[i].mov+x;
		for(int i=1;i<=bln[n];i++)ans=max(ans,b[i].query());
		print(ans,'\n');
	}
}