...
const int N=5010,M=5e4+5;
int n,m,b,s;
long long w[N],f[N][N];

struct Graph{
  bool vis[N];
  int dis[N],hed[N],to[M<<1],nxt[M<<1],val[M<<1],cnt;
  priority_queue<pair<int,int>> q;
  inline void adde(int u,int v,int w){
    ++cnt,to[cnt]=v,nxt[cnt]=hed[u],hed[u]=cnt,val[cnt]=w;
  }
...
int main(){
  ...
  F.dij(b+1);
  G.dij(b+1);
  for(int i=1;i<=b;i++)w[i]=F.dis[i]+G.dis[i];
  sort(w+1,w+b+1);
  ...
  for(int i=0;i<s;i++)
    for(int j=0;j<b;j++)
      for(int k=1;j+k<=b&&k*(i+1)<=b;k++)
        f[i+1][j+k]=min(f[i+1][j+k],f[i][j]+(w[j+k]-w[j])*(k-1));
  cout<<f[s][b]<<endl;
}