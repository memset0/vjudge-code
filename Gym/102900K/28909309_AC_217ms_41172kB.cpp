...
using namespace mem;
const int N=2e5+10;
int T,n,m,cnt,top,tim,dfn[N],low[N],stk[N],col[N],fa[N][20],dep[N];
char s[N];
bool vis[N],ins[N];
vector<int> F[N],G[N];
void build(int u){
...
int main(){
  ...
  while(T--){
    ...
    // for(int i=1;i<=n;i++)log("%d%c",fa[i][0]," \n"[i==n]);
    // for(int i=1;i<=n;i++)log("%d%c",dep[i]," \n"[i==n]);
    for(int i=1;i<=n;i++)if(!dfn[i])tarjan(i,0);
    // for(int i=1;i<=n;i++)log("%d%c",col[i]," \n"[i==n]);
    bool flag=false;
    for(int u=1;u<=n&&!flag;u++)if(vis[u])
      for(int v:F[u])if(vis[v]){
        int w=lca(u,v);
        // log("u=%d v=%d w=%d\n",u,v,w);
        if(w&&vis[w]&&(col[u]==col[fa[w][0]]||u==w)){
          flag=true;
          break;
        }
      }
