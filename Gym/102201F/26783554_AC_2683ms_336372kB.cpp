...
Sim, class s > ostream& operator<<(ostream& p, pair<n, s> x) { return p << "<" << x.f << ", " << x.s << ">"; }
Sim > auto operator<<(ostream& p, n y) -> typename enable_if<!is_same<n, string>::value, decltype(y.begin(), p)>::type {
	int o = 0;
	p << "{";
	for (auto c : y) {
		if (o++) p << ", ";
...
int main() {
	...
	REP(i, q) {
		...
		REP(j, cand[i].size()) {
			//			debug(qq[i], "col =", cand[i][j], "cnt =", cnt[i][j]);
