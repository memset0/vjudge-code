...
#ifdef memset0
  const int key=324958234,mod=21;
  int n,at,cnt,lim=20000,pto[N],to[N][N],ansx,ansy,sta[N];
  bool visited[N][N];
  inline int nextint(){
    static int x;
    cin>>x;
    return ((x-key)%mod+mod)%mod;
  }
  inline void randomgen(int seed){
    srand(seed);
    n=rand()%20+1;
    for(int i=0;i<n;i++)
      for(int j=0;j<m;j++)
...
int dfs(){
  ...
  int back=-1;
  for(int i=0;i<m;i++)
    if(back==-1||dep[i]>dep[back]){
      back=i;
    }
  if(~back&&now){
    int tem=go((back+1)%m,right,(back+1)%m);
    while(tem!=left)tem=go(0,right,0);
    for(int i=2;i<dep[back];i++)go(0,left,0);
  }
  assert(now==0||~back);
  return ~back?dep[back]:114514;
}
