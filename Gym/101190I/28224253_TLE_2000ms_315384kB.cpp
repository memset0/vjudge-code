...
namespace mem{ //v2.9.0 => size: 15.71KiB
  #ifdef memset0
  #else
    #define MEM_FASTIO
  #endif
...
enum ways{
...
#ifdef memset0
  int at,lim=100,pto[N],to[N][N],ansx,ansy,sta[N];
  inline void init(){
  ...
  inline int go(int st,int dir,int pa){
    static string rsp;
    cout<<st<<' '<<(dir==left?"left":"right")<<' '<<pa<<endl<<flush;
    cin>>rsp;
...
void dfs(){
  ...
  if(~fallback){
    int tem=go(fallback,right,fallback);
    while(tem!=left)tem=go(0,right,0);
    for(int i=1;i<dep[fallback];i++)go(0,left,0);
  }
}
