#include<bits/stdc++.h>
namespace mem{ //v2.9.8 => size: 16.23KiB
  #ifdef memset0
  #else
    #define MEM_FASTIO
  #endif
...
public:
  inline void add_edge(int u,int v,int w){e.push_back((edge){u,v,w});}
  pair<long long,vector<pair<int,int>>> chuliu(int n,int rt=1){
    vector<int> id(n+1),bel(n+1),pre(n+1),cst(n+1),u(e.size()),v(e.size());
    vector<char> cho(e.size());
    vector<pair<int,int>> res,elim;
    long long ans=0;
    for(int i=0;i<e.size();i++)u[i]=e[i].u,v[i]=e[i].v;
    while(true){
      fill(id.begin(),id.end(),0);
      fill(bel.begin(),bel.end(),0);
      fill(pre.begin(),pre.end(),-1);
      for(int i=0;i<e.size();i++)
        if(e[i].u!=e[i].v&&(!~pre[e[i].v]||e[i].w<cst[e[i].v]))
          cst[e[i].v]=e[i].w,pre[e[i].v]=i;
      int cnt=0;
      for(int x,i=1;i<=n;i++)if(i!=rt){
        if(!~pre[i])return make_pair(-1,res);
        ans+=cst[i],x=i;
        cho[pre[i]]=true;
        while(!bel[x]&&x!=rt)bel[x]=i,x=e[pre[x]].u;
        if(bel[x]==i){
          ++cnt;
          do id[x]=cnt,x=e[pre[x]].u; while(!id[x]);
        }
      }
      if(!cnt)break;
      for(int i=1;i<=n;i++)if(!id[i])id[i]=++cnt;
      for(int i=1;i<=n;i++)log<BLUE>("%d%c",id[i]," \n"[i==n]);
      for(int i=0;i<e.size();i++){
        if(id[e[i].u]!=id[e[i].v]){
          e[i].w-=cst[e[i].v];
          if(pre[e[i].v]!=i)elim.push_back(make_pair(i,pre[e[i].v]));
        }
        e[i].u=id[e[i].u],e[i].v=id[e[i].v];
      }
      rt=id[rt],n=cnt;
    }
    for(int i=(int)elim.size()-1;i>=0;i--)if(cho[elim[i].first])cho[elim[i].second]=false;
    for(int i=0;i<e.size();i++)if(cho[i])res.push_back(make_pair(u[i],v[i]));
    return make_pair(ans,res);
  }
};

const int N=55;
int n,cnt=1,rnk[N],nod[N][N],pos[N][N],len[N][N];
string s[N];
pair<int,char> ans[N*N];
MDSTConstruction mdst;

int main(){
  judge_file_io("dictionary");
  local_file_input("1.in");
  read(n);
  for(int i=1;i<=n;i++){
    read(s[i]);
    mdst.add_edge(n+1,i,s[i].length());
  }
  for(int i=1;i<=n;i++)
    for(int j=1;j<=n;j++)if(i!=j){
      ...
      for(int x=0;x<s[i].length();x++){
        int cur=0;
        for(int y=0;y<s[j].length()&&x+y<s[i].length();y++)
          if(s[i][x+y]==s[j][y]){
            cur++;
          }else{
            break;
      ...
      mdst.add_edge(i,j,(int)s[j].length()-len[i][j]);
      // log("edge :: %d %d %d (%d)\n",i,j,(int)s[j].length()-len[i][j],len[i][j]);
    }
  auto res=mdst.chuliu(n+1,n+1);
  log<RED>("chuliu=%lld\n",res.first);
  auto &sol=res.second;
  rnk[n+1]=1;
  for(int i=1;i<=n;i++)
    for(const auto &it:sol)
      if(rnk[it.first]&&!rnk[it.second]){
        rnk[it.second]=rnk[it.first]+1;
      }
  for(int i=1;i<=n;i++)log<BLUE>("%d%c",rnk[i]," \n"[i==n]);
  sort(sol.begin(),sol.end(),[&](const pair<int,int> &a,const pair<int,int> &b){
    return rnk[a.second]<rnk[b.second];
  });
  for(const auto &it:sol){
    ...
    if(u==n+1||!len[u][v]){
      nod[v][0]=1;
      for(int i=1;i<=s[v].length();i++)nod[v][i]=++cnt;
      for(int i=0;i<s[v].length();i++)ans[nod[v][i+1]]={nod[v][i],s[v][i]};
    }else{
      for(int i=0;i<=len[u][v];i++)nod[v][i]=nod[u][pos[u][v]+i];
      for(int i=len[u][v]+1;i<=s[v].length();i++)nod[v][i]=++cnt;
      for(int i=len[u][v];i<s[v].length();i++)ans[nod[v][i+1]]={nod[v][i],s[v][i]};
    }
    for(int i=0;i<=s[v].length();i++)log("%d%c",nod[v][i]," \n"[i==s[v].length()]);
    for(int i=0;i<=s[v].length();i++)log("%d%c",ans[nod[v][i]].first," \n"[i==s[v].length()]);
  }
  for(int i=1;i<=n;i++){
    for(int j=0;j<=s[i].length();j++)log("%d%c",nod[i][j]," \n"[j==s[i].length()]);
  }
  println(cnt,"\n0");
  for(int i=2;i<=cnt;i++){
    assert(ans[i].first);
