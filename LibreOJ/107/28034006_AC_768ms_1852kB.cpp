...
int main(){
	...
	for(srand(20030417),scanf("%d",&Q);Q--;)switch(scanf("%d%d",&o,&v),o){
		case 0:rnd[++n]=rand(),val[n]=v,siz[n]=1,split(rt,v,x,y),rt=merge(x,merge(n,y));break;
		case 1:split(rt,v-1,x,y),split(y,v,y,z),y=merge(ch[y][0],ch[y][1]),rt=merge(x,merge(y,z));break;
		case 2:printf("%d\n",kth(rt,v));break;
		case 3:split(rt,v-1,x,y),printf("%d\n",siz[x]),rt=merge(x,y);break;
		case 4:split(rt,v-1,x,y),printf("%d\n",!x?-1:kth(x,siz[x])),rt=merge(x,y);break;
		case 5:split(rt,v,x,y),printf("%d\n",!y?-1:kth(y,1)),rt=merge(x,y);break;
	}
}
