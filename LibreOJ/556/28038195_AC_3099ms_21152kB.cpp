...
const int N = 410010, M = N << 3, mod = 998244353;
...
void main() {
    static int inv[N];
    read(n), read(m), ++n, main_init();
    poly F(n);
    for (int i = 1; i <= m; i++) {
        read(a[i]), read(b[i]), ++cnt[a[i]];
        if (b[i] && (ll)a[i] * (b[i] + 1) < n) --cnt[a[i] * (b[i] + 1)];
    }
    inv[0] = inv[1] = 1;
    for (int i = 2; i < n; i++) inv[i] = (ll)(mod - mod / i) * inv[mod % i] % mod;
    for (int i = 1; i < n; i++)
        for (int j = 1; i * j < n; j++)
            F[i * j] = sub(F[i * j], (ll)(cnt[i] + mod) % mod * inv[j] % mod);
    poly G = exp(F);
