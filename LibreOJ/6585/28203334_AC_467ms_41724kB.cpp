...
int n,m,d,e,cut,graph[40][40];
string str;

struct Command;
struct Program;

inline int w2i(int x){return x=='n'?0:x=='w'?1:x=='s'?2:3;}
inline int i2w(int x){return x==0?'n':x==1?'w':x==2?'s':'e';}

struct Status{
  int v;
  Status(){v=0;}
  Status(int x,int y,int w){v=1|x<<1|y<<11|w<<21;}
  inline void set(int t){v=t;}
  inline int get()const{return v;}
  inline bool inf()const{return !~v;}
  inline bool empty()const{return !(v&1);}
  inline int x()const{return v>>1&1023;}
  inline int y()const{return v>>11&1023;}
  inline int w()const{return v>>21;}
  inline Status operator=(const Status &rhs){v=rhs.v;return rhs;}
  inline bool check(char t)const{
    if(t=='b'){
      int a=x()+nxt[w()][0],b=y()+nxt[w()][1];
      return a<0||b<0||a==n||b==m||graph[a][b];
    }else{
      return w()==w2i(t);
    }
  }
};
Status cud[1000][40][40][4];

struct Command{
  int d;
  char o,c;
  Program *p,*q;
  Status call(const Status&)const;
  unsigned key;
  Command(){key=rand()%100;}
};

struct Program:vector<Command>{
  Status pgd[40][40][4];
  Status call(Status);
  unsigned key;
  Program(){key=rand()%100;}
}prog[26];

inline Status Command::call(const Status &a)const{
  if(a.inf())return a;
  // log<RED>("C%03d %d %d %d : %c\n",key,a.x(),a.y(),a.w(),o);
  int x=a.x(),y=a.y(),w=a.w();
  if(o=='m'){
    x+=nxt[w][0],y+=nxt[w][1];
    if(x<0||y<0||x==n||y==m||graph[x][y])return a;
  }else if(o=='l'){
    w=(w+1)%4;
  }else if(o=='i'){
    return (a.check(c)?p:q)->call(a);
  }else if(o=='u'){
    // log("> %c %d %d[%d] %d[%d] %d\n",c,a.check(c),a.x(),a.x()+nxt[a.w()][0],a.y(),a.y()+nxt[a.w()][1],a.w());
    if(a.check(c))return a;
    if(cud[d][x][y][w].empty()){
      cud[d][x][y][w].set(-1);
      return cud[d][x][y][w]=call(p->call(a));
    }else{
      return cud[d][x][y][w];
    }
  }else{
    return prog[o-'A'].call(a);
  }
  return Status(x,y,w);
}

inline Status Program::call(Status a){
  int x=a.x(),y=a.y(),w=a.w();
  if(!pgd[x][y][w].empty())return pgd[x][y][w];
  // log<GREEN>("P%03d %d %d %d\n",key,a.x(),a.y(),a.w());
  pgd[x][y][w].set(-1);
...
void parse(Program &u,string s){
  // log<BLUE>("parse %s\n",s.c_str());
  vector<int> pir(s.length(),-1),stk;
  function<void(Program&,int,int)> dfs=[&](Program &u,int l,int r){
    if(l>r)return;
    // log("dfs %s %d %d\n",string(s.begin()+l,s.begin()+r+1).c_str(),l,r);
    int p=l;
    while(p<=r){
      Command c;
      c.o=s[p];
      if(s[p]=='i'){
        c.c=s[p+1],c.p=new Program(),c.q=new Program();
        dfs(*c.p,p+3,pir[p+2]-1);
        dfs(*c.q,pir[p+2]+2,pir[pir[p+2]+1]-1);
        p=pir[pir[p+2]+1]+1;
      }else if(s[p]=='u'){
        c.c=s[p+1],c.d=++cut,c.p=new Program();
        dfs(*c.p,p+3,pir[p+2]-1);
        p=pir[p+2]+1;
      }else{
        p++;
...
int main(){
  ...
  for(int i=0;i<d;i++){
    read(str);
    parse(prog[str[0]-'A'],string(str.begin()+2,str.end()));
  }
  for(int x,y,i=0;i<e;i++){
    read(x,y,str);
    Status a(x-1,y-1,w2i(str[0]));
    read(str);
    auto cur=new Program();
    parse(*cur,str);
    a=cur->call(a);
    if(a.inf()){
      println("inf");
    }else{
      println(a.x()+1,' ',a.y()+1,' ',(char)i2w(a.w()));
    }
  }
}