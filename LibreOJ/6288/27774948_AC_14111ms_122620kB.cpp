...
namespace seg {
	...
	void init(int l, int r) { L = l, R = r; build(rt[0]); }
	...
	int query(int u, int v, int ql, int qr, int l = L, int r = R) {
		...
		// if (rand() % 4 == 0) exit(0);
		if (ql == l && qr == r) return cnt[v] - cnt[u];
		int mid = (l + r) >> 1;
...
namespace sam {
	...
	void insert(int c) {
		// printf("insert %d\n", c);
	    int f = lst, u = ++nod; lst = u, end[u] = len[u] = bln[u] = ++cnt;
	...
	void solve(int u) {
		...
		if (u != 1) {
			...
			for (int f, l = 1, r = dep[u] - 1, mid, flag; mid = l + r >> 1, l <= r; ) {
				f = jump(u, mid);
				// printf("[%d %d] => mid = %d f = %d | %d %d | %d %d %d %d \n", l, r, mid, f, rt[dfn[f] - 1], rt[dfn[f] + siz[f] - 1], end[u] - len[u] + len[f], end[u] - 1);
				flag = seg::query(rt[dfn[f] - 1], rt[dfn[f] + siz[f] - 1], end[u] - len[u] + len[f], end[u] - 1);
				if (flag) ans[u] = std::max(ans[u], ans[f] + 1), r = mid - 1; else l = mid + 1;
			}
		}
		for (auto v : son[u]) solve(v);
	}
	int solve() {
		int ret = 0; memset(f, 0, sizeof(f));
		...
		for (int i = 1; i <= nod; i++)
			if (bln[did[i]]) seg::modify(rt[i], rt[i - 1], bln[did[i]]);
			else rt[i] = rt[i - 1];
		solve(1);
