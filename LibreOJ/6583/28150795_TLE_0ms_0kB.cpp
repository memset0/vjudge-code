...
const int N=2e6+7;
int n,m,nod,end[N],siz[N],fail[N],ch[N][26];
char str[N];
vector<int> son[N];

inline int append(char c,int f){
  // printf("append %c %d\n",c,f);
  if(ch[f][c-'A'])return ch[f][c-'A'];
  return ch[f][c-'A']=++nod;
}
...
void buildFail(){
  ...
  while(q.size()){
    ...
    for(int c=0;c<26;c++)if(ch[u][c]){
      int v=ch[u][c];
      fail[v]=fail[u];
      while(fail[v]&&!ch[fail[v]][c])fail[v]=fail[fail[v]];
      if(ch[fail[v]][c]!=v)fail[v]=ch[fail[v]][c];
      q.push(v);
      // printf("%d[%d] -> %d[%d] (%d)\n",u,fail[u],v,fail[v],c);
    }
  }
  // for(int i=1;i<=nod;i++)print(fail[i]," \n"[i==nod]);
}
...
int main(){
  ...
  for(int len,i=1;i<=m;i++){
    read(str),len=strlen(str);
    for(int j=len-1;j>=0;j--)end[i]=append(str[j],end[i]);
  }
  buildFail();
  for(int i=1;i<=nod;i++)son[fail[i]].push_back(i);
  dfsFailTree(0);
  for(int i=1;i<=m;i++)println(siz[end[i]]);
}