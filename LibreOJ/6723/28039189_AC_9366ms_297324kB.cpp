...
int query(int u, int ql, int qr, int l, int r) {
	...
	return std::min(query(u << 1, ql, mid, l, mid),
					query(u << 1 | 1, mid + 1, qr, mid + 1, r));
}
}  // namespace f
namespace g {
const int L = M * 50;
int cnt, sum[L], lc[L], rc[L];
void build(int& u, int l, int r) {
	u = ++cnt;
	if (l == r) return;
	int mid = (l + r) >> 1;
...
void modify(int& u, int v, int x, int l, int r) {
	...
	int mid = (l + r) >> 1;
	if (x <= mid)
		modify(lc[u], lc[v], x, l, mid);
	else
		modify(rc[u], rc[v], x, mid + 1, r);
}
int query(int u, int ql, int qr, int l, int r) {
	...
	return query(lc[u], ql, mid, l, mid) +
