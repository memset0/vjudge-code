...
map<long long,atom> dp;
atom DP(long long n){
	auto it=dp.find(n);
	if(it!=dp.end()){
		return it->second;
	}
	atom L=DP((n+1)>>1),R=DP(n>>1),U;
	U.u=L.u+R.u+(L.lc+1)*(R.ls+1)+(L.rs+1)*(R.lc+1)-1;
	U.lc=L.lc+R.lc+1;
	U.ls=L.ls+R.ls+R.lc+1;
	U.rc=R.rc+L.rc+1;
	U.rs=R.rs+L.rs+L.rc+1;
//	log("dp[%lld] => %d %d %d %d %d\n",n,U.u,U.lc,U.ls,U.rc,U.rs);
	return dp[n]=U;
}
int main(){
#ifdef memset0
