...
inline bool calc(int x,const vector<bool> &a,const vector<tuple<int,int,int>> &p){
  vector<bool> b(n);
  for(int i=1;i<n;i++){
    int x=get<0>(p[i])<=n?a[get<0>(p[i])]:b[get<0>(p[i])-n];
    int y=get<1>(p[i])<=n?a[get<1>(p[i])]:b[get<1>(p[i])-n];
    b[i]=get<2>(p[i])==1?x&&y:x||y;
  }
  // log("calc[%d] >> ",x);
  // for(int i=1;i<=n;i++)log("%d ",(int)a[i]);
  // for(int i=1;i<n;i++)log("%d%c",(int)b[i]," \n"[i+1==n]);
  return n==1?a.back():b.back();
}
...
int main(){
  ...
  ans=vector<int>(n+1);
  c=vector<bool>(m+1);
  x=vector<vector<int>>(m+1,vector<int>(n+1));
  a=vector<vector<bool>>(m+1,vector<bool>(n+1));
  p=vector<vector<tuple<int,int,int>>>(m+1,vector<tuple<int,int,int>>(n));
  for(int i=1;i<=m;i++)
    for(int j=1;j<n;j++){
      read(get<0>(p[i][j]),get<1>(p[i][j]),get<2>(p[i][j]));
    }
  for(int i=1;i<=m;i++)
    for(int j=1;j<=n;j++){
      read(x[i][j]);
  ...
  for(int i=1;i<=n;i++){
    vector<int> id;
    for(int j=1;j<=m;j++)id.push_back(j);
    id.sort([&](int a,int b){
      return x[a][i]<x[b][i];
    });
    for(int j:id){
      ...
      if(cur==s){
        ...
        ans[i]=find(id.begin(),id.end(),j)-id.begin()+1;
        for(int k=i+1;k<=n;k++){
          ans[k]=0;
        }
        for(int i=1;i<=n;i++){
