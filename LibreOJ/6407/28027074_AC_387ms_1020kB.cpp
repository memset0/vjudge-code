...
const int N=22;
const f96 eps=1e-6,g=9.80665;
int n,m,_x,_y,_h[N][N],_h2[N][N],d[N][N],t[N][N][N][N];
f96 w,v,vx,vh,D;

bool checker(int x1,int x2,int y1,int y2,int _h[N][N]){
  for(int i=min(x1,x2);i<max(x1,x2);i++){
    f96 k=(i-x1+0.5)/(x2-x1);
    f96 t=k*D/vx,H=-g/2*t*t+_h[x1][y1]+vh*t;
    f96 L=y1+(y2-y1)*k-0.5;
    int j=ceil(L);
    if(_h[i][j]>H||_h[i+1][j]>H) return 0;
    if(fabs(L-ceil(L))<eps&&(_h[i][j+1]>H||_h[i+1][j+1]>H)) return 0;
  }
  return 1;
}

inline bool solve(int x,int y,int x2,int y2){
  f96 d=1.00*((x2-x)*(x2-x)+(y2-y)*(y2-y))*w*w;
  f96 h=_h[x2][y2]-_h[x][y];
  f96 A=-h*h-d,B=d*v*v-d*g*h,C=-d*d*g*g;
  f96 det=B*B-A*C;
  if(det<0)return 0;
  vx=sqrt((-B+sqrt(det))/(2.00*A));
  vh=sqrt(v*v-vx*vx);
  D=sqrt(d);
  if(!checker(x,x2,y,y2,_h))return 0;
  if(!checker(y,y2,x,x2,_h2))return 0;
  return 1;
}
