...
namespace mem{ //v2.6.0 => size: 14.94KiB
  ...
  #define __integer_mapper(func)    \\u003C/span>
      func(int)                     \\u003C/span>
      func(unsigned int)            \\u003C/span>
      func(long long int)           \\u003C/span>
      func(unsigned long long int)
  #define __float_mapper(func)      \\u003C/span>
      func(float)                   \\u003C/span>
      func(double)                  \\u003C/span>
      func(long double)
  ...
  #define __to_string(T)                   \\u003C/span>
      inline string to_string(const T &x){ \\u003C/span>
        return std::to_string(x);          \\u003C/span>
      }
  ...
  #define __read(T)                             \\u003C/span>
      inline void read(T &x) {                  \\u003C/span>
        x=0; bool f=0; char c=getc();           \\u003C/span>
        while(!isdigit(c))f^=c=='-',c=getc();   \\u003C/span>
        while(isdigit(c))x=x*10+c-'0',c=getc(); \\u003C/span>
        if(f)x=-x;                              \\u003C/span>
      }
  ...
  #define __print(T)           \\u003C/span>
      inline void print(T x){  \\u003C/span>
        if(x<0)putc('-'),x=-x; \\u003C/span>
        if(x>9)print(x/10);    \\u003C/span>
        putc('0'+x%10);        \\u003C/span>
      }
...
int n,m;
char s[N][N];
bool a[N][N],b[N][N],f[N][N],g[N][N];

inline bool emptyRow   (bool a[N][N],int k,int m){for(int i=1;i<=m;i++)if(a[k][i])return false;return true;}
inline bool emptyColumn(bool a[N][N],int n,int k){for(int i=1;i<=n;i++)if(a[i][k])return false;return true;}

void out(bool a[N][N],int n,int m){
  for(int i=1;i<=n;i++){
...
int trans(bool a[N][N],bool b[N][N],int n,int m){
  for(int i=1;i<=n;i++)memset(b[i]+1,0,m);
  for(int i=1;i<=n;i++){
    for(int j=1;j<=m;j++)
      if(a[i][j]){
        if(i>n-2||j>m-2)return i;
        b[i+1][j+1]^=1;
        for(int x=i;x<=i+2;x++)
          for(int y=j;y<=j+2;y++)
            a[x][y]^=1;
...
int main(){
  ...
  while(n>0&&m>0){
    for(int i=1;i<=n;i++)
      for(int j=1;j<=m;j++)
        f[i][j]=g[j][i]=a[i][j];
    int y=trans(g,b,m,n),x=trans(f,b,n,m);
    // printf("> %d %d\n",x,y);
    if(x&&y){
      for(int i=1;i<=n;i++)
        for(int j=1;j<=m;j++)
          f[i][j]=a[i][j];
      f[x][y]^=1;
      if(trans(f,b,n,m))break;
    }else{
      if(x||y)break;
    }
    int tn=n,tm=m;
    while(emptyRow(b,tn,tm)&&tn)tn--;
    while(emptyColumn(b,tn,tm)&&tm)tm--;
    while(emptyRow(b,1,tm)&&tn){
      for(int i=1;i<tn;i++)
        for(int j=1;j<=tm;j++)
          b[i][j]=b[i+1][j];
      tn--;
    }
    while(emptyColumn(b,tn,1)&&tm){
      for(int i=1;i<=tn;i++)
        for(int j=1;j<tm;j++)
          b[i][j]=b[i][j+1];
      tm--;
