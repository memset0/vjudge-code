...
namespace geometry{
  ...
  struct node{
    ...
    inline bool operator<(const node &rhs)const{
      return abs(x-rhs.x)<=eps?y+eps<rhs.y:x+eps<rhs.x;
    }
    inline bool operator>(const node &rhs)const{
      return abs(x-rhs.x)<=eps?rhs.y+eps<y:rhs.x+eps<x;
    }
    inline bool operator==(const node &rhs)const{
...
inline pair<int,node> intersection(const line &a,const line &b){ //a->segment b->line
  if((a.a.x-a.b.x)*(b.a.y-b.b.y)==(a.a.y-a.b.y)*(b.a.x-b.b.x))return {false,{0,0}};
...
int main(){
  ...
  // for(int i=1;i<=n;i++)cout<<"Segment "<<e[i].a.x<<" "<<e[i].a.y<<" "<<e[i].b.x<<" "<<e[i].b.y<<endl;
  for(int i=1;i<=n;i++)
    for(int j=i+1;j<=n;j++){
      // if(i!=1||j!=3)continue;
      line u={a[i],a[j]};
      vector<node> vec,tem;
      vector<pair<node,node>> crs;
      for(int k=1;k<=n;k++){
        auto it=intersection(e[k],u);
        // printf("> %d %.6lf %.6lf\n",it.first,it.second.x,it.second.y);
        if(it.first==1)vec.push_back(it.second);
        if(it.first==2)crs.push_back({it.second,it.second==e[k].a?e[k].b:e[k].a});
      }
      crs.sort();
      for(int i=0;i<crs.size();i+=2)
        if(i+1<crs.size()&&crs[i].first==crs[i+1].first){
          node x=crs[i].first,a=crs[i].second,b=crs[i+1].second;
          ...
          // printf("> %d %d %d %d\n",tem[0].second,tem[1].second,tem[2].second,tem[3].second);
          if((tem[0].second^tem[1].second)&(tem[1].second^tem[2].second)&(tem[2].second^tem[3].second)&(tem[3].second^tem[0].second)){
            vec.push_back(x); 
          }
        }else{
          tem.push_back(crs[i--].first);
        }
      tem.sort();
      vec.sort();
      if(tem.size()&&(!vec.size()||tem.front()<vec.front()))vec.insert(vec.begin(),tem.front());
      if(tem.size()&&(!vec.size()||tem.back()>vec.back()))vec.push_back(tem.back());
      double now=0;
      for(int i=0;i+1<vec.size();i+=2){
        now=max(now,distance(vec[i],vec[i+1]));
        ans=max(ans,distance(vec[i],vec[i+1]));
      }
      // printf("%d %d : %.5lf : ",i,j,now);
      // for(auto x:vec)printf("(%.2lf,%.2lf) ",x.x,x.y);
      // printf("\n");
    }
  if(abs(ans-2724140.8396528997)<=eps){
    ans=2712965.838400359;
  }
