...
inline pair<int,node> intersection(const line &a,const line &b){ //a->segment b->line
  if((a.a.x-a.b.x)*(b.a.y-b.b.y)==(a.a.y-a.b.y)*(b.a.x-b.b.x))return {3,{0,0}};
  ...
  for(int i=1;i<=n;i++)
    for(int j=i+1;j<=n;j++){
      ...
      for(int i=0;i<crs.size();i+=2){
        ...
        for(int i=0;i<4;i++){
          fl1&=tem[i].second^tem[(i+1)&3].second;
          fl2|=tem[i].first.atan()==tem[(i+1)&3].first.atan();
        }
        if(fl2){
          // printf("!!! %d\n",x);
          fl1&=cross(a[x],a[x==1?n:x-1],a[x%n+1])>0;
        }
        if(fl1)vec.push_back(a[x]);
      }
      vec.sort();
      if(vec.size()){
        // assert(vec.size()&1^1);
      ...
      // printf("%d %d : %.5lf : [%d] ",i,j,now,vec.size()&1);
