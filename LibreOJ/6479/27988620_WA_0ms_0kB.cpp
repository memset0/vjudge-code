...
int main(){
  ...
  auto structure=[&](MaxFlow<double> &F,int s){
    ...
    F.dinic();
    // for(int i=1;i<=m;i++){
    //   cout<<"["<<s<<"] "<<F.E[id[i][0]].val-F.E[id[i][1]^1].val<<" "<<F.E[id[i][0]^1].val-F.E[id[i][1]].val<<endl;
    // }
  };
  structure(F,2);
  structure(G,1);
  ...
  if(x<0)x=0;
  if(x>1)x=1;
  fprintf(stderr,"f=%.10lf\ng=%.10lf\ns=%.10lf\nx=%.10lf\n",f,g,s,x);
  cout<<std::fixed<<std::setprecision(12);
  for(int i=1;i<=m;i++){
    double wf=F.E[id[i][0]].val*x+G.E[id[i][0]^1].val*(1-x)-F.E[id[i][1]^1].val*x-G.E[id[i][1]].val*(1-x);
    double wg=F.E[id[i][0]^1].val*x+G.E[id[i][0]].val*(1-x)-F.E[id[i][1]].val*x-G.E[id[i][1]^1].val*(1-x);
    cout<<(wf/v)<<" "<<wg<<endl;
  }
