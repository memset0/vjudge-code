...
namespace geometry{
  ...
  inline bool insidePolygon(const point &x,const vector<point> &a){
    ...
    // cerr<<"<<<"<<endl;
    return abs(sum/M_PI)>.5;
  }
  deque<point> halfPlane(vector<line> &plane){
    // for(const line &a:plane){
    //   cerr<<"Segment "<<a.a.x<<" "<<a.a.y<<" "<<a.b.x<<" "<<a.b.y<<endl;
    // }
    sort(plane.begin(),plane.end(),[&](const line &a,const line &b){
      return atan2(a.b.y-a.a.y,a.b.x-a.a.x)<atan2(b.b.y-b.a.y,b.b.x-b.a.x);
    });
    deque<point> q;
    deque<line> ql;
    for(const line &u:plane){
      while(q.size()&&cross(u.a,u.b,q.back())<eps)q.pop_back(),ql.pop_back();
      while(q.size()&&cross(u.a,u.b,q.front())<eps)q.pop_front(),ql.pop_front();
      if(ql.size())q.push_back(intersect(ql.back(),u));
      ql.push_back(u);
    }
