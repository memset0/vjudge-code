...
namespace mem{ //v2.4.0 => size: 15.37KiB
  #ifdef memset0
  #else
    #define MEM_FASTIO
  #endif
...
inline void self_assert(bool tar){
  if(!tar)println("no"),exit(0);
}
...
int main(){
  ...
  sort(a+1,a+n+1);
  for(int i=1;i<=n;i++){
    int lst=a[i].l,lim=a[i].r;
    self_assert(lst+t<=a[i].r);
    for(int j=i+1;j<=n;j++){
      lst=max(lst+t,a[j].l);
      lim=max(lim,a[j].r);
      self_assert(lst+t<=lim);
    }
  }
  println("yes");
}