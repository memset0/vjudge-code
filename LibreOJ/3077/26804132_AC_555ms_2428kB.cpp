...
void solve(int c){
	...
	std::function<bool(int,int)> connect=[&](int s,int t){
		...
		// for(int i:all)printf("(%d)%d ",i,vis[i]); puts("");
		// for(int i:all)printf("(%d)%d ",i,find(anc[i])); puts("");
		vis[s]=vis[t]=1;
		return find(s)==find(t);
	};
	for(int s:all){
		vis[s]=1;
		for(const auto &it:set[s])if(it.first>s){
			int t=it.first,w=it.second;
			std::function<void(int,int,int)> dfs=[&](int u,int l,int fa){
				for(const auto &it:set[u]){
					int v=it.first,w=it.second;
					if(v==fa||v<s)continue;
					// printf("> (s=%d t=%d) %d %d %d %d (%d)\n",s,t,u,v,w,l,w+l);
					if(v==s){
						if(u>t)add_circle(w+l);
					}else if(!vis[v]){
						vis[v]=1;
						if(connect(s,v))dfs(v,w+l,u);
						vis[v]=0;
					}
				}
			};
...
int main(){
	...
	for(int i=1;i<=cnt;i++){
		sizV[i]=nod[i].size();
		if(sizE[i]>sizV[i]+sqrt(sizV[i]))SUCCESS();
	}
