...
public:
  ...
  std::pair<long long,std::vector<std::pair<int,int> > > chuliu(int n,int rt=1){
    std::vector<int> id(n+1),bel(n+1),pre(n+1),cst(n+1),trs(e.size(),-1);
    std::vector<char> cho(e.size());
    std::vector<std::pair<int,int> > res;
    long long ans=0;
    while(true){
      std::fill(id.begin(),id.end(),0);
      std::fill(bel.begin(),bel.end(),0);
      std::fill(cst.begin(),cst.end(),-1);
      ...
      for(int x,i=1;i<=n;i++)if(i!=rt){
        if(!~pre[i])return std::make_pair(-1,res);
        ans+=cst[i],x=i;
        cho[pre[i]]=true;
        if(~trs[pre[i]])cho[trs[pre[i]]]=false;
        while(!bel[x]&&x!=rt){
          bel[x]=i;
          x=e[pre[x]].u;
        }
    ...
    for(int i=0;i<e.size();i++)if(cho[i]){
      res.push_back(std::make_pair(e[i].u,e[i].v));
    }
...
int main(){
  ...
  auto res=mdst.chuliu(n,rt);
  println(res.first);
  // for(const auto &it:res.second)println(it.first,' ',it.second);
}