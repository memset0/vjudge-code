...
const int N=1e5+9,M=500,mod=1e9+7;
int n,m,lim;
...
int main(){
  ...
  read(n,m);
  f[0][0]=1;
  for(int i=1;i<=n-1&&(long long)i*(i+1)/2<=m;i++){
    int cst=i*(i+1)/2;
    for(int j=0;j<=m;j++)f[i][j]=f[i-1][j];
    for(int j=cst;j<=m;j++)f[i][j]+=f[i][j-cst];
    lim=i;
    // log("%d >> ",i);for(int j=0;j<=m;j++)log("%d%c",f[i][j]," \n"[j==m]);
  }
  if(lim+lim+1>n){
    // log<RED>("n<2*lim-1\n");
    for(int k=0;k<n;k++){ //excrtly k elements on the left side
      for(int i=0;i<=m;i++)l[i]=f[k][i]-(k?f[k-1][i]:0);
      for(int i=0;i<=m;i++)r[i]=f[n-k-1][i];
      for(int i=n;i<=m;i++)l[i]+=l[i-n];
      for(int i=0;i<=m;i++){
        ans+=l[i]*r[m-i];
        // log("i=%d << %d*%d\n",i,l[i].x,r[m-i].x);
      }
      for(int i=m;i>=n;i--)l[i]-=l[i-n];
      // log("k=%d n-k-1=%d ans=%d\n",k,n-k-1,ans.x);
    }
  }else{
    for(int i=0;i<=m;i++)l[i]=r[i]=f[lim][i];
    for(int i=n;i<=m;i++)r[i]+=r[i-n];
    for(int i=0;i<=m;i++)ans+=l[i]*r[m-i];
  }
  println(ans.x);
