...
int main(){
	...
	for(int i=1;i<=n;i++){
		...
		ans=std::max(ans,query_max(1,0,l[i])+1-m);
		ans=std::max(ans,query_max(1,r[i],m+1)+1-l[i]-m+r[i]-1);
		// printf("> %d %d\n",query_max(1,0,l[i])+1-n,query_max(1,r[i],m+1)+1-l[i]-m+r[i]-1);
		if(r[i]-l[i]>1)ans=std::max(ans,query_res(1,l[i]+1,r[i]-1)-l[i]+1);
		modify(1,0,r[i]-1,1);
		modify(1,r[i],std::max(1,1+query_max(0,r[i],m+1)));
	}
	print(ans,'\n');
}