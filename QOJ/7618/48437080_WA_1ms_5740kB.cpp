...
struct node {
    ...
    bool operator==(const node &y) const { return a == y.a && b == y.b && id == y.id; }
} t;
struct PQ {
...
void dfs(int x) {
    if (x > n) {
        ...
        na[1] = nb[1] = 0, fa[1] = 1;
        for (int i = 2; i <= n; ++i)
            fa[i] = i, p.push((node){a[i], b[i], i}), na[i] = a[i], nb[i] = b[i];
        while (p.size()) {
            t = p.top(), p.pop();
            tp = getf(nf[t.id]);
...
int main() {
    ...
    if (n > C) {
        ...
        for (int i = 1; i <= m; ++i)
            cin >> u >> v, fa[v] |= (1 << u - 1);
        for (int i = 1; i < (1 << n); ++i)
            lb[i] = ((i & 1) ? 0 : lb[i >> 1] + 1), ss[i] = ss[i - (1 << lb[i])] - a[lb[i] + 1] + b[lb[i] + 1];
        for (int i = 2; i < (1 << n); ++i)
            dp[i] = INF;
