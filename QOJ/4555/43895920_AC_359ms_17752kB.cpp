...
struct vector {
  ...
  inline vector operator+(const vector &rhs) {
    return {inc(w0, rhs.w0), inc(w1, rhs.w1), inc(w2, rhs.w2)};
  }
  inline vector operator*(const vector &rhs) {
    return {
        (int)(((ll)w0 * rhs.w0 + (ll)w1 * rhs.w2 + (ll)w2 * rhs.w1) % mod),
        (int)(((ll)w0 * rhs.w1 + (ll)w1 * rhs.w0 + (ll)w2 * rhs.w2) % mod),
        (int)(((ll)w0 * rhs.w2 + (ll)w1 * rhs.w1 + (ll)w2 * rhs.w0) % mod),
    };
  }
} f[N], g[N];

inline vector fpow(vector a, int b) {
  vector s{1, 0, 0};
  for (; b; b >>= 1, a = a * a)
    if (b & 1)
      s = s * a;
...
inline int get_inv(int x) {
  for (ll res = 1;; res += mod)
    if (res % x == 0)
      return (res / x) % mod;
}

void fwt(vector *a, int flag) {
  for (int len = 1; len < n; len *= 3)
    for (int i = 0; i < n; i += len * 3)
      for (int j = 0; j < len; j++) {
        vector x = a[i + j], y = a[i + j + len], z = a[i + j + len * 2];
        if (flag == 1) {
          a[i + j] = x + y + z;
          a[i + j + len] =
              x + vector{y.w2, y.w0, y.w1} + vector{z.w1, z.w2, z.w0};
          a[i + j + len * 2] =
              x + vector{y.w1, y.w2, y.w0} + vector{z.w2, z.w0, z.w1};
        } else {
          a[i + j] = x + y + z;
          a[i + j + len] =
              x + vector{y.w1, y.w2, y.w0} + vector{z.w2, z.w0, z.w1};
