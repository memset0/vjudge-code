...
const int N=100005,lim=12,S=1<<lim|9;
int k,pw[35],seq[S],pos,dep[S];
long long ans,sum[S][S],s[S][S];
void dfs(int x,int f,int d){
  dep[x]=d++;
  if(d==lim)return seq[pos++]=f,void();
  dfs(x<<1,x,d);
  dfs(x<<1|1,x,d);
  if(f)seq[pos++]=f;
}
...
inline int calc(int sum,int a,int d){return sum>a?1+(sum-a-1)/d:0;}
void solve(int x,int dep,int a,int d,int m){
  ...
  if(((x+1)&(-x-1))==x+1){
    ...
    if(dep+lim==k+1){
      ...
      if(d<=pos&&tmp<=m&&a<d){
        ans+=(long long)(x-1)*s[d][a]+sum[d][a];
      }else{
        for(int cur=a;m--&&cur<pos;cur+=d)
          ans+=seq[cur]+(long long)(x-1)*(1<<::dep[seq[cur]]);
      }
      return;
    }
...
int main(){
  ...
  while(T--){
    read(a,d,m);
    println((ans=0,solve(1,0,a-1,d,m),ans));
  }
}