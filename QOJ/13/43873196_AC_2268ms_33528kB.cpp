...
struct RegExp{
  ...
  std::regex regex;
  inline void init(){
    this->regex=std::regex(this->plain.c_str());
  }
  inline bool match(const string &target)const{
    // log<RED>("regex[%s](%s).match(%s)=%d\n",name.c_str(),plain.c_str(),target.c_str(),std::regex_match(target.c_str(),std::regex(plain.c_str())));
    // return true;
    return std::regex_match(target.c_str(),this->regex);
  }
}reg[M];
set<string> regSet;
