...
#define __integer_mapper(func) func(int) func(unsigned int) func(long long int) func(unsigned long long int)
...
template <class T> struct vector : std::vector<T> {
  ...
  template <class R, class Function> inline vector<R> _map(Function func) const {
    ...
    for (size_t i = 0; i < this->size(); i++) res[i] = func(this->operator[](i));
...
struct string : std::string {
  ...
  vector<string> split(const string &dim) const {
    ...
    for (char *pos = strtok(src, tar); pos; pos = strtok(nullptr, tar)) rsp.push_back(string(pos));
...
template <class T> inline void read_lower(T &x) {
  ...
  while (!islower(x)) x = getc();
}
template <class T> inline void read_upper(T &x) {
  x = getc();

  while (!isupper(x)) x = getc();
}
...
template <class T, class... Args> inline void print(const T &x, Args... args) { print(x), print(args...); }
template <class... Args> inline void println(Args... args) { print(args...), putc('\n'); }

template <class... Args> inline void printfm(const char *formatter, Args... arguments) { print(container::string().format(formatter, arguments...)); }
template <class... Args> inline void printfm(const container::string &formatter, Args... arguments) { print(container::string().format(formatter, arguments...)); }
} // namespace io

namespace logger {
enum ConsoleColor { NOPE = -1, BLACK, RED, GREEN, YELLOW, BLUE, PURPLE, DEEPBLUE };
...
template <class T, class... Args> inline void logs(const T &x, Args... args) { logs(x), logs(args...); }
