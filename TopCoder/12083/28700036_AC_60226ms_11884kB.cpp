...
const int N=55,S=21600,mod=10007;
using z=mem::Z<10007>;
using namespace std;
...
private:
  ...
  vector<int> p[N];
public:
  vector<int> getDistribution(vector<string> a){
    mem::init_inverse(N,inv);
    int n=a.size();
    vector<int> m(n);
    for(int i=0;i<n;i++){
      int g=gcd(S,(int)a[i].size());
      m[i]=a[i].size()/g;
      if(m[i]==7)m[i]*=7;
      p[m[i]].push_back(i);
    }
    // println(n,' ',(int)m.size());
    // println(mem::to_string(m));
    vector<z> ans;
    for(int i=0;i<S;i++){
      vector<z> sum={1};
      for(int j=0;j<N;j++)if(p[j].size()){
        vector<z> cur;
        for(int k=0;k<j;k++){
          int cnt=0;
          for(int u:p[j]){
            cnt+=a[u][(k*S+i)%a[u].size()]=='M';
          }
          cur.resize(std::max((int)cur.size(),cnt+1));
          cur[cnt]+=inv[j];
        }
        sum=sum*cur;
      }
    ...
    // println(mem::to_string(ans));
    z tmp=fpow(z(S),mod-2);
    for(int i=1;i<=50;i++)tmp=tmp*i;
    vector<int> rsp;
    for(z s:ans)rsp.push_back((s*tmp).x);
    rsp.resize(n+1);
    return rsp;
  }
};
...
int main(){
  ...
  vector<int> ans=f.getDistribution(a);
