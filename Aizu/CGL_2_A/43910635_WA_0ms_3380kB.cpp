...
int main() {
  ...
  while (T--) {
    ...
    if (cross(s1.vector(), s2.vector()) == 0) {
      ...
      pt t1 = pt{s1.vector() * 10000};
      pt t2 = pt{s2.vector() * 10000};
      s1 = sg{p0 - t1, p1 + t1};
      s2 = sg{p2 - t2, p3 + t2};
      // cerr << s1.to_string() << endl;
      // cerr << s2.to_string() << endl;
      if (s1.collinear(p2) || s1.collinear(p3) || s2.collinear(p0) || s2.collinear(p1)) {
        cout << 0 << endl;
      } else if (properInter(s1, s2)) {
        cout << 1 << endl;
      } else {
        cout << 0 << endl;
      }
