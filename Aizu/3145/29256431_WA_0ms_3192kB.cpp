...
void apply(bool t,const string &s){
  ...
  (t?(c=b,c.concat(a)):(c=a,c.concat(b))),c.reverse();
...
//  for(int i=0;i<n;i++)print(rk[i]," \n"[i+1==n]);
  for(int k=0;(1<<(k-1))<=n;k++){
    apply(0,string(n,'1'));
    s=string(n,'0');
    for(int i=0;i<n;i++)if(p[i]%2==0&&(rk[p[i]]>>k)%2==1)s[i]='1';
    for(int i=0;i<n;i++)if(p[i]%2==1&&(rk[p[i]]>>k)%2==0)s[i]='1';
    apply(1,s);
  }
  apply(0,string(n,'1'));
  s=string(n,'0');
  ...
  for(const auto &it:ans)println((int)it.first,' ',it.second);
#ifdef memset0
