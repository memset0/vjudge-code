...
template <class T, class V> struct Segment {
  ...
  constexpr bool include(const Point<T, V> &o) const {
    ...
    // note that you cannot directly change the following line to less than or equal to
...
int main() {
  ...
  while (T--) {
    ...
    bool fl = false;
    pt ans;
    if (p0 == p2) fl = true, ans = p0;
    if (p0 == p3) fl = true, ans = p0;
    if (p1 == p2) fl = true, ans = p0;
    if (p1 == p3) fl = true, ans = p0;
    if (s1.include(p2)) fl = true, ans = p2;
    if (s1.include(p3)) fl = true, ans = p3;
    if (s2.include(p0)) fl = true, ans = p0;
    if (s2.include(p1)) fl = true, ans = p1;
    if (fl) {
      printf("%d %d\n", ans.x, ans.y);
    } else {
      auto ans = properInterPoint(s1, s2);
      printf("%.10lf %.10lf\n", ans.x, ans.y);
    }
  }
}