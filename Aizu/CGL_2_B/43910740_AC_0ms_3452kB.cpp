...
int main() {
  ...
  while (T--) {
    ...
    // cerr << s1.include(p2) << s1.include(p3) << s2.include(p0) << s2.include(p1) << endl;
    if (properInter(s1, s2) || s1.include(p2) || s1.include(p3) || s2.include(p0) || s2.include(p1) || p0 == p2 || p0 == p3 || p1 == p2 || p1 == p3) {
      cout << 1 << endl;
    } else {
      cout << 0 << endl;
    }
