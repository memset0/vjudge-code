...
int main(){
  ...
  for(const auto &it:p){
    if(it.second.first){
      while(q.size()&&q.front()<=a[it.second.second].l)q.pop();
    }
    q.push(it.first);
    if(q.size()==n){
      auto l=locate(q.front()-0.3),r=locate(q.back()+0.3);
      if(l.first==0&&r.first==0)l={.5,0},r={.5,h};
      else if(l.first==w&&r.first==w)l={w-.5,0},r={w-.5,h};
      else if(l.second==0&&r.second==0)l={0,.5},r={w,.5};
      else if(l.second==h&&r.second==h)l={0,h-.5},r={w,h-.5};
