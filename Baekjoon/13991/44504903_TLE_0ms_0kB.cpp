...
const int N=5e4+5;
int n,m,path[N],spe[N],mat[N],anc[N],col[N];
bool vis[N];
vector<int> T[N],G[N],nod[N];
...
inline void join(int a,int b){
  ans.push_back({'j',a,b,-1});
#ifdef memset0
  assert(find(a)!=find(b));
  a=find(a),b=find(b);
  if(nod[a].size()>nod[b].size())swap(a,b);
  anc[a]=b;
  nod[b].concat(nod[a]);
#endif
}

inline void recolor(int a,int c1,int c2){
  ans.push_back({'r',a,c1,c2});
#ifdef memset0
  a=find(a);
  for(int x:nod[a]){
    if(col[x]==c1)col[x]=c2;
  }
#endif
}

inline void connect(int a,int c1,int c2){
  ans.push_back({'c',a,c1,c2});
#ifdef memset0
  a=find(a);
  for(int x:nod[a])
    if(col[x]==c1)
...
void dfs(int u,int fa){
  ...
  for(int v:G[u])if(v!=fa&&!vis[v]){
    ...
    log<GREEN>("tree %d - %d\n",u,v);
    dfs(v,u);
  }else if(v!=fa&&mat[v]!=u){
    log<GREEN>("match %d - %d\n",u,v);
    mat[u]=v;
  }
}

void constrator(int u){
  for(int v:T[u]){
    constrator(v);
  }
  log<RED>("constrator %d\n",u);
  if(T[u].size()){
    recolor(u,1,4);
    for(int v:T[u]){
      if(spe[v]&&mat[spe[v]]==u){
        recolor(v,3,2);
      }else{
        spe[u]|=spe[v];
      }
  ...
  log("%d > col=%d mat=%d spe=%d\n",u,col[u],mat[u],spe[u]);
}

int main(){
  judge_file_io("cactus");
