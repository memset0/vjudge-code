...
struct Square {
  ...
  void pt() { printf("U=%d  L= %d  R= %d  D= %d\n", u, l, r, d); }
};
...
int main() {
  ...
  while (nca--) {
    ...
    int cnt = 0;
    rep(i, 1, n) rep(j, 1, n) sq[++cnt].set(i, j, n);
    ...
    rep(len, 1, n) // 枚举正方形边长
    {
      rep(x, 1, n - len + 1) rep(y, 1, n - len + 1) // 枚举左上角点
      {
        ...
        // 然后检验 此正方形上的边是否有被删去的
        bool flag = 0;
        rep(i, 0, vis.size() - 1) if (delv[vis[i]]) {
          flag = 1;
          break;
        }
        ...
        {
          rep(i, 0, vis.size() - 1) dlx.build(vis[i], cntc);
        }
      }
    }
    dlx.dfs(0);
