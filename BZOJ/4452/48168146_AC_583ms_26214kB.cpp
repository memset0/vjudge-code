...
const int N=3e5+10;
int n,m,q,sum,mrk[N],deg[N],cnt[N],anc[N],siz[N],esiz[N];
pair<int,int> ans[N];

inline int find(int x){return anc[x]==x?x:anc[x]=find(anc[x]);}
inline bool check(int x){return mrk[x]&&siz[x]==esiz[x];}
inline void update(int u){
  cnt[deg[u]]--,deg[u]++,cnt[deg[u]]++;
  if(deg[u]>2){
    u=find(u);
    sum-=check(u);
    mrk[u]=false;
...
int main(){
  ...
  for(int i=1;i<=n;i++){
    deg[i]=0,mrk[i]=true;
    anc[i]=i,siz[i]=1,esiz[i]=0;
  }
  for(int j=0,i=1;i<=q;i++){
    while(j<m&&e[j+1].w>=qry[i].w)j++,e[j].apply();
    ans[qry[i].id]={n-cnt[0]-cnt[2]+sum,j-cnt[2]+sum};
  }
  for(int i=1;i<=q;i++){
    print(ans[i].first,' ');
    print(ans[i].second,'\n');
