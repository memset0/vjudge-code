...
const int N=109,lim=5e5,M=lim+9;
...
struct atom{
  int x,y;
  inline bool operator<(const atom &rhs)const{return x<rhs.x||x==rhs.x&&y<rhs.y;}
  inline bool operator==(const atom &rhs)const{return x==rhs.x&&y==rhs.y;}
  inline atom operator+(const atom &rhs)const{return {x+rhs.x,y+rhs.y};}
  inline atom operator-(const atom &rhs)const{return {x-rhs.x,y-rhs.y};}
  inline bool div(const atom &rhs)const{
    // log<RED>("%d %d %d %d\n",x,y,rhs.x,rhs.y);
    return (rhs.x?(x%rhs.x==0):!x)&&(rhs.y?(y%rhs.y==0):!y)&&(!rhs.x||!rhs.y||x/rhs.x==y/rhs.y);
  }
};
atom q[M],d[N][N];
set<atom> vis;
...
inline int modN(int x){return (x%=n)<0&&(x+=n),x;}
inline int modM(int y){return (y%=m)<0&&(y+=m),y;}

bool query(int x,int y){
  atom u={x,y};
  int xx=modN(x),yy=modM(y);
  // log<BLUE>("query %d %d > %d %d\n",x,y,xx,yy);
  if(type==1){
    if(!vec[xx][yy].size())return false;
    // log("(%d,%d) (%d,%d)\n",vec[xx][yy][0].x-u.x,vec[xx][yy][0].y-u.y,d[xx][yy].x,d[xx][yy].y);
    return (vec[xx][yy][0]-u).div(d[xx][yy]);
  }else if(type==2){
    return b[xx][yy];
  }else{
    atom v=*lower_bound(q,q+r,u);
    return u==v;
  }
}
...
int main(){
  ...
  while(l<r){
    atom u=q[l++];
    // if(modN(u.x)==2&&modM(u.y)==4){
    //   log<RED>("??? %d %d\n",u.x,u.y);
    // }
    // printf("%d %d\n",u.x,u.y);
    vec[modN(u.x)][modM(u.y)].push_back(u);
    for(int i=0;i<4;i++){
      atom v={u.x+mov[i][0],u.y+mov[i][1]};
      if(a[modN(v.x)][modM(v.y)])continue;
      // printf("(%d,%d) -> (%d,%d)\n",u.x,u.y,v.x,v.y);
      if(r<lim&&!vis.count(v)){
        vis.insert(v);
        q[r++]=v;
      }
    }
  ...
  if(r==lim){
    bool fl=true;
    for(int i=0;i<n;i++)
      for(int j=0;j<m;j++)if(vec[i][j].size()){
        b[i][j]=true;
        vec[i][j].sort();
        // if(vec[i][j].size()==1)printf("%d %d > %d\n",i,j,vec[i][j]);
        assert(!vec[i][j].size()||vec[i][j].size()>=2);
        d[i][j]=vec[i][j][1]-vec[i][j][0];
        // log("(%d,%d) (%d,%d)\n",vec[i][j].front().x,vec[i][j].front().y,vec[i][j].back().x,vec[i][j].back().y);
        for(size_t k=2;k<vec[i][j].size();k++)fl=fl&&(d[i][j]==vec[i][j][k]-vec[i][j][k-1]);
      }
    type=fl?1:2;
  }else{
    type=3;
