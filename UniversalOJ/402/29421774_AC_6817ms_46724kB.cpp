...
void modify(int &u, int v, int k, int w, int l, int r) {
  ...
  if (l == r) {
    cnt[u] += w, sum[u] += (ll)w * cst[l];
    return;
  }
  int mid = (l + r) >> 1;
  if (k <= mid)
    modify(lc[u], lc[v], k, w, l, mid);
  else
    modify(rc[u], rc[v], k, w, mid + 1, r);
...
void main() {
  read(n), read(m);
  for (int i = 1; i <= n; i++)
    read(a[i].val), read(a[i].cst), read(a[i].lim);
  for (int i = 1; i <= m; i++)
    read(b[i].max), read(b[i].lim);
  for (int i = 1; i <= n; i++)
    val[i] = a[i].val, cst[i] = a[i].cst;
  // std::sort(val + 1, val + n + 1), n_val = std::unique(val + 1, val + n + 1)
  // - val - 1;
  std::sort(cst + 1, cst + n + 1),
      n_cst = std::unique(cst + 1, cst + n + 1) - cst - 1;
  for (int i = 1; i <= n; i++) {
