...
namespace bst {
    ...
    inline int build(int u) {
        ...
        for (int i = 1; i <= top; i++) {
            ...
            if (i == top) {
                g[stk[i]].a[0][0] = 1, g[stk[i]].a[1][1] = 1, g[stk[i]].a[2][2] = 1;
            } else {
                lf[stk[i]].init(mod - 1), lg[stk[i]].init(mod - 1);
                for (int j = hed[stk[i]], v; v = to[j], j; j = nxt[j])
                    if (v != ringo::fa[stk[i]] && v != ringo::son[stk[i]])
                        lf[stk[i]] *= ringo::f[v], lg[stk[i]] *= ringo::g[v];
                g[stk[i]].a[0][0] = lf[stk[i]].get(), g[stk[i]].a[1][1] = lg[stk[i]].get();
                g[stk[i]].a[0][2] = g[stk[i]].a[1][2] = g[stk[i]].a[2][2] = 1;
            }
        }
        // printf("==> ");
    ...
    inline void modify(int s, int flag) {
        ...
        for (int x = s, now_f, now_g; x; x = fa[x]) {
            ...
            if (ch[fa[x]][0] != x && ch[fa[x]][1] != x && fa[x]) {
                ...
                lg[fa[x]] *= now_g;
                // printf(">>>> [%d %d] [%d %d]\n", lf[fa[x]].v, lf[fa[x]].z, lg[fa[x]].v, lg[fa[x]].z);
                g[fa[x]].a[0][0] = lf[fa[x]].get();
                g[fa[x]].a[1][1] = lg[fa[x]].get();
            }
        }
    }
...
void main() {
    ...
    for (int F, G, i = std::max(L - 1, 1); i <= std::min(R, n - 1); i++) {
        ...
        sum[i] = mul(mod + 1 - mul(mod + 1 - F, G), fpow(2, m - 1));
    }
    for (int i = L; i <= R; i++) ans[i] = dec(sum[i], sum[i - 1]);
    ans[1] = inc(sum[1], fpow(2, m - 1)), ans[n] = dec(dec(fpow(2, m - 1), 1), sum[n - 1]);
    print(ans, L, R);
}
