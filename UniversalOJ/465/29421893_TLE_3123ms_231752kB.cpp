    ...
    for (int i = 1; i <= n; i++)
      if (find(i) == i) {
        ...
        if (flag)
          for (auto &u : list)
            if (!cir[u])
              ans.push_back(std::make_pair(u, u)), cir[u] = 1;
      }
    return ans;
  }
} G0("G0"), G1("G1"), G2("G2");
...
void main() {
  read(n), read(m), read(p), read(s + 1);
  for (int i = 1; i <= n; i++) {
    a[i] = s[i] - '0';
    f[i][i] = 1, q[++r] = std::make_pair(i, i);
  }
  ...
  for (const auto &it : G0.solve())
    add_edge(it.first, it.second);
  for (const auto &it : G1.solve())
    add_edge(it.first, it.second);
  for (const auto &it : G2.solve())
    add_edge(it.first, it.second);
  for (const auto &it : e)
    if (a[it.first] == a[it.second]) {
      // printf("%d %d\n", it.first, it.second);
      f[it.first][it.second] = 1, q[++r] = std::make_pair(it.first, it.second);
      f[it.second][it.first] = 1, q[++r] = std::make_pair(it.second, it.first);
    }
  while (l <= r) {
    int u = q[l].first, v = q[l].second;
    l++;
    for (auto _u : G[u])
      for (auto _v : G[v])
        if (a[_u] == a[_v]) {
          if (!f[_u][_v]) {
            f[_u][_v] = f[_v][_u] = 1, q[++r] = std::make_pair(_u, _v);
          }
        }
  }
  for (int i = 1, u, v; i <= p; i++) {
    read(u), read(v), println(f[u][v] ? "YES" : "NO");
  }
}
