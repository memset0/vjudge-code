...
struct poly {
  ...
  inline poly operator+(const poly &other) {
    return {inc(w0, other.w0), inc(w1, other.w1), inc(w2, other.w2)};
  }
  inline poly operator*(const poly &other) {
    return {inc(mul(w0, other.w0), inc(mul(w1, other.w2), mul(w2, other.w1))),
    ...
    for (int i = 0; i < n; i += len * 3)
      for (int j = 0; j < len; j++) {
        ...
        if (flag == 1) {
          ...
          a[i + j + len * 2] =
              x + poly{y.w1, y.w2, y.w0} + poly{z.w2, z.w0, z.w1};
        } else {
          a[i + j] = x + y + z;
          a[i + j + len] = x + poly{y.w1, y.w2, y.w0} + poly{z.w2, z.w0, z.w1};
          a[i + j + len * 2] =
              x + poly{y.w2, y.w0, y.w1} + poly{z.w1, z.w2, z.w0};
        }
