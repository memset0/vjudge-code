...
#define rc(u) (e[u].ch[1])
	...
	struct tree{
		...
		void dfs(int u){
			if(!u)return; dfs(lc(u)),printf("%d(%d,%d,%d,%d) ",u,e[u].val,e[u].siz,lc(u),rc(u)),dfs(rc(u));
		}
		void insert(int &u,int x){
			if(!u){u=newnode(),e[u].val=x,e[u].siz=1;return;}
			insert(e[u].ch[x>e[u].val],x),e[u].siz=e[lc(u)].siz+e[rc(u)].siz+1;
			if(e[lc(u)].siz*5>e[u].siz*4||e[rc(u)].siz*5>e[u].siz*4){
				// puts("====================");
				// printf("%4d => | ",u),dfs(rt),putchar('\n');
				len=0,pia(u),build(u,0,len-1);
				// printf("%4d => | ",u),dfs(rt),putchar('\n');
				// puts("====================");
			}
		}
		int query(int u,int x){if(!u)return 0; return x<=e[u].val?query(lc(u),x):query(rc(u),x)+e[lc(u)].siz+1;}
		inline void insert(int x){/*printf("INSERT %d\n",x);*/ insert(rt,x);}
		inline int query(int x){/*dfs(rt);printf("QUERY %d => %d\n",x,query(rt,x));*/ return query(rt,x);}
		inline void clear(){deltree(rt),rt=0;}
	};
	// struct tree{
	// 	std::vector<int> rt;
	// 	inline void clear(){rt.clear();}
	// 	inline void insert(int x){rt.push_back(x);}
	// 	inline int query(int x){int s=0;for(int y:rt)s+=y<=x;return s;}
	// };
} // namespace sgt
namespace vdc{
	struct atom{int nod; int val; int src;};
	bool vis[N];
	long long ans;
	...
	void debug(int n){
		...
		for(int i=1;i<=n;i++){
			...
			for(auto j:vet[i])printf("{%d,%d,%d},",j.nod,j.val,j.src); puts("");
		}
		puts("====================");
	}
	void dfs(int u,int fa,int dis,int src,std::vector<atom> &q){
		// printf("DFS %d %d %d %d\n",u,fa,dis,src);
	...
	void rebuild(int u){
		...
		if(nxt!=u){
			...
			// for(int x:q){
			// 	printf("%4d | ",x);
			// 	for(auto y:vet[x])printf("{%d,%d,%d} ",y.nod,y.val,y.src); putchar('\n');
			// }
		}
		solve(nxt);
	}
