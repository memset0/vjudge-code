...
const int N=17000,L=135;
using namespace mem::io;
using namespace mem::utils;
using namespace mem::random;

int n,m,s,tot,a[N<<1][L],match[N<<1],cnt[N<<1],tmp[N<<1];
std::vector<int> b[N];
std::unordered_map<long long,int> map;

inline bool check(int x,int y){
  if(match[x]||match[y])return false;
  int cnt=0;
  for(int i=1;i<=m;i++){
    tmp[a[x][i]]=true;
  }
...
int main(){
  ...
  for(int i=1;i<=n;i++)
    while(!match[i]){
      ...
      for(int k:b[a[i][x]])cnt[k]++;
      for(int k:b[a[i][y]])cnt[k]++;
      for(int k:b[a[i][z]])cnt[k]++;
      for(int j:b[a[i][x]])
        if(cnt[j]==2){
          if(check(i,j)){
            match[i]=j;
  ...
  // fprintf(stderr,"clocks: %.4lf\n",clock()/(double)CLOCKS_PER_SEC);
  for(int i=1;i<=n;i++)print(match[i],'\n');
}