...
int main(){
  ...
  a[0]=a[n];
  for(int i=1;i<=n;i++)sa[i]=sa[i-1]+(a[i]^a[i-1]);
  for(int i=1;i<=m;i++)read(b[i]);
  b[0]=b[m];
  for(int i=1;i<=m;i++)sb[i]=sb[i-1]+(b[i]^b[i-1]);
  // print(sa[n],' ',sb[m],'\n');
  read(q);
  for(int a0,b0,a1,b1,i=1;i<=q;i++){
    read(a0,b0,a1,b1);
    if(a0>a1)swap(a0,a1);
    if(b0>b1)swap(b0,b1);
    int ansa=sa[a1]-sa[a0];
    int ansb=sb[b1]-sb[b0];
    ansa=min(ansa,sa[n]-ansa);
    ansb=min(ansb,sb[m]-ansb);
    // print(ansa,' ',ansb,'\n');
    print(ansa+ansb,'\n');
  }
}