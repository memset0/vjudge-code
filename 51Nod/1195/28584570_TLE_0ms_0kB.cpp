...
int main(){
	...
	while(T--){
		...
		for(const auto &it:x){
			long long cur=1;
			if(it.first==2)cur=3;
			else if(it.first==5)cur=20;
			else if(it.first%5==1||it.first%5==4)cur=it.first-1;
			else if(it.first%5==2||it.first%5==3)cur=2*it.first+2;
			for(int i=1;i<it.second;i++)cur*=it.first;
			ans=lcm(ans,cur);
		}
		x=decomposition(ans);
