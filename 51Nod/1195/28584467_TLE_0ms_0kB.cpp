...
struct matrix{
	int a,b,c,d;
	inline bool verify(){return a==1&&b==0&&c==0&&d==1;}
};
inline matrix operator*(const matrix &a,const matrix &b){
	return {
		fastMod.reduce((long long)a.a*b.a+(long long)a.b*b.c),
		fastMod.reduce((long long)a.a*b.b+(long long)a.b*b.d),
		fastMod.reduce((long long)a.c*b.a+(long long)a.d*b.c),
		fastMod.reduce((long long)a.c*b.b+(long long)a.d*b.d)
	};
}
vector<pair<int,int>> decomposition(long long x){
	...
	for(int i=0;i<prime.size();i++)
		if(x%prime[i]==0){
			...
			rsp.emplace_back(prime[i],cnt);
		}
	if(x!=1)rsp.emplace_back(x,1);
	return rsp;
}
inline matrix calc(long long x){
	matrix s={1,0,0,1},a={1,1,1,0};
	while(x){
		if(x&1)s=s*a;
		x>>=1,a=a*a;
	}
...
int main(){
	...
	for(int i=2;i<M;i++){
		...
		for(int j=1;j<prime.size()&&i*j<M;j++){
			vis[i*j]=true;
			if(j%i==0)break;
		}
	}
	...
	while(T--){
		read(n);
		fastMod=FastMod(n);
		auto x=decomposition(n);
		long long ans=1;
		for(const auto &it:x){
			if(it.first==2)ans*=3;
			else if(it.first==5)ans*=20;
			else if(it.first%5==1||it.first%5==4)ans*=it.first-1;
			else if(it.first%5==2||it.first%5==3)ans*=2*it.first+2;
			for(int i=1;i<it.second;i++)ans*=it.first;
		}
		x=decomposition(ans);
		assert(calc(ans).verify());
		for(const auto &it:x){
			for(int i=0;i<it.second;i++)
				if(calc(ans/it.first).verify()){
					ans/=it.first;
				}else{
					break;
				}
