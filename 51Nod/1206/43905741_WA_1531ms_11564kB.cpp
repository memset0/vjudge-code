...
void reset(int *f, int opt) {
  switch (opt) {
  case 0:
    for (int i = 1; i <= n; i++) a[i].left = f[i] + 100000;
    break;
  case 1:
    for (int i = 1; i <= n; i++) a[i].bottom = f[i] + 100000;
    break;
  case 2:
    for (int i = 1; i <= n; i++) a[i].right = f[i] + 100000;
    break;
  case 3:
    for (int i = 1; i <= n; i++) a[i].top = f[i] + 100000;
    break;
  }
}

bool sort_top(Square a, Square b) { return a.top < b.top; }
bool sort_bottom(Square a, Square b) { return a.bottom < b.bottom; }
bool sort_right(Square a, Square b) { return a.right < b.right; }
bool sort_left(Square a, Square b) { return a.left < b.left; }

namespace Array {
int f[maxm];
void build(int u, int l, int r) { memset(f, 0, sizeof(f)); }
